...
class ITEM_MANAGER : public singleton<ITEM_MANAGER>
{
	...
	public:
		...
		const static int32_t MAX_NORM_ATTR_NUM = ITEM_ATTRIBUTE_NORM_NUM; // @correction105
		const static int32_t MAX_RARE_ATTR_NUM = ITEM_ATTRIBUTE_RARE_NUM; // @correction105
...
#endif
	public:
		uint8_t GetInventoryType(uint32_t dwVnum, TItemTable* pkItemTable = nullptr);
#ifdef __PICK_ALL_ITEMS__
	public:
		std::map<uint32_t, LPITEM>& GetItemMapByID() { return m_map_pkItemByID; }
#endif
};
