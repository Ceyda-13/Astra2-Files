...
bool DragonSoulTable::CheckRefineStrengthTables()
{
	...
	for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
	{
		for (int32_t j = MATERIAL_DS_REFINE_NORMAL; j < MATERIAL_MAX_NUM; j++)
		{
			uint64_t fee;
			float prob;
...
bool DragonSoulTable::GetRefineGradeValues(uint8_t ds_type, uint8_t grade_idx, OUT int32_t& need_count, OUT uint64_t& fee, OUT std::vector<float>& vec_probs)
{
...
bool DragonSoulTable::GetRefineStepValues(uint8_t ds_type, uint8_t step_idx, OUT int32_t& need_count, OUT uint64_t& fee, OUT std::vector<float>& vec_probs)
{
...
bool DragonSoulTable::GetRefineStrengthValues(uint8_t ds_type, uint8_t material_type, uint8_t strength_idx, OUT uint64_t& fee, OUT float& prob)
{
	if (material_type < MATERIAL_DS_REFINE_NORMAL || material_type >= MATERIAL_MAX_NUM)
	{
	...
	std::string stStrengthIdx = std::to_string((int32_t)strength_idx);
