...
bool CHackShieldImpl::CreateClientHandle(uint32_t dwPlayerID)
{
#ifdef ENABLE_HSHIELD_SYSTEM
...
void CHackShieldImpl::DeleteClientHandle(uint32_t dwPlayerID)
{
#ifdef ENABLE_HSHIELD_SYSTEM
...
bool CHackShieldImpl::SendCheckPacket(LPCHARACTER ch)
{
	...
	uint32_t ret = _AhnHS_MakeRequest( iter->second, &(pack.Req) );
...
bool CHackShieldImpl::VerifyAck(LPCHARACTER ch, TPacketGCHSCheck* buf)
{
	...
	uint32_t ret = _AhnHS_VerifyResponseEx( iter->second, buf->Req.byBuffer, buf->Req.nLength, &dwError );
