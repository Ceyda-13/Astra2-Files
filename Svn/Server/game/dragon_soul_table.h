...
public:
	...
	bool	GetRefineGradeValues(uint8_t ds_type, uint8_t grade_idx, OUT int32_t& need_count, OUT uint64_t& fee, OUT std::vector<float>& vec_probs);
	bool	GetRefineStepValues(uint8_t ds_type, uint8_t step_idx, OUT int32_t& need_count, OUT uint64_t& fee, OUT std::vector<float>& vec_probs);
	bool	GetRefineStrengthValues(uint8_t ds_type, uint8_t material_type, uint8_t strength_idx, OUT uint64_t& fee, OUT float& prob);
