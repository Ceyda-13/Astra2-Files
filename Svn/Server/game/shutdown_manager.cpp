...
CShutdownManager::CShutdownManager() : m_pTime(nullptr), m_lTime(0), m_bShutdownAlarm(false)
{
	m_lstDesc.clear();
}
...
{
	if(!CheckLocale() || pDesc == nullptr || SearchDesc(pDesc) != -1) return;
...
{
	if(!CheckLocale() || pDesc == nullptr) return;

	int32_t nIndex = SearchDesc(pDesc);
	if(nIndex < 0) return;
...
int32_t CShutdownManager::SearchDesc(LPDESC pDesc)
{
	if(!CheckLocale()) return -1;

	std::list<LPDESC>::iterator iter = m_lstDesc.begin();
	for(uint32_t i = 0; i < m_lstDesc.size(); i++)
	{
...
bool CShutdownManager::CheckShutdownAge(const char* szSocialID)
{
	...
	int32_t nAge = CharToInt(szSocialID[0]) * 10 + CharToInt(szSocialID[1]);
	...
	if(nAge < gShutdownAge) // @correction138
		return true;
	else
		return false;
}
...
bool CShutdownManager::CheckCorrectSocialID(const char* szSocialID)
{
	...
	if(szSocialID == nullptr || strlen(szSocialID) != 13) return false;

	UpdateTime();

	int32_t nMonth = CharToInt(szSocialID[2]) * 10 + CharToInt(szSocialID[3]);
	int32_t nDay = CharToInt(szSocialID[4]) * 10 + CharToInt(szSocialID[5]);
	int32_t nGender = CharToInt(szSocialID[6]);
