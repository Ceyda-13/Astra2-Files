...
const static int32_t MAX_ROWS = 6;
const static int32_t MAX_COLS = 8;
const static uint32_t MAX_LENS = (MAX_ROWS * MAX_COLS * 2);
const static uint32_t MAX_LENE = (MAX_LENS * 2);
const static int32_t ASLENGTH = 8;

bool EncodeMatrix(const char* szsrc, const char* szpwd, char* lpdes, const uint32_t usize)
{
	int32_t nlen = strlen(szsrc);
	int32_t i;

	if (strlen(szsrc) != MAX_LENS || strlen(szpwd) != MAX_LENS || lpdes == nullptr || usize < (MAX_LENE + 1))
		return false;
...
bool DecodeMatrix(const char* szsrc, const char* szpwd, char* lpdes, const uint32_t usize)
{
	int32_t nlen = strlen(szpwd);
	int32_t i;

	if (strlen(szsrc) != MAX_LENE || strlen(szpwd) != MAX_LENS || lpdes == nullptr || usize < (MAX_LENS + 1))
		return false;
...
void MatrixCardRndCoordinate(uint32_t & rows, uint32_t & cols)
{
	for (uint32_t i = 0; i < (ASLENGTH >> 1); i++)
	{
...
bool ChkCoordinate(const uint32_t rows, const uint32_t cols, const char* matrix, const char* answer)
{
	uint32_t max_lens = strlen(matrix);
	int32_t answer_lens = strlen(answer);
	...
	uint16_t * pmatrix = (uint16_t *)matrix;
	uint16_t * panswer = (uint16_t *)answer;
...
bool MatrixCardCheck(const char * src, const char * answer, uint32_t rows, uint32_t cols)
{
...
void GetRightAnswer(const uint32_t rows, const uint32_t cols, const char* matrix, char* answer, const uint32_t nsize)
{
	if (strlen(matrix) != MAX_LENS || answer == nullptr || nsize < (ASLENGTH + 1))
		return;
...
int32_t main(int32_t argc, char* argv[])
{
	...
	uint32_t rand_rows = 0, rand_cols = 0;
	MatrixCardRndCoordinate(rand_rows, rand_cols);
	...
	char answer[9];
	GetRightAnswer(rand_rows, rand_cols, szmatrix, answer, sizeof(answer));
	printf("Answer: %s\r\n", answer);

	bool f = ChkCoordinate(rand_rows, rand_cols, szmatrix, answer);
	printf("Result: %s\n", (f) ? "true" : "false");
	return 1;
}
