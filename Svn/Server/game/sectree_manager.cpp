#include "stdafx.h"
#include <sstream>
#include "../libsrc/libthecore/targa.h"
#include "../libsrc/libthecore/attribute.h"
#include "config.h"
#include "utils.h"
...
{
	if (lMapIndex >= 1 && lMapIndex <= 2)
		return 1;

	if (lMapIndex >= 3 && lMapIndex <= 4)
		return 2;

	if (lMapIndex >= 5 && lMapIndex <= 6)
		return 3;
...
struct FCountSpecifiedMonster
{
	...
	bool bForceVnum;
	FCountSpecifiedMonster(uint32_t id, bool force) : SpecifiedVnum(id), cnt(0), bForceVnum(force) {}
#else
...
#ifdef __GUILD_DRAGONLAIR__
size_t SECTREE_MANAGER::GetMonsterCountInMap(int32_t lMapIndex, uint32_t dwVnum, bool bForceVnum)
#else
...
{
	SRestartCityPos(int32_t i2Empire = 0, int32_t i2X = 0, int32_t i2Y = 0, int32_t i2Z = 0) : iEmpire(i2Empire), iX(i2X), iY(i2Y), iZ(i2Z) {}
	int32_t iEmpire, iX, iY, iZ;
};

static std::map<int32_t, SRestartCityPos> m_restart_city_pos;

void SECTREE_MANAGER::GetRestartCityPos(int32_t iMapIndex, int32_t iEmpire, int32_t &iTargetX, int32_t &iTargetY, int32_t &iTargetZ)
{
...
void SECTREE_MANAGER::AddRestartCityPos(int32_t iMapIndex, int32_t iEmpire, int32_t iX, int32_t iY, int32_t iZ)
{
	int32_t iTargetX = 0, iTargetY = 0, iTargetZ = 0;
...
#ifdef __SHOW_MAP_BOSS_AND_STONE__
void SECTREE_MANAGER::RemoveNPCPosition(int32_t lMapIndex, int32_t x, int32_t y, uint32_t vid)
{
	if (m_mapNPCPosition[lMapIndex].empty())
		return;
	...
	for (auto it = m_mapNPCPosition[lMapIndex].begin(); it != m_mapNPCPosition[lMapIndex].end(); ++it)
	{
		if (it->vid == vid)
		{
			m_mapNPCPosition[lMapIndex].erase(it);
			break;
		}
	}
}
