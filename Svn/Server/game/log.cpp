...
{
	LOG_LEVEL_CHECK_N_RET(LOG_LEVEL_MIN); // @correction222
...
void LogManager::MoneyLog(uint8_t type, uint32_t vnum, uint64_t gold)
{
	LOG_LEVEL_CHECK_N_RET(LOG_LEVEL_MAX); // @correction222
	if (type == MONEY_LOG_RESERVED || type >= MONEY_LOG_TYPE_MAX_NUM)
	{
		sys_err("TYPE ERROR: type %d vnum %u gold %llu", type, vnum, gold);
		return;
	}

	Query("INSERT DELAYED INTO money_log%s VALUES (NOW(), %d, %d, %llu)", get_table_postfix(), type, vnum, gold);
}

void LogManager::HackLog(const char * c_pszHackName, const char * c_pszLogin, const char * c_pszName, const char * c_pszIP)
{
	LOG_LEVEL_CHECK_N_RET(LOG_LEVEL_MID); // @correction222
