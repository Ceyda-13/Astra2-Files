...
#ifdef ENABLE_PC_OPENSHOP // @correction157
#include "shop.h"
...
namespace quest
{
	...
	int32_t pc_give_gold(lua_State* L)
	{
		...
		uint64_t iAmount = (uint64_t) lua_tonumber(L, 1);
		...
		ch->ChangeGold(true, iAmount, true);
		return 0;
	}
	...
	int32_t pc_give_item_from_special_item_group(lua_State* L)
	{
		...
		int32_t count = 0;
		uint16_t wCount = 1;
		ch->GiveItemFromSpecialItemGroup(dwGroupVnum, dwVnums, dwCounts, item_gets, count, wCount);
	...
	int32_t pc_set_level(lua_State * L)
	{
		...
		else
		{
			...
			ch->PointChange(POINT_STAT, ((MINMAX(1, newLevel, gPlayerMaxLevel) - ch->GetLevel()) * 3) + ch->GetPoint(POINT_LEVEL_STEP)); // @correction001
	...
	int32_t pc_get_wear(lua_State * L)
	{
		...
		LPITEM item = CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(wCell);
	...
	int32_t pc_change_money(lua_State * L)
	{
		...
		if ((int64_t)(gold + ch->GetGold()) < 0)
			sys_err("QUEST wrong ChangeGold %lld (now %llu)", gold, ch->GetGold());
		else
		{
			DBManager::instance().SendMoneyLog(MONEY_LOG_QUEST, ch->GetPlayerID(), gold);
			if (gold < 0)
				ch->ChangeGold(false, -gold, false);
			else
				ch->ChangeGold(true, gold, true);
		}
	...
	int32_t pc_give_exp_perc(lua_State* L)
	{
		...
		uint32_t exp = (uint32_t)((exp_table[MINMAX(0, lev, PLAYER_MAX_LEVEL_CONST)] * proc) / 100); // @correction001 PLAYER_EXP_TABLE_MAX -> PLAYER_MAX_LEVEL_CONST
		PC * pPC = CQuestManager::instance().GetCurrentPC();
	...
	int32_t pc_set_skillgroup(lua_State* L)
	{
		...
		else
		{
			CQuestManager & q = CQuestManager::Instance();
			LPCHARACTER ch = q.GetCurrentCharacterPtr();
			ch->RemoveGoodAffect(true); // @correction098
...
// @correction229 END
	...
	int32_t pc_diamond_refine(lua_State* L)
	{
		...
		uint64_t cost = (uint64_t) lua_tonumber(L, 1);
...
teleport_area:
	...
	int32_t pc_change_empire(lua_State* L)
	{
		...
		if (ch->GetParty())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("변경 시킬 속성이 없습니다"));
			lua_pushnumber(L, 4);
			return 0;
		}
		// @correction079 END
	...
	{
#ifdef ENABLE_LOCALECHECK_CHANGENAME // @correction156
		lua_pushnumber(L, 5);
		return 1;
#endif
	...
	{
		LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();
		uint8_t bType = (uint8_t)lua_tonumber(L, 1);

		if ( pChar != nullptr )
		{
			lua_pushnumber(L, pChar->CountEmptyInventory(bType));
		}
		else
		{
			lua_pushnumber(L, 0);
		...
		if (nullptr != pChar)
		{
			...
			if (nullptr != Unique1)
			{
				if (UNIQUE_GROUP_SPECIAL_RIDE == Unique1->GetSpecialGroup())
				{
					...
					lua_pushnumber(L, Unique1->GetSocket(0)); // @correction069 (2->0)
					return 2;
				}
			}
			...
			if (nullptr != Unique2)
			{
				if (UNIQUE_GROUP_SPECIAL_RIDE == Unique2->GetSpecialGroup())
				{
					...
					lua_pushnumber(L, Unique2->GetSocket(0)); // @correction069 (2->0)
					return 2;
				}
			}
	...
	int32_t pc_give_poly_marble(lua_State* L)
	{
		...
		int32_t iEmptyCell = ch->GetEmptyInventory(item);
	...
	#define PC_MI0L_ARG1	2
	#define PC_MI0L_ARG2	3
	#define PC_MI0L_ARG3	7*2
	...
	int32_t pc_make_item0(lua_State* L)
	{
		...
		int32_t m_socket[ITEM_SOCKET_MAX_NUM] = {0};
		m_idx = 0;
		lua_pushnil(L);
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		int32_t amount = MINMAX(0, lua_tonumber(L, 1), JOB_MAX_NUM);
	...
	int32_t pc_get_exp_level(lua_State* L)
	{
		...
		lua_pushnumber(L, (uint32_t)(exp_table[MINMAX(0, lua_tonumber(L, 1), PLAYER_MAX_LEVEL_CONST)] / 100)); // @correction001 PLAYER_EXP_TABLE_MAX -> PLAYER_MAX_LEVEL_CONST
		return 1;
	}
	...
	int32_t pc_get_exp_level0(lua_State* L)
	{
		...
		lua_pushnumber(L, (uint32_t)((exp_table[MINMAX(0, lua_tonumber(L, 1), PLAYER_MAX_LEVEL_CONST)] / 100) * MINMAX(1, lua_tonumber(L, 2), 100))); // @correction001 PLAYER_EXP_TABLE_MAX -> PLAYER_MAX_LEVEL_CONST
		return 1;
	}
...
// @correction230 END
...
#ifdef ENABLE_PC_OPENSHOP // @correction157
	int32_t pc_open_shop0(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	ALUA(pc_make_guild0)
	{
		...
		int32_t ret_type = MKGLD_GUILD_NOT_CREATED;
	...
	int32_t pc_open_acce(lua_State * L)
	{
	...
	int32_t pc_open_changelook(lua_State * L)
	{
		CQuestManager & qMgr = CQuestManager::instance();
		LPCHARACTER pkChar = qMgr.GetCurrentCharacterPtr();
		if (pkChar)
	...
	int32_t pc_all_skill_master(lua_State * L)
	{
		uint32_t dwlevel = (uint32_t)lua_tonumber(L, 1);
		CQuestManager & qMgr = CQuestManager::instance();
	...
	int32_t pc_give_and_equip_item(lua_State* L)
	{
...
#ifdef __12ZI__
	int32_t pc_change_bead(lua_State* L)
	{
		int32_t bead = (int32_t)lua_tonumber(L, -1);
	...
	int32_t pc_get_bead(lua_State* L)
	{
	...
	int pc_learn_sage_master_skill(lua_State* L)
	{
		...
		if (!lua_isnumber(L, 1))
			return 0;
		if (CQuestManager::instance().GetEventFlag("sage_skills"))
			return 0;
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "delf",				pc_del_another_quest_flag},
			{ "make_item0",			pc_make_item0		},
			{ "pointchange",		pc_pointchange		},
			{ "pullmob",			pc_pullmob			},
			{ "select_pid",			pc_select_pid		},
			{ "select_vid",			pc_select_vid		},
			{ "set_level0",			pc_set_level0		},
			...
			{ "get_exp_level0",		pc_get_exp_level0	},
			{ "set_max_health",		pc_set_max_health	},
			{ "get_ip0",			pc_get_ip0			},

			{ "get_client_version0",	pc_get_client_version0	},
			{ "dc_delayed0",		pc_dc_delayed0	},
			{ "dc_direct0",			pc_dc_direct0	},
			{ "is_trade0",			pc_is_trade0	},
			{ "is_busy0",			pc_is_busy0		},
			{ "is_busy",			pc_is_busy0		},
			{ "is_arena0",			pc_is_arena0		},
			{ "is_arena_observer0",	pc_is_arena_observer0		},
			{ "equip_slot0",		pc_equip_slot0		},
			{ "unequip_slot0",		pc_unequip_slot0	},
			{ "is_available0",		pc_is_available0	},
			{ "give_random_book0",	pc_give_random_book0}, // @correction124
#endif
#ifdef ENABLE_PC_OPENSHOP // @correction157
			{ "open_shop0",			pc_open_shop0		},
#endif
#ifdef ENABLE_NEWGUILDMAKE // @correction145
...
#endif
			{ "all_skill_master",		pc_all_skill_master },
