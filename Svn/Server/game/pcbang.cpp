...
bool CPCBangManager::Log(const char* c_szIP, uint32_t pid, time_t playTime)
{
...
void CPCBangManager::RequestUpdateIPList(PCBang_ID id)
{
#ifdef ENABLE_PCBANG_FEATURE // @correction005
	{
		if ( id == 0 )
		{
			DBManager::instance().ReturnQuery(QID_PCBANG_IP_LIST_CHECK, 0, nullptr, "show table status");
		}
		else
		{
			DBManager::instance().ReturnQuery(QID_PCBANG_IP_LIST_SELECT, 0, nullptr, "SELECT pcbang_id, ip FROM pcbang_ip WHERE pcbang_id=%u", id);
		}
	}
#endif
}
