...
int32_t main(int32_t argc, char **argv)
{
	...
	WriteVersion();
#ifdef __GUILD_DRAGONLAIR__
	CMgr	MeleyLairManager;
#endif
#ifdef __TEMPLE_OCHAO__
	TempleOchao::CMgr	TempleOchaoManager;
#endif
#ifdef __BOSS_TRACKING_SYSTEM__
	...
	ani_init();
#ifdef __12ZI__
	quest::CQuestManager::instance().RequestSetEventFlag("zodiac_portals_per_day", 0);
#endif
#ifdef __OFFLINE_PRIVATE_SHOP__
	if (CShopManager::instance().ReadPriceCheckTable("/usr/game/share/locale/turkey/price_check.txt"))
		sys_err("PriceCheck Okey");
	else
		sys_err("PriceCheck GG");
#endif
	PanamaLoad();
	...
	while (!exppet_table_open.eof())
	{
		...
		if (exppet_table_common[exppet_table_counter] < 4294967295)
		{
			sys_log(0, "Livelli Pet caricati da exppettable.txt: %d %s !", exppet_table_common[exppet_table_counter], temp_exp_line.c_str());
...
#ifdef __GUILD_DRAGONLAIR__
	if (!g_bAuthServer)
		MeleyLairManager.Initialize();
#endif
#ifdef __TEMPLE_OCHAO__
	if (!g_bAuthServer)
		TempleOchaoManager.Initialize();
#endif
	while (idle());
	...
	if (!g_bAuthServer)
	{
		sys_log(0, "<shutdown> Destroying MeleyLairManager.");
		MeleyLairManager.Destroy();
	}
#endif
#ifdef __TEMPLE_OCHAO__
	if (!g_bAuthServer)
	{
		sys_log(0, "<shutdown> Destroying TempleOchaoManager.");
		TempleOchaoManager.Destroy();
	}
#endif
#ifdef __MINI_GAME__
	sys_log(0, "<shutdown> Destroying MiniGameManager...");
...
int32_t start(int32_t argc, char **argv)
{
	...
	if ((tcp_socket = socket_tcp_bind(*g_szInternalIP ? g_szInternalIP : g_szPublicIP, mother_port)) == INVALID_SOCKET)
	{
	...
	if ((udp_socket = socket_udp_bind(*g_szInternalIP ? g_szInternalIP : g_szPublicIP, mother_port)) == INVALID_SOCKET)
	{
