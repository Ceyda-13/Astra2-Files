...
#include "questmanager.h" // @correction068
...
void exchange_packet(LPCHARACTER ch, uint8_t sub_header, bool is_me, uint64_t arg1, TItemPos arg2, uint64_t arg3, void * pvData = nullptr);

void exchange_packet(LPCHARACTER ch, uint8_t sub_header, bool is_me, uint64_t arg1, TItemPos arg2, uint64_t arg3, void * pvData)
{
	if (!ch->GetDesc())
		return;
	...
	if (sub_header == EXCHANGE_SUBHEADER_GC_ITEM_ADD && pvData)
	{
		...
		pack_exchg.changelook = ((LPITEM) pvData)->GetChangeLook();
#endif
	}
	else
	{
...
bool CExchange::CheckSpace()
{
	...
	LPITEM item = nullptr;
	uint16_t pkInvenGrid[INVENTORY_MAX_NUM];
	victim->CopyItemGrid(pkInvenGrid);

	static std::vector <uint16_t> s_vDSGrid(DRAGON_SOUL_INVENTORY_MAX_NUM);
	bool bDSInitialized = false;

	for (int32_t x = 0; x < EXCHANGE_ITEM_MAX_NUM; ++x)
	{
		if (!(item = m_apItems[x]))
			continue;
		...
		if (item->IsDragonSoul())
		{
			...
			for (int32_t i = 0; i < DRAGON_SOUL_BOX_SIZE; i++)
			{
				...
				if (0 == s_vDSGrid[wPos]) // @correction071 (wBasePos to wPos)
				{
					bool bEmpty = true;
		...
		else
		{
			...
			for (int32_t i = INVENTORY_ONE_MAX_NUM * bType; i < victim->GetInventoryMax(bType); ++i)
			{
				if (pkInvenGrid[i] == 0)
				{
					bool bEmpty = true;
					uint8_t bPage = i / INVENTORY_PAGE_SIZE;
					for (int32_t j = 1; j < item->GetSize(); j++)
					{
						int32_t p = i + (INVENTORY_PAGE_COLUMN * j);
...
bool CExchange::Done()
{
	...
	if (m_lGold)
	{
		GetOwner()->ChangeGold(false, m_lGold, true);
		victim->ChangeGold(true, m_lGold, true);
	...
	// GetOwner()->Save();
	// CHARACTER_MANAGER::instance().FlushDelayedSave(GetOwner());
	// victim->Save();
	// CHARACTER_MANAGER::instance().FlushDelayedSave(victim);
