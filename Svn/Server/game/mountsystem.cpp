...
uint32_t CMountActor::Mount()
{
	if (!m_pkOwner)
		return 0;

	int32_t x = m_pkOwner->GetX(), y = m_pkOwner->GetY();
...
void CMountActor::Summon(const char* mName, uint32_t mobVnum)
{
	if (!m_pkOwner)
		return;
...
bool CMountSystem::IsMounting(uint32_t mobVnum)
{
...
CMountActor* CMountSystem::Summon(uint32_t mobVnum, const char* mName)
{
	CMountActor* MountActor = this->GetByVnum(mobVnum);
	if (0 == MountActor)
	{
...
CMountActor* CMountSystem::Unmount(uint32_t mobVnum)
{
	CMountActor* MountActor = this->GetByVnum(mobVnum);
	if (MountActor)
		MountActor->Unmount();
