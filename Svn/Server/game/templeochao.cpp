...
namespace TempleOchao
{
	...
	{
		SRooms(int32_t i2X = 0, int32_t i2Y = 0, int32_t i2Z = 0) : iX(i2X), iY(i2Y), iZ(i2Z) {}
		int32_t iX, iY, iZ;
	} TRooms;

	static std::map<uint8_t, TRooms> m_rooms;
	...
	void CMgr::Prepare()
	{
		...
		for (uint8_t i = 0; i < TEMPLE_OCHAO_ROOMS; i++)
		{
			int32_t iX = pos[i][0], iY = pos[i][1];
			m_rooms.insert(std::make_pair(i + 1, TRooms(iX, iY)));
	...
	void CMgr::Spawn()
	{
		int32_t iRoom = 0, iX = 0, iY = 0, iZ = 0;
	...
	void CMgr::RandomRoom(int32_t &iRoom, int32_t &iX, int32_t &iY, int32_t &iZ)
	{
		int32_t iGenerated;
	...
	void CMgr::OnGuardianKilled(int32_t iX, int32_t iY, int32_t iZ)
	{
