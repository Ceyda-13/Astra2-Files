...
void P2P_MANAGER::SendEmpires(const void * c_pvData, int32_t iSize)
{
	std::unordered_set<LPDESC>::iterator it = m_set_pkPeers.begin();
	...
	while (it != m_set_pkPeers.end())
	{
		...
		if ((pkDesc->GetP2PPort() >= 9021 && pkDesc->GetP2PPort() <= 9032)
		|| (pkDesc->GetP2PPort() >= 9077 && pkDesc->GetP2PPort() <= 9088)
		|| (pkDesc->GetP2PPort() >= 9133 && pkDesc->GetP2PPort() <= 9144)
		|| (pkDesc->GetP2PPort() >= 9189 && pkDesc->GetP2PPort() <= 9200)
		|| (pkDesc->GetP2PPort() >= 9245 && pkDesc->GetP2PPort() <= 9256)
		|| (pkDesc->GetP2PPort() >= 9301 && pkDesc->GetP2PPort() <= 9312))
			pkDesc->Packet(c_pvData, iSize);
	}
}
