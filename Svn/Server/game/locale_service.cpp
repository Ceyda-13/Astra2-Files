...
int32_t is_twobyte_gb2312(const char * str)
{
	if (!str || !*str)
		return 0;
...
int32_t is_twobyte_big5(const char * str)
{
	if (!str || !*str)
		return 0;
...
int32_t check_name_gb2312(const char * str)
{
	static const uint8_t exceptions[5][2] =
	{
		{ 0xd7, 0xfa },
		{ 0xd7, 0xfb },
		{ 0xd7, 0xfc },
...
int32_t check_name_big5(const char * str )
{
	int32_t i;
	uint8_t b1, b2;
...
int32_t check_name_euckr(const char * str)
{
	int32_t		code;
	const char*	tmp;
...
static int32_t is_char_sjis(const char *p, const char *e)
{
	return (issjishead((uint8_t) *p) && (e-p)>1 && issjistail((uint8_t)p[1]) ? true : false);
}

int32_t is_twobyte_sjis(const char *str)
{
	if (str && str[0] && str[1])
		return issjishead((uint8_t)str[0]) && issjistail((uint8_t)str[1]);
	else
		return 0;
}

int32_t check_name_sjis(const char *str)
{
	const char	*p = str;
	...
	while (*p)
	{
		if (is_char_sjis(p, e))
		{
			...
			if ((uint8_t)p[0]==0x81 && (uint8_t)p[1]==0x40) return false;
...
void LocaleService_LoadEmpireTextConvertTables()
{
	...
	for (int32_t iEmpire = 1; iEmpire <= 3; ++iEmpire)
	{
...
{
#ifndef ENABLE_PLAYER_PER_ACCOUNT5 // @correction112
...
bool LocaleService_Init()
{
	...
	if (false == LC_InitLocalization(g_stServiceName))
		return false;
...
bool LC_InitLocalization( const std::string& szLocal )
{
	...
	if ( !g_stLocal.compare("ymir") )
		...
		g_eLocalType = LC_USA;
	else if ( !g_stLocal.compare("we_korea") )
