...
namespace quest
{
...
#endif
#ifdef __12ZI__
			if (ch && ch->IsRaceFlag(RACE_FLAG_ATT_CZ) && !ch->IsStone())
			{
				bool boss = (ch->GetRaceNum() >= 2752 && ch->GetRaceNum() <= 2862) ? 1 : 0;
				ch->SetLevel(GetRandomLevel(pDungeon->GetFlag("zodiac_floor"), boss));
				int32_t newHP = ch->GetMaxHP() + (ch->GetLevel() * 5000);
				ch->SetMaxHP(newHP);
				ch->SetHP(newHP);
				ch->UpdatePacket();
			}
...
#ifdef __DEFENSE_WAVE__
	int32_t dungeon_purge_vid(lua_State* L)
	{
		uint32_t dwVID = (uint32_t)lua_tonumber(L, 1);
	...
	int32_t dungeon_set_victim_mast(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
	...
	int32_t dungeon_get_mast(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		...
		if (pDungeon)
		{
			if (!pDungeon->GetMast() || pDungeon->GetMast()->GetHP() <= 0 || pDungeon->GetMast()->IsStun() || pDungeon->GetMast()->IsDead())
				lua_pushnumber(L, 0);
			else
				lua_pushnumber(L, 1);
		}
...
#ifdef __12ZI__
	int32_t dungeon_notice_zodiac(lua_State* L)
	{
		if (!lua_isstring(L, 1))
			return 0;
	...
	int32_t dungeon_start_cannon_attack(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
	...
	void RegisterDungeonFunctionTable()
	{
		luaL_reg dungeon_functions[] =
		{
...
#ifdef NEW_ICEDAMAGE_SYSTEM // @correction118
			{ "get_damage_from_race",			dungeon_get_damage_from_race	},
			{ "get_damage_from_affect",			dungeon_get_damage_from_affect	},
			{ "set_damage_from_race",			dungeon_set_damage_from_race	},
			{ "set_damage_from_affect",			dungeon_set_damage_from_affect	},
			{ "unset_damage_from_race",			dungeon_unset_damage_from_race	},
			{ "unset_damage_from_affect",		dungeon_unset_damage_from_affect},
			{ "reset_damage_from_race",			dungeon_reset_damage_from_race	},
			{ "reset_damage_from_affect",		dungeon_reset_damage_from_affect},
#endif
#ifdef ENABLE_NEWSTUFF //@correction232
			{ "is_available0",					dungeon_is_available0			},
#endif
			...
			{ "start_cannon_attack",	dungeon_start_cannon_attack	},
#endif
			{ nullptr,				nullptr			}
		};
