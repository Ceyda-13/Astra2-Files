...
const int32_t STONE_INFO_MAX_NUM = 24+4;
...
extern const uint32_t		party_exp_distribute_table[PLAYER_EXP_TABLE_MAX + 1]; // @correction001 PLAYER_MAX_LEVEL_CONST -> PLAYER_EXP_TABLE_MAX

extern const uint32_t		exp_table_common[PLAYER_MAX_LEVEL_CONST + 1]; // @correction001 PLAYER_EXP_TABLE_MAX -> PLAYER_MAX_LEVEL_CONST
...
extern const int32_t aiItemMagicAttributePercentHigh[ITEM_ATTRIBUTE_MAX_LEVEL];
...
enum
{
	ITEM_ACCESSORY_SOCKET_MAX_NUM = 4 // @correction140
};
...
#ifdef __SUPPORT__
extern const uint32_t		exp_support_table[121];
#endif

#ifdef __GROWTH_PET__
extern uint32_t		exppet_table_common[121];
extern uint32_t* exppet_table;
extern const uint32_t		Pet_SKill_TablePerc[3][20];
extern const uint32_t		Pet_Skill_Table[18][23];
extern const uint32_t		Pet_Types[9][3];

#endif

#ifdef __MINI_GAME__
extern const uint32_t dwMiniGameNpcVnums[MINIGAME_TYPE_MAX];
extern const uint32_t dwMiniGameNpcPos[MINIGAME_TYPE_MAX][6];
#endif

#ifdef __12ZI__
extern const int32_t iCzTable[2][30][2];
int32_t GetRandomLevel(uint8_t bFloor, bool bIsBoss = false);
#endif

#ifdef __AFAR_BIOLOG__
extern const uint32_t		BiyologSistemi[11][14];
#endif