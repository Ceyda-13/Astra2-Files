...
#define LZO_CHECK_FPOS_DET(m_pos,m_off,in,ip,max_offset) \
	(m_pos == NULL || (m_off = (lzo_moff_t) (ip - m_pos)) > max_offset)

#define LZO_CHECK_FPOS_NON_DET(m_pos,m_off,in,ip,max_offset) \
...
#  define LZO_CHECK_FPOS    LZO_CHECK_MPOS_DET
#else
#  define LZO_CHECK_FPOS    LZO_CHECK_MPOS_NON_DET
#endif
						  ...
						  lzo_voidp wrkmem )
{
	...
	for (;;)
	{
		...
		if (LZO_CHECK_FPOS_NON_DET(m_pos,m_off,in,ip,M4_MAX_OFFSET))
			goto literal;
#if 1
