...
void AuctionBoard::Sort(TItemInfoVec& vec, uint8_t order)
{
	switch (order)
	{
...
bool AuctionBoard::InsertItemInfo (TAuctionItemInfo* item_info)
{
	...
	uint32_t offer_id = item_info->get_offer_id();
	...
	uint32_t item_id = item_info->item_id;
...
bool AuctionBoard::DeleteItemInfo (uint32_t key)
{
...
TAuctionItemInfo* AuctionBoard::GetItemInfo (uint32_t key)
{
	TItemInfoMap::iterator it = item_map.find (key);
	if (it == item_map.end())
		return nullptr;
...
void AuctionBoard::YourItemInfoList (TItemInfoVec& vec, uint32_t player_id, int32_t start_idx, uint8_t size)
{
	int32_t pass = 0;
	vec.clear();
...
struct FCheckGradeSatisfied
{
	...
	bool operator() (int32_t item_level)
	{
		switch (grade)
		{
		case 0:
...
void AuctionBoard::SortedItemInfos (TItemInfoVec& vec, uint8_t grade, uint8_t category, int32_t start_idx, uint8_t size, uint8_t order[5])
{
	...
	case AUCTION_ITEM_NAME_AC:
		for (SortByItemName::iterator it = item_name_map.begin(); it != item_name_map.end() && vec.size() < size; it++)
		{
			...
			for (uint32_t i = 0; i < temp.size(); i++)
			{
				...
				for (int32_t j = 0; j < ITEM_LIMIT_MAX_NUM; j++)
				{
...
TSaleItemInfo* SaleBoard::GetItemInfo (uint32_t item_id)
{
...
bool SaleBoard::DeleteItemInfo (uint32_t item_id)
{
	TItemInfoMap::iterator it = item_map.find (item_id);
	if (it == item_map.end())
		return false;
...
bool SaleBoard::DeleteFromPCMap (TPCMap& pc_map, uint32_t player_id, uint32_t item_id)
{
...
{
	TSaleItemInfo* c = GetItemInfo (uint32_t (item_info->item_id));
	if (c != nullptr)
	{
		return false;
	}
...
bool SaleBoard::InsertInPCMap (TPCMap& pc_map, uint32_t player_id, TSaleItemInfo* item_info)
{
	TPCMap::iterator pc_it = pc_map.find (player_id);
	TItemMap* item_map;
	if (pc_it == pc_map.end())
...
void SaleBoard::WisherItemInfoList (TItemInfoVec& vec, uint32_t wisher_id, int32_t start_idx, uint8_t size)
{
	int32_t pass = 0;
	vec.clear();
...
TWishItemInfo* WishBoard::GetItemInfo (uint32_t wisher_id, uint32_t item_num)
{
	TPCMap::iterator wish_it = wisher_map.find (wisher_id);
	if (wish_it == wisher_map.end())
		return nullptr;
...
bool WishBoard::DeleteItemInfo (uint32_t wisher_id, uint32_t item_num)
{
	TPCMap::iterator wish_it = wisher_map.find (wisher_id);
	if (wish_it == wisher_map.end())
		return false;
...
{
	uint32_t wisher_id = item_info->offer_id;
	uint32_t item_num = item_info->item_num;
...
std::pair <int32_t, bool> MyBidBoard::GetMoney (uint32_t player_id, uint32_t item_id)
{
...
void MyBidBoard::Insert (uint32_t player_id, uint32_t item_id, int32_t money)
{
	...
	if (it == item_map->end())
	{
		item_map->insert (TItemMap::value_type (item_id, std::pair <int32_t, bool> (money, false)));
	}
	else
	{
...
void MyBidBoard::Lock (uint32_t player_id, uint32_t item_id)
{
	TMyBidBoard::iterator pc_it = pc_map.find (player_id);
	if (pc_it == pc_map.end())
	{
...
void MyBidBoard::UnLock (uint32_t player_id, uint32_t item_id)
{
	TMyBidBoard::iterator pc_it = pc_map.find (player_id);
	if (pc_it == pc_map.end())
	{
...
void MyBidBoard::YourBidInfo (TItemVec& vec, uint32_t bidder_id, int32_t start_idx, int32_t size)
{
	vec.clear();
...
void AuctionManager::Boot (const char* &data, uint16_t size)
{
	...
	if (auction_server)
	{
		for (uint16_t i = 0; i < size; ++i, ++item)
		{
			InsertItem (item);
		}
	...
	if (auction_server)
	{
		for (uint16_t i = 0; i < size; ++i, ++auction_item_info)
		{
	...
	if (auction_server)
	{
		for (uint16_t i = 0; i < size; ++i, ++sale_item_info)
		{
	...
	if (auction_server)
	{
		for (uint16_t i = 0; i < size; ++i, ++wish_item_info)
		{
			Wish.InsertItemInfo (wish_item_info);
		}
	}

	if (decode_2bytes(data) != (sizeof(uint32_t) + sizeof(uint32_t) + sizeof(int32_t)))
	{
	...
	if (auction_server)
	{
		...
		{
			uint32_t player_id = *((uint32_t*) data);
		...
		data += size * (sizeof(uint32_t) + sizeof(uint32_t) + sizeof(int32_t));
}
...
LPITEM AuctionManager::GetInventoryItem (uint32_t item_id)
{
...
bool AuctionManager::DeleteItem (uint32_t item_id)
{
	TItemMap::iterator it = auction_item_map.find (item_id);
	if (it == auction_item_map.end())
		return false;
...
void AuctionManager::YourBidItemInfoList (AuctionBoard::TItemInfoVec& vec, uint32_t bidder_id, int32_t start_idx, int32_t size)
{
	vec.clear();
	MyBidBoard::TItemVec item_id_vec;
	MyBid.YourBidInfo (item_id_vec, bidder_id, 0, 6);
	for (int32_t i = 0; i < item_id_vec.size(); i++)
	{
...
void AuctionManager::get_auction_list (LPCHARACTER ch, int32_t start_idx, int32_t size, int32_t cond)
{
	uint8_t order[5] = {0,};
...
void AuctionManager::get_my_auction_list (LPCHARACTER ch, int32_t start_idx, int32_t size)
{
	AuctionBoard::TItemInfoVec auction_vec;
	Auction.YourItemInfoList (auction_vec, ch->GetPlayerID(), 0, 6);

	for (uint32_t i = 0; i < auction_vec.size(); i++)
	{
...
void AuctionManager::get_my_purchase_list (LPCHARACTER ch, int32_t start_idx, int32_t size)
{
	AuctionBoard::TItemInfoVec auction_vec;
...
void AuctionManager::enroll_auction (LPCHARACTER ch, LPITEM item, uint8_t empire, int32_t bidPrice, int32_t immidiatePurchasePrice)
{
	if (ch != item->GetOwner())
	{
...
void AuctionManager::enroll_sale (LPCHARACTER ch, LPITEM item, uint32_t wisher_id, int32_t salePrice)
{
	if (ch != item->GetOwner())
	{
		sys_err ("Item %d's owner is %s, not %s",ch->GetName(), item->GetOwner()->GetName());
...
void AuctionManager::enroll_wish (LPCHARACTER ch, uint32_t item_num, uint8_t empire, int32_t wishPrice)
{
...
void AuctionManager::bid (LPCHARACTER ch, uint32_t item_id, int32_t bid_price)
{
	std::pair <int32_t, bool> mb = MyBid.GetMoney(ch->GetPlayerID(), item_id);
	if (mb.first != -1)
	{
	...
	ch->ChangeGold(false, bid_price, false);
...
void AuctionManager::immediate_purchase (LPCHARACTER ch, uint32_t item_id)
{
	...
	ch->ChangeGold(false, item_info->get_impur_price(), false);
...
void AuctionManager::get_auctioned_item (LPCHARACTER ch, uint32_t item_id, uint32_t item_num)
{
	TItemTable* proto = ITEM_MANAGER::instance().GetTable(item_num);
	int32_t pos = ch->GetEmptyInventory(proto->bSize);
...
void AuctionManager::buy_sold_item (LPCHARACTER ch, uint32_t item_id)
{
...
void AuctionManager::cancel_auction (LPCHARACTER ch, uint32_t item_id)
{
...
void AuctionManager::cancel_wish (LPCHARACTER ch, uint32_t item_num)
{
...
void AuctionManager::cancel_sale (LPCHARACTER ch, uint32_t item_id)
{
...
void AuctionManager::rebid (LPCHARACTER ch, uint32_t item_id, int32_t bid_price)
{
	std::pair <int32_t, bool> mb = MyBid.GetMoney (ch->GetPlayerID(), item_id);
	int32_t money = mb.first;
	bool lock = mb.second;
	if (money == -1)
	{
	...
	ch->ChangeGold(false, (bid_price - money), false);
...
void AuctionManager::bid_cancel (LPCHARACTER ch, uint32_t item_id)
{
	std::pair <int32_t, bool> mb = MyBid.GetMoney (ch->GetPlayerID(), item_id);
	int32_t money = mb.first;
	bool lock = mb.second;
...
void AuctionManager::recv_result_auction (uint32_t commander_id, TPacketDGResultAuction* cmd_result)
{
	...
	case AUCTION_CANCEL_SALE:
		...
		{
			uint8_t cmd = cmd_result->cmd;
			cmd_result++;
			TPlayerItem* player_item = (TPlayerItem*)cmd_result;

			uint32_t item_id = player_item->id;
	...
	case AUCTION_REBID:
		if (cmd_result->result <= AUCTION_FAIL)
		{
			uint8_t result = cmd_result->result;
			uint32_t item_id = cmd_result->target;
			cmd_result++;
			int32_t restore_money = *((int32_t*)cmd_result);
			ch->ChangeGold(true, restore_money, true);
