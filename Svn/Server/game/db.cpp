...
bool CheckPasspod(const char * account)
{
	...
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery)); // @correction023
...
void DBManager::Query(const char * c_pszFormat, ...)
{
	...
	m_sql.AsyncQuery(sQuery.substr(0, sQuery.find_first_of(";") == sQuery.npos ? sQuery.length(): sQuery.find_first_of(";")).c_str()); // @correction062
}
...
SQLMsg * DBManager::DirectQuery(const char * c_pszFormat, ...)
{
	...
	std::string sQuery(szQuery);
	return m_sql_direct.DirectQuery(sQuery.substr(0, sQuery.find_first_of(";") == sQuery.npos ? sQuery.length(): sQuery.find_first_of(";")).c_str()); // @correction062
}
...
void DBManager::FlushBilling(bool bForce)
{
	...
	if (!m_vec_kUseTime.empty())
	{
		...
		{
			int32_t nNewSize = m_vec_kUseTime.size() - dwCount;
		...
		while (it != m_map_pkLoginData.end())
		{
			...
			switch (pkLD->GetBillType())
			{
				...
				case BILLING_TIME:
					...
					else if (pkLD->GetRemainSecs() <= 600)
					{
						uint32_t dwSecsConnected = (dwCurTime - pkLD->GetLogonTime()) / 1000;

						if (dwSecsConnected >= 60)
						{
...
void DBManager::LoginPrepare(uint8_t bBillType, uint32_t dwBillID, int32_t lRemainSecs, LPDESC d, uint32_t * pdwClientKey, int32_t * paiPremiumTimes)
{
	const TAccountTable & r = d->GetAccountTable();
	...
	{
#ifdef ENABLE_PASSPOD_FEATURE // @correction005
		{
			if (!g_bNoPasspod)
			{
				if (CheckPasspod(r.login))
				{
					uint8_t id = HEADER_GC_REQUEST_PASSPOD;
					d->Packet(&id, sizeof(uint8_t));
...
bool GetGameTimeIP(MYSQL_RES * pRes, uint8_t & bBillType, uint32_t & dwBillID, int32_t & seconds, const char * c_pszIP)
{
	if (!pRes)
		return true;
	...
	int32_t ip_postfix = 0;
	str_to_number(ip_postfix, p);
	int32_t valid_ip = false;
...
bool GetGameTime(MYSQL_RES * pRes, uint8_t & bBillType, int32_t & seconds)
{
	if (!pRes)
		return true;
...
void SendBillingExpire(const char * c_pszLogin, uint8_t bBillType, int32_t iSecs, CLoginData * pkLD)
{
		...
		case QID_AUTH_LOGIN:
			{
				...
				if (pMsg->Get()->uiNumRows == 0)
				{
					...
					{
						// @correction058 1. PASSWORD('%s') -> %s 2. pinfo->passwd wrapped inside mysql_hash_password(%s).c_str()
...
// @correction138 END
					...
					else
					{
						...
						std::unique_ptr<SQLMsg>(DBManager::instance().DirectQuery("UPDATE account SET cpu_id = '%s', hdd_model = '%s', machine_guid = '%s', mac_addr = '%s', hdd_serial = '%s' WHERE id = %u", szCpuID, szHDDModel, szMachineGUID, szHwid, szHDDSerial, dwID));
#endif
		...
		case QID_BRAZIL_CREATE_ID :
			{
				...
				else
				{
					...
					ReturnQuery(QID_AUTH_LOGIN, qi->dwIdent, pinfo,
							...
							" FROM account WHERE login='%s'", pinfo->login);
#else
					// @correction058 1. PASSWORD('%s') -> %s 2. pinfo->passwd wrapped inside mysql_hash_password(%s).c_str()
					ReturnQuery(QID_AUTH_LOGIN, qi->dwIdent, pinfo,
...
void DBManager::SendMoneyLog(uint8_t type, uint32_t vnum, uint64_t gold)
{
	if (!gold)
		return;
	TPacketMoneyLog p;
...
void DBManager::RequestBlockException(const char *login, int32_t cmd)
{
...
SQLMsg* AccountDB::DirectQuery(const char * szQuery)
{
	std::string sQuery(szQuery);
	return m_sql_direct.DirectQuery(sQuery.substr(0, sQuery.find_first_of(";") == sQuery.npos ? sQuery.length(): sQuery.find_first_of(";")).c_str()); // @correction062
}

void AccountDB::AsyncQuery(const char* szQuery)
{
	std::string sQuery(szQuery);
	m_sql.AsyncQuery(sQuery.substr(0, sQuery.find_first_of(";") == sQuery.npos ? sQuery.length(): sQuery.find_first_of(";")).c_str()); // @correction062
}
...
{
	AccountDB::instance().ReturnQuery(QID_SPAM_DB, 0, nullptr, "SELECT word, score FROM spam_db WHERE type='SPAM'");
#ifdef ENABLE_SPAMDB_REFRESH // @correction154
