...
class CWarMap
{
	...
	private:
		...
		typedef struct STeamData
		{
			...
			int32_t GetAccumulatedJoinerCount();
			int32_t GetCurJointerCount();
	...
	public:
		void	SendKillNotice(const char * c_szKillerName, const char * c_szVictimName, uint32_t dwKillerRace, uint32_t dwVictimRace, uint32_t dwWeapon);
#endif
};
