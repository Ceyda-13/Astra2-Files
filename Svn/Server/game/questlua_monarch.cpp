...
namespace quest
{
	...
	int32_t monarch_powerup(lua_State * L)
	{
		...
		{
			int32_t	next_sec = CMonarch::instance().GetPowerUpCT(ch->GetEmpire()) / passes_per_sec;
	...
	int32_t monarch_defenseup(lua_State * L)
	{
		...
		{
			int32_t	next_sec = CMonarch::instance().GetDefenseUpCT(ch->GetEmpire()) / passes_per_sec;
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("쿨타임 적용중  %d 후 사용가능"), next_sec);
			return 0;
		}
	...
	int32_t frog_to_empire_money(lua_State * L)
	{
		...
		{
			int32_t empire_money = CMonarch::instance().GetMoney(ch->GetEmpire());
