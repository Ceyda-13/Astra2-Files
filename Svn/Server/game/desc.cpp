...
void DESC::Destroy()
{
	...
	if (!g_bAuthServer)
	{
		...
		CShutdownManager::instance().RemoveDesc((LPDESC)this); // @correction138
	}
...
bool DESC::Setup(LPFDWATCH _fdw, int32_t _fd, const struct sockaddr_in & c_rSockAddr, uint32_t _handle, uint32_t _handshake)
{
	m_lpFdw		= _fdw;
	m_sock		= _fd;
...
void DESC::SetPhase(int32_t _phase, const char* func, int line)
{
	m_iPhase = _phase;
	...
	if (func)
		sys_log(0, "SetPhase(%d) %p, %s %d", _phase, this, func, line);
...
void DESC::SetMatrixCardRowsAndColumns(uint32_t rows, uint32_t cols)
{
	m_dwMatrixRows = rows;
	m_dwMatrixCols = cols;
}

uint32_t DESC::GetMatrixRows()
{
	return m_dwMatrixRows;
}

uint32_t DESC::GetMatrixCols()
{
