...
#include "party.h"
#ifdef ENABLE_D_NJGUILD // @correction187
#include "guild.h"
#endif
#include "affect.h"
#include "packet.h"
...
{
	pParty->SetDungeon(this); // @correction010 the begin of the nightmare
...
void CDungeon::QuitParty(LPPARTY pParty)
{
	...
	TPartyMap::iterator it = m_map_pkParty.find(pParty); // @correction010 boom! crash!
...
EVENTINFO(dungeon_cannon_info)
{
	LPCHARACTER boss, cannon;
	bool is_cannon_dead;
	LPDUNGEON pDungeon;

	dungeon_cannon_info() : boss(nullptr), cannon(nullptr), is_cannon_dead(false), pDungeon(nullptr)
	{

	}
};

EVENTFUNC(dungeon_cannon_event)
{
	dungeon_cannon_info* info = dynamic_cast<dungeon_cannon_info*>(event->info);

	if (info == nullptr)
	{
		sys_err("dungeon_cannon_event> <Factor> Null pointer");
		return 0;
	}
	...
	if (info->is_cannon_dead)
	{
		info->is_cannon_dead = false;
		M2_DESTROY_CHARACTER(info->cannon);
		info->cannon = nullptr;
		return PASSES_PER_SEC(50);
	}
	else if (info->boss->GetHP() < 2000)
		return 0;
	else
	{
		if (!info->cannon)
			info->cannon = info->pDungeon->SpawnMob(20464, 745, 278);
		else
		{
			info->cannon->SetTarget(nullptr);

			if (info->cannon->GetHP() < 60)
			{
				info->cannon->SendMovePacket(FUNC_ATTACK, 0, info->cannon->GetX(), info->cannon->GetY(), 0, get_dword_time());
				info->boss->EffectPacket(SE_EFFECT_SKILL);
				info->boss->SetHP(info->boss->GetHP() - info->boss->GetMaxHP() / 9);
				info->is_cannon_dead = true;
			}
		}

		return PASSES_PER_SEC(0.1);
	}
}

void CDungeon::StartCannonAttack(LPCHARACTER ch)
{
	dungeon_cannon_info* info = AllocEventInfo<dungeon_cannon_info>();
	info->boss = ch;
	info->pDungeon = this;

	event_cancel(&cannon_event);
	cannon_event = event_create(dungeon_cannon_event, info, PASSES_PER_SEC(1));
}
#endif