...
#if defined(ENABLE_WOLFMAN_CHARACTER) && defined(USE_ITEM_BLEEDING_AS_POISON) // @correction190
	iRate = MAX(0, iRate - ch->GetPoint(POINT_POISON_REDUCE));
#endif
	return iRate;
}
...
// #if defined(ENABLE_WOLFMAN_CHARACTER) && defined(USE_MOB_BLEEDING_AS_POISON) // @correction190
	// if (table->cEnchants[MOB_ENCHANT_POISON] != 0)
		// ApplyPoint(APPLY_BLEEDING_PCT, table->cEnchants[MOB_ENCHANT_POISON]/50); // @correction008
// #endif
...
bool CHARACTER::IsImmune(uint32_t dwImmuneFlag)
{
	...
	{
#ifdef ENABLE_IMMUNE_PERC // @correction152
