	...
	for (itertype(m_vec_shopTabs) it = m_vec_shopTabs.begin(); it != m_vec_shopTabs.end(); it++)
	{
		...
		for (uint8_t i = 0; i < SHOP_HOST_ITEM_MAX_NUM; i++)
		{
			...
			case SHOP_COIN_TYPE_GOLD:
				if (bOtherEmpire && !g_bEmpireShopPriceTripleDisable) // @correction206
					pack_tab.items[i].dwPrices = shop_tab.items[i].price * 3;
				else
					pack_tab.items[i].dwPrices = shop_tab.items[i].price;
				break;
...
int32_t CShopEx::Buy(LPCHARACTER ch, uint8_t pos)
{
	...
	case SHOP_COIN_TYPE_GOLD:
		...
		if ((uint32_t)ch->GetGold() < dwPrice)
		{
...
#ifdef ENABLE_FLUSH_CACHE_FEATURE // @correction005
	{
		ch->SaveReal();
