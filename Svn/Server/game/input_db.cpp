...
bool GetServerLocation(TAccountTable & rTab, uint8_t bEmpire)
{
	...
	for (int32_t i = 0; i < PLAYER_PER_ACCOUNT; ++i)
	{
					...
					rTab.players[i].wPort))
		{
			...
			{
				sys_err("cannot find server for mapindex %d %d x %d (name %s)", lIndex, rTab.players[i].x, rTab.players[i].y, rTab.players[i].szName);
// @correction123 BEGIN
...
// @correction139 BEGIN
	...
	if (pMsg->Get()->uiNumRows > 0)
	{
		...
		if (strcmp(row[0], "YES"))
		{
			std::unique_ptr<SQLMsg> pMsg2(DBManager::instance().DirectQuery("SELECT type FROM account.special_accs WHERE login = '%s'", pTab->login));
...
// @correction139 END
	...
	CShutdownManager::Instance().AddDesc(d); // @correction138
...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
...
#ifdef ENABLE_GOHOME_IF_MAP_NOT_EXIST // @correction176
		lMapIndex = EMPIRE_START_MAP(d->GetAccountTable().bEmpire);
		pos.x = EMPIRE_START_X(d->GetAccountTable().bEmpire);
		pos.y = EMPIRE_START_Y(d->GetAccountTable().bEmpire);
#else
...

	{
		...
		char buf[51];
		snprintf(buf, sizeof(buf), "%s %llu %d %d %d",
...
void CInputDB::Boot(const char* data)
{
	...
	sys_err("RANGE: %u %u %u", range->dwMin, range->dwMax, range->dwUsableItemIDMin);
	sys_err("RANGESPARE: %u %u %u", rangespare->dwMin, rangespare->dwMax, rangespare->dwUsableItemIDMin);
...
#if defined(__MINI_GAME__) && defined(__MINI_GAME_ATTENDANCE__) || defined(__MINI_GAME_MONSTERBACK__)
...
void CInputDB::MapLocations(const char * c_pData)
{
	...
	while (bCount--)
	{
		for (int32_t i = 0; i < MAP_ALLOW_LIMIT; ++i) // @correction100
		{
			if (0 == pLoc->alMaps[i])
				break;
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData)
{
	...
	for (uint32_t i = 0; i < dwCount; ++i, ++p)
	{
		...
		if (!item)
		{
			sys_err("cannot create item by vnum %u (%u %u) (name %s id %u)", p->vnum, p->window, p->pos, ch->GetName(), p->id);
			continue;
		}
...
bool CInputDB::Process(LPDESC d, const void * orig, int32_t bytes, int32_t & r_iBytesProceed)
{
	...
	for (m_iBufferLeft = bytes; m_iBufferLeft > 0;)
	{
		...
		bHeader		= *((uint8_t *) (c_pData));
		m_dwHandle	= *((uint32_t *) (c_pData + 1));
		iSize		= *((uint32_t *) (c_pData + 5));
