...
struct FWarpToDragronLairWithGuildMembers
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent && ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (pChar && pChar->IsPC() && pChar->GetGuild() && dwGuildID == pChar->GetGuild()->GetID())
...
CDragonLair::CDragonLair(uint32_t guildID, int32_t BaseMapID, int32_t PrivateMapID) : GuildID_ (guildID), BaseMapIndex_ (BaseMapID), PrivateMapIndex_ (PrivateMapID)
{
