...
bool CBattleArena::IsBattleArenaMap(int32_t nMapIndex)
{
...
struct FWarpToHome
{
	void operator() (LPENTITY ent)
	{
		if ( ent->IsType(ENTITY_CHARACTER) == true )
		{
			...
			if ( lpChar->IsPC() == true )
			{
				if (!test_server && lpChar->GetGMLevel() != GM_PLAYER)
					return;

				int32_t nEmpire, nMapIndex, x, y;
...
bool CBattleArena::Start(int32_t nEmpire)
{
...
{
	static const uint32_t vnum[7] = { 8012, 8013, 8014, 8024, 8025, 8026, 8027 };

	static const int32_t region_info[3][4] = {
	   	{688900, 247600, 692700, 250000},
	   	{740200, 251000, 744200, 247700},
	   	{791400, 250900, 795900, 250400} };

	int32_t idx = m_nMapIndex - 190;
...
{
	static const uint32_t vnum = 8616;

	static const int32_t position[3][2] = {
		{ 691000, 248900 },
		{ 742300, 249000 },
		{ 793600, 249300 } };
