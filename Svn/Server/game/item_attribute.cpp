...
void CItem::SetAttribute(int32_t i, uint8_t bType, int16_t sValue)
{
	...
	if (bType)
	{
		...
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(i, bType, sValue, GetID(), "SET_ATTR", "", pszIP ? pszIP : "", GetOriginalVnum())); // @correction222
	}
}
...
void CItem::SetForceAttribute(int32_t i, uint8_t bType, int16_t sValue)
{
	...
	if (bType)
	{
		...
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(i, bType, sValue, GetID(), "SET_FORCE_ATTR", "", pszIP ? pszIP : "", GetOriginalVnum())); // @correction222
	}
}
...
int32_t CItem::GetRareAttrCount()
{
	...
	for (uint32_t dwIdx = ITEM_ATTRIBUTE_RARE_START; dwIdx < ITEM_ATTRIBUTE_RARE_END; dwIdx++) // @correction105
	{
...
bool CItem::ChangeRareAttribute()
{
	...
	{
		m_aAttr[i + ITEM_ATTRIBUTE_RARE_START].bType = 0; // @correction105
		m_aAttr[i + ITEM_ATTRIBUTE_RARE_START].sValue = 0; // @correction105
	}

	if (GetOwner() && GetOwner()->GetDesc())
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(GetOwner(), this, "SET_RARE_CHANGE", "")) // @correction222
	else
		LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(0, 0, 0, GetID(), "SET_RARE_CHANGE", "", "", GetOriginalVnum())) // @correction222
...
bool CItem::AddRareAttribute()
{
	...
	if (count >= ITEM_ATTRIBUTE_RARE_NUM) // @correction105
		return false;

	int32_t pos = count + ITEM_ATTRIBUTE_RARE_START; // @correction105
	...
	LOG_LEVEL_CHECK(LOG_LEVEL_MAX, LogManager::instance().ItemLog(pos, attr.bType, attr.sValue, GetID(), "SET_RARE", "", pszIP ? pszIP : "", GetOriginalVnum())); // @correction222
	return true;
}
...
bool CItem::HasApply(uint8_t bApply)
{
	for (int32_t i = 0; i < ITEM_APPLY_MAX_NUM; ++i)
		if (m_pProto->aApplies[i].bType == bApply)
			return true;
...
bool CItem::HasOnlyAttr(uint8_t bApply)
{
	for (int32_t i = 0; i < MAX_NORM_ATTR_NUM; ++i)
		if (GetAttributeType(i) == bApply)
			return true;
...
void CItem::AddNewAttr(uint8_t index, uint8_t bApply, uint8_t bLevel)
{
	if (HasAttr(bApply))
		return;
