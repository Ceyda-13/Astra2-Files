...
{
	for (int32_t i = 1; i < EMPIRE_MAX_NUM; i++)
	{
...
bool CSpeedServerEmpireExp::Initialize (uint8_t e)
{
	empire = e;
...
bool CSpeedServerEmpireExp::LoadWdayExpTable(int32_t wday, char *str)
{
	...
	{
		t = strtok (p, delim);
		e = strtok (nullptr, delim);
		h = strtok (t, ":");
		m = strtok (nullptr, delim);
...
bool CSpeedServerManager::WriteExpTableOfEmpire(uint8_t empire)
{
	...
	while (fgets(one_line, 256, fp))
	{
		...
		else TOKEN("HOLIDAY")
		{
			...
			if (!str_to_number (year, strtok (v, "."))
					|| !str_to_number ( mon, strtok (nullptr, "."))
					|| !str_to_number ( day, strtok (nullptr, ".")))
			{
...
std::list <HME>& CSpeedServerManager::GetWdayExpTableOfEmpire(uint8_t empire, int32_t wday)
{
	return Empire[empire].GetWdayExpTable(wday);
}

std::list <HME>& CSpeedServerEmpireExp::GetWdayExpTable(int32_t wday)
{
	return wday_exp_table[wday];
}

void CSpeedServerManager::SetWdayExpTableOfEmpire (uint8_t empire, int32_t wday, HME&& hme)
{
	Empire[empire].SetWdayExpTable(wday, hme);
}

void CSpeedServerEmpireExp::SetWdayExpTable (int32_t wday, HME& hme)
{
	wday_exp_table[wday].push_back(hme);
	WriteExpTable();
}

void CSpeedServerManager::InitWdayExpTableOfEmpire (uint8_t empire, int32_t wday)
{
...
void CSpeedServerEmpireExp::InitWdayExpTable(int32_t wday)
{
	wday_exp_table[wday].clear();
}


std::list <HME>& CSpeedServerManager::GetHolidayExpTableOfEmpire(uint8_t empire, Date date, bool &is_exist)
{
...
void CSpeedServerManager::SetHolidayExpTableOfEmpire (uint8_t empire, Date date, HME&& hme)
{
...
void CSpeedServerManager::InitHolidayExpTableOfEmpire (uint8_t empire, Date date)
{
	if (empire > EMPIRE_MAX_NUM)
	{
		sys_err ("invalid empire");
...
HME* CSpeedServerManager::GetCurrentExpPrivOfEmpire (uint8_t empire, int32_t &duration, bool &is_change)
{
	return Empire[empire].GetCurrentExpPriv (duration, is_change);
}

HME* CSpeedServerEmpireExp::GetCurrentExpPriv(int32_t &duration, bool &is_change)
{
	struct tm* datetime;
	time_t t;
	t = time(nullptr);
	...
	int32_t total_sec = datetime->tm_hour * 3600 + datetime->tm_min * 60 + datetime->tm_sec;
