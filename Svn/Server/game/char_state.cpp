...
#ifdef __12ZI__
|| ((pkChr->GetRaceNum() >= 6520) && (pkChr->GetRaceNum() <= 6528))
#endif
)
					return;
...
void CHARACTER::__StateIdle_Stone()
{
	...
	int32_t iPercent = 0; // @correction056
	if (GetMaxHP() >= 0)
...
void CHARACTER::__StateIdle_Monster()
{
...
#ifdef __GUILD_DRAGONLAIR__
				if ((!victim) && (GetRaceNum() == (uint16_t)(BOSS_VNUM)))
	...
	if (!no_wander && !IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NOMOVE))
	{
		if (!number(0, 6))
		{
			...
			if (test_server) // @correction009 g_test_server -> test_server
			{
				if (number(0, 100) < 60)
					SetNowWalking(false);
				else
...
void CHARACTER::StateBattle()
{
...
#ifdef __GUILD_DRAGONLAIR__
			if ((!new_victim) && (GetRaceNum() == (uint16_t)(BOSS_VNUM)))
	...
	if (IsSummonMonster() && !IsDead() && !IsStun())
	{
		...
		int32_t count = 1;
		uint32_t dwVnum = GetSummonVnum();
#ifdef __TEMPLE_OCHAO__
		if (GetRaceNum() == TEMPLE_OCHAO_BOSS)
			count += 1;
#endif
		for (int32_t i = 0; i < count; ++i)
		{
#ifdef __TEMPLE_OCHAO__
		if (GetRaceNum() == TEMPLE_OCHAO_BOSS && i)
			dwVnum = TEMPLE_OCHAO_HEALLER;
#endif
			bool bPct = !number(0, 3);

			if (bPct && pParty && pParty->CountMemberByVnum(dwVnum) < SUMMON_MONSTER_COUNT)
			{
				...
				LPCHARACTER tch = CHARACTER_MANAGER::instance().SpawnMobRange(dwVnum, GetMapIndex(), sx, sy, ex, ey, true, true);
...
#ifdef __GUILD_DRAGONLAIR__
		bool bPass = true;
		if (GetRaceNum() == (uint16_t)(BOSS_VNUM) && (fDist < 16000.0f))
			bPass = false;
...
#ifdef __GUILD_DRAGONLAIR__
				if ((GetRaceNum() == (uint16_t)(BOSS_VNUM)) && (CMgr::instance().IsMeleyMap(victim->GetMapIndex())))
				{
