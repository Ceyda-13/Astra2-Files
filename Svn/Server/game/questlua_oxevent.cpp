...
void __SetOxQuizFileName(const char* szOxQuizFN=nullptr, bool bAddLocale=true);
std::string & __GetOxQuizFileName();

void __SetOxQuizFileName(const char* szOxQuizFN, bool bAddLocale)
{
	if (szOxQuizFN==nullptr)
...
#ifdef ENABLE_NEWSTUFF //@correction232
		int32_t result = lua_dofile(quest::CQuestManager::instance().GetLuaState(), __GetOxQuizFileName().c_str());
#else
		char script[256];
...
#ifdef ENABLE_NEWSTUFF //@correction232
			{	"get_oxquiz_fn",	oxevent_get_oxquiz_fn	},
			{	"set_oxquiz_fn",	oxevent_set_oxquiz_fn	},
			{	"set_oxquiz_fn0",	oxevent_set_oxquiz_fn0	},
#endif
			{ nullptr, nullptr}
		};
