...
LPCLIENT_DESC g_TeenDesc		= nullptr;
LPCLIENT_DESC g_PasspodDesc = nullptr;
		...
		case PHASE_DBCLIENT:
			{
				...
				if (!g_bAuthServer)
				{
					...
					if (!bSentBoot)
					{
						...
						p.dwItemIDRange[0] = 0;
						p.dwItemIDRange[1] = 0;
						memcpy(p.szIP, *g_szInternalIP ? g_szInternalIP : g_szPublicIP, 16);
				...
				if (!g_bAuthServer)
				{
					...
					p.wP2PPort	= p2p_port;
					p.bAuthServer = false;
					map_allow_copy(p.alMaps, MAP_ALLOW_LIMIT); // @correction100
