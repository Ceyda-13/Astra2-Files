...
struct FSendWarKillNotice
{
	char buf[512];

	FSendWarKillNotice(const char * c_szKillerName, const char * c_szVictimName, uint32_t dwKillerRace, uint32_t dwVictimRace, uint32_t dwWeapon) {
		snprintf(buf, sizeof(buf), "GW_Kill_Update %s %u %s %u %u", c_szKillerName, dwKillerRace, c_szVictimName, dwVictimRace, dwWeapon);
	}
...
void CWarMap::SendKillNotice(const char * c_szKillerName, const char * c_szVictimName, uint32_t dwKillerRace, uint32_t dwVictimRace, uint32_t dwWeapon)
{
	FSendWarKillNotice f(c_szKillerName, c_szVictimName, dwKillerRace, dwVictimRace, dwWeapon);
	std::for_each(m_set_pkChr.begin(), m_set_pkChr.end(), f);
}
#endif
