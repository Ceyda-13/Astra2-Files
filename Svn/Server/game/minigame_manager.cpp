...
void CMiniGameManager::SpawnNPC(uint32_t dwVnum, uint8_t bType)
{
	...
	if (CHARACTER_MANAGER::instance().GetCharactersByRaceNum(dwVnum, i))
	{
		...
		while (it != i.end())
		{
			...
			if (tch->GetMapIndex() == 1)
				isNpcA1 = true;
			else if (tch->GetMapIndex() == 3)
				isNpcB1 = true;
			else if (tch->GetMapIndex() == 4)
				isNpcC1 = true;
		}
	}
	...
	if (map_allow_find(3) && !isNpcB1)
		if (LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(3))
			CHARACTER_MANAGER::instance().SpawnMob(dwVnum, 3, pkMap->m_setting.iBaseX + dwMiniGameNpcPos[bType][2] * 100, pkMap->m_setting.iBaseY + dwMiniGameNpcPos[bType][3] * 100, 0, false, 110, true);

	if (map_allow_find(5) && !isNpcC1)
		if (LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(5))
			CHARACTER_MANAGER::instance().SpawnMob(dwVnum, 5, pkMap->m_setting.iBaseX + dwMiniGameNpcPos[bType][4] * 100, pkMap->m_setting.iBaseY + dwMiniGameNpcPos[bType][5] * 100, 0, false, 140, true);
}

void CMiniGameManager::SetEvent(bool bIsEnable, uint8_t bType)
{
...
void CMiniGameManager::SpawnRegen(bool bIsEnable, uint8_t bType)
{
	if (bIsEnable)
	{
		switch(bType)
		{
			...
			case MINIGAME_ATTENDANCE:
			{
				const char * c_apszRegenNames[4] = {
					"/usr/game/share/data/event/attendance/nemere_regen.txt",
					"/usr/game/share/data/event/attendance/razador_regen.txt",
					"/usr/game/share/data/event/attendance/red_dragon_regen.txt",
					"/usr/game/share/data/event/attendance/beran_setaou_regen.txt"
				};
			...
			{
				uint32_t stoneMaps[19] = {
					1, 2, 3, 4, 5, 6, 61, 62, 63, 64, 65, 67, 68, 69, 70, 
				...
				snprintf(szFileName10, sizeof(szFileName10), "/usr/game/share/data/event/attendance/easter/stone_level_10.txt");
				snprintf(szFileName20, sizeof(szFileName20), "/usr/game/share/data/event/attendance/easter/stone_level_20.txt");
				snprintf(szFileName30, sizeof(szFileName30), "/usr/game/share/data/event/attendance/easter/stone_level_30.txt");
				snprintf(szFileName40, sizeof(szFileName40), "/usr/game/share/data/event/attendance/easter/stone_level_40.txt");
				snprintf(szFileName50, sizeof(szFileName50), "/usr/game/share/data/event/attendance/easter/stone_level_50.txt");
				snprintf(szFileName60, sizeof(szFileName60), "/usr/game/share/data/event/attendance/easter/stone_level_60.txt");
				snprintf(szFileName70, sizeof(szFileName70), "/usr/game/share/data/event/attendance/easter/stone_level_70.txt");
				snprintf(szFileName80, sizeof(szFileName80), "/usr/game/share/data/event/attendance/easter/stone_level_80.txt");
				snprintf(szFileName90, sizeof(szFileName90), "/usr/game/share/data/event/attendance/easter/stone_level_90.txt");
				snprintf(szFileName95, sizeof(szFileName95), "/usr/game/share/data/event/attendance/easter/stone_level_95.txt");
				...
				for (size_t i = 0; i < sizeof(stoneMaps) / sizeof(stoneMaps[0]); i++)
				{
					if (map_allow_find(stoneMaps[i]))
					{
						...
						{
							if(stoneMaps[i] == 1 || stoneMaps[i] == 3 || stoneMaps[i] == 5)
							{
							...
							else if(stoneMaps[i] == 2 || stoneMaps[i] == 4 || stoneMaps[i] == 6)
							{
								regen_load(szFileName20, stoneMaps[i], pkMap->m_setting.iBaseX, pkMap->m_setting.iBaseY);
...
{
	for (int32_t i = 0; i < MINIGAME_TYPE_MAX; ++i)
		if (bEventState[i])
...
void CMiniGameManager::AttendanceButtonClick(LPCHARACTER pChar, const char * data)
{
	...
	uint32_t rewardVnum = 0, rewardCount = 0;
	if (!attendanceRewardVec.empty())
	{
		for (uint32_t i = 0; i < attendanceRewardVec.size(); i++)
		{
