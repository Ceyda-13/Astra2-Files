...
#if defined(ENABLE_WOLFMAN_CHARACTER) || defined(ENABLE_PARTY_FLAG) // @correction190 | @correction116
...
bool CGuild::ChargeSP(LPCHARACTER ch, int32_t iSP)
{
	...
	if (gold < iSP || (int32_t)ch->GetGold() < gold)
		return false;
...
int32_t CGuild::GetMaxMemberCount()
{
	...
	// @correction005
	if (g_bGuildInfiniteMembers) // @correction214
		return INT_MAX;
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int32_t iGold)
{
	...
	if ((int32_t)ch->GetGold() < iGold)
		return;


	ch->ChangeGold(false, iGold);
...
void CGuild::RecvWithdrawMoneyGive(int32_t iChangeGold)
{
	...
	if (ch)
	{
		ch->ChangeGold(true, iChangeGold);
...
CGuild::GuildJoinErrCode CGuild::VerifyGuildJoinableCondition( const LPCHARACTER pchInvitee )
{
	...
	// @correction005
	else if (g_bGuildInviteLimit) // @correction213
	{
