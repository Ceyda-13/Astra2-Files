...
bool DSManager::ExtractDragonHeart(LPCHARACTER ch, LPITEM pItem, LPITEM pExtractor)
{
	...
	fCharge = std::clamp<float>(0.f, fCharge, 100.f); // @correction104
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos* aItemPoses)
{
	if (!ch)
		return false;
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos* aItemPoses)
{
	if (!ch)
		return false;
...
|| pItem->GetSubType() == MATERIAL_DS_REFINE_MASTER
#endif
);
}

bool DSManager::DoRefineStrength(LPCHARACTER ch, TItemPos* aItemPoses)
{
	if (!ch)
		return false;
...
bool DSManager::ActivateDragonSoul(LPITEM pItem)
{
	...
	if (INVENTORY_MAX_NUM + WEAR_MAX_NUM + DS_SLOT_MAX * deck_idx <= pItem->GetCell() && pItem->GetCell() < INVENTORY_MAX_NUM + WEAR_MAX_NUM + DS_SLOT_MAX * (deck_idx + 1))
	{
