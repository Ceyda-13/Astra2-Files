...
class DESC
{
	...
	public:
		...
		virtual void		SetPhase(int32_t _phase, const char* func = nullptr, int line = 0);

		void			FlushOutput();

		bool			Setup(LPFDWATCH _fdw, int32_t _fd, const struct sockaddr_in & c_rSockAddr, uint32_t _handle, uint32_t _handshake);

		int32_t		GetSocket() const	{ return m_sock; }
