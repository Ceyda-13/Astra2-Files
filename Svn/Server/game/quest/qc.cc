...
{
	int32_t ret = luaL_loadbuffer(L, str.c_str(), str.size(), module.c_str());
...
void CheckUsedFunction()
{
	...
	for (decltype(function_calls.begin()) it = function_calls.begin(); it != function_calls.end(); ++it)
	{
	...
	if (hasError)
	{
		...
		for (decltype(error_func.begin()) it = error_func.begin(); it != error_func.end(); ++it)
		{
			cout << *it << endl;
		}
	}
...
void parse(char * filename)
{
			...
			case ST_WHEN_BODY:
				{
					...
					decltype(lexstate) prev = lexstate;
					string callname;
	...
	assert(nested==0);
	for (map<int32_t,string>::iterator it = used_state_name_map.begin();it!=used_state_name_map.end();++it)
	{
	...
	if (!define_state_name_set.empty())
	{
		...
		{
			int32_t crc = (int32_t) CRC32((*it).c_str());
...

	{
		...
		for (it = state_arg_script_map.begin(); it != state_arg_script_map.end(); ++it)
		{
			...
			{
				for (int32_t i = 0; i < it2->second.size(); ++i)
				{
					ostringstream os;
					os << i;
					{
