...
EVENTFUNC(r_meleystatues_event)
{
	...
	if (pEventInfo)
	{
		...
		else
		{
			...
			return PASSES_PER_SEC((uint32_t)(TIME_LIMIT_TO_KILL_STATUE));
		}
	}
...
EVENTFUNC(r_meleyspawn_event)
{
	...
	{
		size_t stones_count = SECTREE_MANAGER::instance().GetMonsterCountInMap(pMap->GetMapIndex(), (uint32_t)(MOBVNUM_RESPAWN_STONE_STEP2), true);
		if (stones_count == 0)
		{
			...
			if (dwTime >= pMap->GetLastStoneKilledTime())
			{
				...
				{
					uint32_t dwRandom = number(3, 10);
					pMap->Spawn((uint32_t)(MOBVNUM_RESPAWN_STONE_STEP2), stoneSpawnPos[i][0], stoneSpawnPos[i][1] + dwRandom, 0);
				}
		...
		if ((pEventInfo->bStep == 3) && (pEventInfo->bFirst))
		{
			...
			{
				uint32_t dwRandom = number(3, 10);
				pMap->Spawn((uint32_t)(MOBVNUM_RESPAWN_BOSS_STEP3), stoneSpawnPos[i][0], stoneSpawnPos[i][1] + dwRandom, 0);
				pMap->Spawn((uint32_t)(MOBVNUM_RESPAWN_SUBBOSS_STEP3), stoneSpawnPos[i][0], stoneSpawnPos[i][1] + dwRandom, 0);
				pMap->Spawn((uint32_t)(MOBVNUM_RESPAWN_SUBBOSS_STEP3), stoneSpawnPos[i][0], stoneSpawnPos[i][1] + dwRandom, 0);
			}
...
EVENTFUNC(r_meleyeffect_event)
{
	...
	if (bStep == 1)
	{
		if (bEffectStep == 1)
		{
			...
			if (pMap->GetMapSectree())
			{
				PIXEL_POSITION pos = CMgr::instance().GetXYZ();
...
CMgrMap::CMgrMap(int32_t lMapIndex, uint32_t dwID)
{
	e_pEndEvent = nullptr;
...
LPCHARACTER CMgrMap::Spawn(uint32_t dwVnum, int32_t iX, int32_t iY, int32_t iDir, bool bSpawnMotion)
{
	if (!pkSectreeMap)
		return nullptr;
...
{
	sys_log(0, "StartDungeon: 0");
	if (!pkChar)
		return;
	sys_log(0, "StartDungeon: 1");
	...
	pkChar->ChatPacket(CHAT_TYPE_NOTICE, szBuf1);
	pkChar->ChatPacket(CHAT_TYPE_NOTICE, szBuf2);
	
	pkChar->SetQuestNPCID(0);
	pkGate->Dead();
	pkGate = nullptr;
	sys_log(0, "StartDungeon: 3");

	SetDungeonStep(1);
	if (e_pEndEvent != nullptr)
		event_cancel(&e_pEndEvent);
	sys_log(0, "StartDungeon: 4");
...
void CMgrMap::DungeonResult()
{
	...
	{
		v_Rewards.clear();
		SetRewardTime(get_global_time() + 75);
		pkStatue1->Dead();
		pkStatue2->Dead();
		pkStatue3->Dead();
		pkStatue4->Dead();
...
void CMgrMap::GiveReward(LPCHARACTER pkChar, uint8_t bReward)
{
	uint32_t dwPlayerID = pkChar->GetPlayerID();
...
bool CMgr::Enter(LPCHARACTER pkChar)
{
	if (pkChar)
	{
		...
		if (!pos.x)
		{
			LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap((int32_t)(MAP_INDEX));
			if (pkSectreeMap)
		...
		if (!posSub.x)
		{
			LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap((int32_t)(SUBMAP_INDEX));
			if (pkSectreeMap)
				SetSubXYZ(pkSectreeMap->m_setting.iBaseX + 87 * 100, pkSectreeMap->m_setting.iBaseY + 853 * 100, 0);
		}
		uint32_t dwID = ++dwUniqueID;

		CMgrMap* pMap = M2_NEW CMgrMap(lMapIndex, dwID);
		if (!pMap)
			return false;
...
void CMgr::Leave(LPCHARACTER pkChar, bool bSendOut)
{
	if (!pkChar)
		return;
...
void CMgr::LeaveRequest(LPCHARACTER pkChar)
{
	if (!pkChar)
		return;
...
void CMgr::Check(LPCHARACTER pkChar)
{
	if (!pkChar)
		return;
...
void CMgr::Start(LPCHARACTER pkChar)
{
	sys_log(0, "Start: 0");
	...
	sys_log(0, "Start: 2 %x %x", pkChar, pMap);
}

bool CMgr::Damage(LPCHARACTER pkStatue, LPCHARACTER pkChar)
{
	if (!pkStatue || !pkChar)
		return false;
...
void CMgr::OnKill(uint32_t dwVnum, LPCHARACTER pkKiller)
{
	if (!dwVnum || !pkKiller)
		return;
...
void CMgr::OnKillStatue(LPITEM pkItem, LPCHARACTER pkChar, LPCHARACTER pkStatue)
{
...
void CMgr::OnKillCommon(LPCHARACTER pkMonster, LPCHARACTER pkChar)
{
	if (!pkMonster || !pkChar)
		return;
...
bool CMgr::CanGetReward(LPCHARACTER pkChar)
{
	if (!pkChar)
		return false;
...
void CMgr::Reward(LPCHARACTER pkChar, uint8_t bReward)
{
	if (!pkChar)
		return;
...
void CMgr::DungeonSet(LPCHARACTER pkChar)
{
	if (!pkChar)
		return;
	...
	{
		auto* pMap = it.second;
		if (pMap && pMap->IsPartecipant(pkChar->GetPlayerID()))
			pkChar->SetMeley(pMap);
	}
}
