...
void fdwatch_clear_fd(LPFDWATCH fdw, int32_t fd)
{
    fdw->fd_data[fd] = nullptr;
    fdw->fd_rw[fd] = 0;
}

void fdwatch_add_fd(LPFDWATCH fdw, int32_t fd, void * client_data, int rw, int oneshot)
{
	int flag;
...
void fdwatch_del_fd(LPFDWATCH fdw, int32_t fd)
{
    fdwatch_register(fdw, EV_DELETE, fd, 0);
    fdwatch_clear_fd(fdw, fd);
}

void fdwatch_clear_event(LPFDWATCH fdw, int32_t fd, unsigned int event_idx)
{
...
int fdwatch_check_event(LPFDWATCH fdw, int32_t fd, unsigned int event_idx)
{
    assert(event_idx < fdw->nfiles * 2);
...
int fdwatch_get_buffer_size(LPFDWATCH fdw, int32_t fd)
{
...
static bool win32_init()
{
    ...
    uint16_t wVersion = MAKEWORD(2, 0);
    WSADATA wsaData;
...
LPFDWATCH fdwatch_new(int nfiles)
{
    ...
    CREATE(fdw->select_fds, int32_t, nfiles);
...
static int fdwatch_get_fdidx(LPFDWATCH fdw, int32_t fd) {
	int i;
...
int fdwatch_check_fd(LPFDWATCH fdw, int32_t fd)
{
    int idx = fdwatch_get_fdidx(fdw, fd);
	if (idx < 0) {
		return 0;
...
void fdwatch_clear_event(LPFDWATCH fdw, int32_t fd, unsigned int event_idx)
{
	...
	int32_t rfd = fdw->select_fds[idx];
	if (fd != rfd) {
		return;
	}
