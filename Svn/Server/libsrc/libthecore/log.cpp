...
static int32_t log_level_bits = 0;

void log_set_level(int32_t bit)
{
	log_level_bits = bit;
}
...
int32_t log_init(void)
{
	...
	do
	{
		...
		if( nullptr == log_file_sys ) break;

		log_file_err = log_file_init(SYSERR_FILENAME, "a+");
		if( nullptr == log_file_err ) break;

		log_file_pt = log_file_init(PTS_FILENAME, "w");
		if( nullptr == log_file_pt ) break;
...
void sys_log(int32_t bit, const char *format, ...)
{
	// sys_err("%d %d", bit, log_level_bits);
	if (bit > log_level_bits)
		return;
...
void log_file_delete_old(const char *filename)
{
	...
	new_tm = *tm_calc(nullptr, -log_keep_days);
