...
bool CStmt::Prepare(CAsyncSQL * sql, const char * c_pszQuery)
{
	...
	for (uint32_t i = 0; i < m_stQuery.length(); ++i)
		if (c_pszQuery[i] == '?')
			++iParamCount;
	...
	if (iParamCount)
	{
		...
		m_puiParamLen = (uint32_t *) calloc(iParamCount, sizeof(uint32_t));
	}
...
bool CStmt::BindParam(enum_field_types type, void * p, int32_t iMaxLen)
{
	...
	bind->length	= (long unsigned int*)m_puiParamLen + m_uiParamCount;
...
bool CStmt::BindResult(enum_field_types type, void * p, int32_t iMaxLen)
{
...
int32_t CStmt::Execute()
{
	...
	for (uint32_t i = 0; i < m_uiParamCount; ++i)
	{
