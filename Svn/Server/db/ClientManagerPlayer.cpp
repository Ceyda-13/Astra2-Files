...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, uint32_t dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		sys_log(0, "[PLAYER_LOAD] ID %s pid %d gold %llu ", pTab->name, pTab->id, pTab->gold);
		...
		if (pSet)
		{
			...
			snprintf(szQuery, sizeof(szQuery),
					...
					GetTablePostfix(), pTab->id);
			// @correction076 ClientHandleInfo+pTab->id
			CDBManager::instance().ReturnQuery(szQuery, QID_AFFECT, peer->GetHandle(), new ClientHandleInfo(dwHandle, pTab->id));
		}
		else
		{
			snprintf(szQuery, sizeof(szQuery),
					"SELECT id,window+0,pos,count,vnum,socket0,socket1,socket2,socket3,socket4,socket5,attrtype0,attrvalue0,attrtype1,attrvalue1,attrtype2,attrvalue2,attrtype3,attrvalue3,attrtype4,attrvalue4,attrtype5,attrvalue5,attrtype6,attrvalue6" // @correction144
#ifdef __SOULBIND__ // @correction202
					",sealbind"
#endif
#ifdef __CHANGE_LOOK__ // @correction192
...
#endif
					" FROM item%s WHERE owner_id=%d AND "
#ifdef __OFFLINE_PRIVATE_SHOP__
					"shop_id = 0 AND window > 0 AND "
#endif
					"(window in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','BELT_INVENTORY'"
#ifdef __SWITCHBOT__
					",'SWITCHBOT'"
#endif
					"))",
...
void CClientManager::ItemAward(CPeer * peer,char* login)
{
	...
	while(it != pSet->end() )
	{
		...
		char command[20] = "";
		// @correction073 directly GetCommand instead of strcpy
...
char* CClientManager::GetCommand(char* str, char* command) // @correction073
{
	char* tok;
...
void CClientManager::RESULT_COMPOSITE_PLAYER(CPeer * peer, SQLMsg * pMsg, uint32_t dwQID)
{
	...
	switch (dwQID)
	{
		...
		case QID_AFFECT:
			...
			// @correction076 RESULT_AFFECT_LOAD+info->player_id
...
// @correction076 (RESULT_AFFECT_LOAD +dwRealPID)
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, uint32_t dwHandle, TPlayerCreatePacket* packet)
{
	...
	sys_log(0, "PlayerCreate accountid %d name %s level %d gold %llu, st %d ht %d job %d",
			packet->account_id,
...
#ifdef __12ZI__
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", %d", packet->player_table.bBead);
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, ", %d", packet->player_table.dwBeadTime);
#endif
	...
	{
		peer->EncodeHeader(HEADER_DG_PLAYER_CREATE_ALREADY, dwHandle, 0);
		sys_log(0, "ALREADY EXIST3 query: %s AffectedRows %lu", queryStr, pMsg3->Get()->uiAffectedRows);
		return;
	}

	player_id = pMsg3->Get()->uiInsertID;
...
void CClientManager::RESULT_HIGHSCORE_REGISTER(CPeer * pkPeer, SQLMsg * msg)
{
	...
	else
	{
		...
		if (row && row[0])
		{
			...
			if (((pi->account_index) && (current_value >= value)) || ((!pi->account_index) && (current_value <= value))) // @correction111
			{
				value = current_value;
			}
			else
