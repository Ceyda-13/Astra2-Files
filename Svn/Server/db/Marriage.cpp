...
namespace marriage
{
	const uint32_t WEDDING_LENGTH = 60 * 60;
	...
	bool CManager::Initialize()
	{
		...
		{
			for (uint32_t uiRow = 0; uiRow != pRes->uiNumRows; ++uiRow)
			{
	...
	void CManager::Update(uint32_t dwPID1, uint32_t dwPID2, int32_t iLovePoint, uint8_t byMarried)
	{
		...
		if (res->uiAffectedRows == 0 || res->uiAffectedRows == (uint32_t)-1)
		{
			sys_log(0, "cannot update marriage : PID:%u %u", dwPID1, dwPID2); // @correction011
			return;
		}
