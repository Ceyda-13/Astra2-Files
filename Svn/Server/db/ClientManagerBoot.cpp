...
bool CClientManager::InitializeMobTable()
{
	...
	if(!nameData.Load("/usr/game/share/conf/mob_names.txt",'\t'))
#else
	...
	cCsvTable data;
#ifdef ENABLE_COMMON_FILES // @correction149
	if(!data.Load("/usr/game/share/conf/mob_proto.txt",'\t'))
#else
...
bool CClientManager::InitializeShopTable()
{
	...
	if (!pRes2 || !pRes2->uiNumRows) // @correction023
	{
	...
	while ((data = mysql_fetch_row(pRes2->pSQLResult)))
	{
		...
		{
			shop_table = new TShopTable{}; // @correction114
...
bool CClientManager::InitializeItemTable()
{
	...
	cCsvTable nameData;
#ifdef ENABLE_COMMON_FILES // @correction149
	if (!nameData.Load("/usr/game/share/conf/item_names.txt",'\t'))
#else
	...
	cCsvTable data;
#ifdef ENABLE_COMMON_FILES // @correction149
	if(!data.Load("/usr/game/share/conf/item_proto.txt",'\t'))
#else
	...
	while (it != m_vec_itemTable.end())
	{
		...
		sys_log(1, "ITEM: #%-5lu %-24s %-24s VAL: %d %d %d %d %d %d WEAR %lu ANTI %lu IMMUNE %lu REFINE %lu REFINE_SET %u MAGIC_PCT %u",
				item_table->dwVnum,
				item_table->szName,
...
bool CClientManager::InitializeBanwordTable()
{
	...
	if (pRes && pRes->uiNumRows == 0) // @correction023
		return true;
...
bool CClientManager::InitializeItemAttrTable()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		TItemAttrTable t{}; // @correction114
