...
#include <unordered_set> // @correction106
...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	void	GetPeerP2PHostNames(std::string& peerHostNames); // @correction109
	...
	void		AddPeer(int32_t fd);
	void		RemovePeer(CPeer * pPeer);
	CPeer *		GetPeer(uint32_t ident); // @correction102 IDENT -> uint32_t
	...
	void		MyshopPricelistUpdate(const TItemPriceListTable* pPacket); // @correction077 (TPacketMyshopPricelistHeader to TItemPriceListTable)
