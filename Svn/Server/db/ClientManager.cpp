...
void CClientManager::GetPeerP2PHostNames(std::string& peerHostNames) // @correction109
{
...
#ifdef ENABLE_DEFAULT_PRIV // @correction110
static bool bCleanOldPriv = true;
static bool __InitializeDefaultPriv()
{
	if (bCleanOldPriv)
	{
		std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery("DELETE FROM priv_settings WHERE value <= 0 OR duration <= NOW();", SQL_COMMON));
		printf("DEFAULT_PRIV_EMPIRE: removed %u expired priv settings.\n", pMsg->Get()->uiAffectedRows);
	}
...
{
	const uint8_t bPacketVersion = 6;
...
#ifdef __AUCTION__
		sizeof(uint16_t) + sizeof(uint16_t) + sizeof(TPlayerItem) * AuctionManager::instance().GetAuctionItemSize() +
		sizeof(uint16_t) + sizeof(uint16_t) + sizeof(TAuctionItemInfo) * AuctionManager::instance().GetAuctionSize() +
		sizeof(uint16_t) + sizeof(uint16_t) + sizeof(TSaleItemInfo) * AuctionManager::instance().GetSaleSize() +
		sizeof(uint16_t) + sizeof(uint16_t) + sizeof(TWishItemInfo) * AuctionManager::instance().GetWishSize() +
		sizeof(uint16_t) + sizeof(uint16_t) + (sizeof(uint32_t) + sizeof(uint32_t) + sizeof(int32_t)) * AuctionManager::instance().GetMyBidSize() +
#endif
		sizeof(time_t) +
...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	if (pi->account_index == 0)
	{
		...
		snprintf(szQuery, sizeof(szQuery),
				"SELECT id, window+0, pos, count, vnum, socket0, socket1, socket2, socket3, socket4, socket5, " // @correction144
				...
				while (it != pSet->end())
				{
					...
					else if ((dwItemVnum == 50300 || dwItemVnum == 70037) && pItemAward->dwSocket0 == 0)
					{
						...
						do
						{
							...
							if (dwSkillVnum > 120) // @correction027
								continue;
...

					{
						...
						snprintf(szQuery, sizeof(szQuery),
								"INSERT INTO item%s (id, owner_id, window, pos, vnum, count, socket0, socket1, socket2, socket3, socket4, socket5) " // @correction144
								"VALUES(%u, %u, '%s', %d, %u, %u, %u, %u, %u, %u, %u, %u)", // @correction144
								GetTablePostfix(),
								GainItemID(),
								pi->account_id,
								pi->ip[0] == 0 ? "SAFEBOX" : "MALL",
								iPos,
								pItemAward->dwVnum, pItemAward->dwCount, pItemAward->dwSocket0, pItemAward->dwSocket1, dwSocket2, pItemAward->dwSocket3, pItemAward->dwSocket4, pItemAward->dwSocket5); // @correction144
					}
					...
					if (!pRes || pRes->uiAffectedRows == 0 || pRes->uiInsertID == 0 || pRes->uiAffectedRows == (uint32_t)-1) // @correction023
						break;
...
// @correction144 BEGIN
					...
					item.alSockets[5] = pItemAward->dwSocket5;
// @correction144 END
...
void CClientManager::RESULT_SAFEBOX_CHANGE_PASSWORD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	if (msg->Get()->uiNumRows > 0)
	{
		...
		if ((row[0] && *row[0] && !strcasecmp(row[0], p->login)) || ((!row[0] || !*row[0]) && !strcmp("000000", p->login))) // @correction111
		{
			char szQuery[QUERY_MAX_LEN];
			char escape_pwd[64];
...
void CClientManager::QUERY_EMPIRE_SELECT(CPeer * pkPeer, uint32_t dwHandle, TEmpireSelectPacket * p)
{
	...
	sys_log(0, "EmpireSelect: %s", szQuery);
	{
		...
		if (pRes && pRes->uiNumRows) // @correction023
		{
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		snprintf(szQuery, sizeof(szQuery),
			"REPLACE INTO item%s (id, owner_id, window, pos, count, vnum, socket0, socket1, socket2, socket3, socket4, socket5, " // @correction144
			"attrtype0, attrvalue0, "
			"attrtype1, attrvalue1, "
			"attrtype2, attrvalue2, "
			"attrtype3, attrvalue3, "
...
#endif
			") "
			"VALUES(%u, %u, %d, %d, %u, %u, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d" // @correction144
#ifdef __SOULBIND__ // @correction202
			", %d"
#endif
#ifdef __CHANGE_LOOK__ // @correction192
...
#endif
#ifdef __OFFLINE_PRIVATE_SHOP__
			, p->shop_id, p->prices, p->bIsLocked
#endif
);
...
void CClientManager::BillingCheck(const char * data)
{
	...
	if (!vec.empty())
		m_pkAuthPeer->Encode(&vec[0], sizeof(uint32_t) * vec.size());
}
...
void CClientManager::MyshopPricelistUpdate(const TItemPriceListTable* pPacket) // @correction077 (TPacketMyshopPricelistHeader to TItemPriceListTable)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_MYSHOP_PRICELIST_UPDATE:
				MyshopPricelistUpdate((TItemPriceListTable*)data); // @correction077 (TPacketMyshopPricelistHeader to TItemPriceListTable)
				break;
...
void CClientManager::AddPeer(int32_t fd)
{
...
CPeer * CClientManager::GetPeer(uint32_t ident) // @correction102 IDENT -> uint32_t
{
	for (itertype(m_peerList) i = m_peerList.begin(); i != m_peerList.end();++i)
	{
		CPeer * tmp = *i;
...
int32_t CClientManager::Process()
{
...
#ifdef ENABLE_ITEMAWARD_REFRESH // @correction113
	...
	for (idx = 0; idx < num_events; ++idx)
	{
		...
		if (!peer)
		{
			...
			{
				sys_log(0, "FDWATCH: peer null in event: ident %d", fdwatch_get_ident(m_fdWatcher, idx)); // @correction011
			}
...
bool CClientManager::InitializeLocalization()
{
	...
	std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery, SQL_COMMON)); // @correction023
	...
	for (int32_t n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != nullptr; ++n)
	{
		...
		// @correction006
		if (strcmp(locale.szKey, "DB_NAME_COLUMN") == 0)
		{
...
bool CClientManager::__GetAdminInfo(const char *szIP, std::vector<tAdminInfo> & rAdminVec)
{
	...
	if (pMsg->Get()->uiNumRows == 0)
	{
		// sys_err("__GetAdminInfo() ==> DirectQuery failed(%s)", szQuery); // @correction012
		return false;
	}
...
bool CClientManager::__GetHostInfo(std::vector<std::string> & rIPVec)
{
	...
	if (pMsg->Get()->uiNumRows == 0)
	{
		// sys_err("__GetHostInfo() ==> DirectQuery failed(%s)", szQuery); // @correction012
		return false;
	}
...
void CClientManager::Election(CPeer * peer, uint32_t dwHandle, const char* data)
{
	...
	if (!(Success = CMonarch::instance().VoteMonarch(selectingpid, idx)))
	{
		...
		peer->EncodeHeader(HEADER_DG_ELECT_MONARCH, dwHandle, sizeof(int32_t));
		peer->Encode(&Success, sizeof(int32_t));
		return;
	}
	else
	{
...
void CClientManager::Candidacy(CPeer *  peer, uint32_t dwHandle, const char* data)
{
	...
	if (!CMonarch::instance().AddCandidacy(pid, data))
	{
		...
		peer->EncodeHeader(HEADER_DG_CANDIDACY, dwHandle, sizeof(int32_t) + 32);
		peer->Encode(0, sizeof(int32_t));
		peer->Encode(data, 32);
		return;
	}
	else
	{
		...
		for (itertype(m_peerList) it = m_peerList.begin(); it != m_peerList.end(); ++it)
		{
			...
			if (p == peer)
			{
				p->EncodeHeader(HEADER_DG_CANDIDACY, dwHandle, sizeof(int32_t) + 32);
				p->Encode(&pid, sizeof(int32_t));
				p->Encode(data, 32);
			}
			else
			{
				p->EncodeHeader(HEADER_DG_CANDIDACY, 0, sizeof(int32_t) + 32);
				p->Encode(&pid, sizeof(int32_t));
				p->Encode(data, 32);
			}
		}
...
void CClientManager::AddMonarchMoney(CPeer * peer, uint32_t dwHandle, const char * data)
{
	...
	for (itertype(m_peerList) it = m_peerList.begin(); it != m_peerList.end(); ++it)
	{
		...
		if (p == peer)
		{
			p->EncodeHeader(HEADER_DG_ADD_MONARCH_MONEY, dwHandle, sizeof(int32_t) + sizeof(int32_t));
			p->Encode(&Empire, sizeof(int32_t));
			p->Encode(&Money, sizeof(int32_t));
		}
		else
		{
			p->EncodeHeader(HEADER_DG_ADD_MONARCH_MONEY, 0, sizeof(int32_t) + sizeof(int32_t));
			p->Encode(&Empire, sizeof(int32_t));
			p->Encode(&Money, sizeof(int32_t));
		}
...
void CClientManager::DecMonarchMoney(CPeer * peer, uint32_t dwHandle, const char * data)
{
	int32_t Empire = *(int32_t *) data;
	data += sizeof(int32_t);
	...
	for (itertype(m_peerList) it = m_peerList.begin(); it != m_peerList.end(); ++it)
	{
		...
		if (p == peer)
		{
			p->EncodeHeader(HEADER_DG_DEC_MONARCH_MONEY, dwHandle, sizeof(int32_t) + sizeof(int32_t));
			p->Encode(&Empire, sizeof(int32_t));
			p->Encode(&Money, sizeof(int32_t));
		}
		else
		{
			p->EncodeHeader(HEADER_DG_DEC_MONARCH_MONEY, 0, sizeof(int32_t) + sizeof(int32_t));
			p->Encode(&Empire, sizeof(int32_t));
			p->Encode(&Money, sizeof(int32_t));
		}
	}
...
void CClientManager::TakeMonarchMoney(CPeer * peer, uint32_t dwHandle, const char * data)
{
	...
	{
		peer->EncodeHeader(HEADER_DG_TAKE_MONARCH_MONEY, dwHandle, sizeof(int32_t) + sizeof(int32_t));
		peer->Encode(&Empire, sizeof(int32_t));
		peer->Encode(&Money, sizeof(int32_t));
	}
	else
	{
		Money = 0;
...
void CClientManager::RMCandidacy(CPeer * peer, uint32_t dwHandle, const char * data)
{
	...
	int32_t iRet = CMonarch::instance().DelCandidacy(szName) ? 1 : 0;
	...
	if (1 == iRet)
	{
		for (itertype(m_peerList) it = m_peerList.begin(); it != m_peerList.end(); ++it)
		{
			...
			if (p == peer)
			{
				p->EncodeHeader(HEADER_DG_RMCANDIDACY, dwHandle, sizeof(int32_t) + sizeof(szName));
				p->Encode(&iRet, sizeof(int32_t));
...
void CClientManager::SetMonarch(CPeer * peer, uint32_t dwHandle, const char * data)
{
	...
	int32_t iRet = CMonarch::instance().SetMonarch(szName) ? 1 : 0;
...
void CClientManager::RMMonarch(CPeer * peer, uint32_t dwHandle, const char * data)
{
	...
	int32_t iRet = CMonarch::instance().DelMonarch(szName) ? 1 : 0;
	...
	if (1 == iRet)
	{
		for (itertype(m_peerList) it = m_peerList.begin(); it != m_peerList.end(); ++it)
		{
			...
			if (p == peer)
			{
				p->EncodeHeader(HEADER_DG_RMMONARCH, dwHandle, sizeof(int32_t) + sizeof(szName));
				p->Encode(&iRet, sizeof(int32_t));
				p->Encode(szName, sizeof(szName));
			}
			else
...
void CClientManager::ChangeMonarchLord(CPeer * peer, uint32_t dwHandle, TPacketChangeMonarchLord* info)
{
	...
	std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery, SQL_PLAYER)); // @correction023
	...
	if (pMsg->Get()->uiNumRows != 0)
	{
		...
		std::unique_ptr<SQLMsg> pMsg2(CDBManager::instance().DirectQuery(szQuery, SQL_PLAYER)); // @correction023
...
void CClientManager::ChargeCash(const TRequestChargeCash* packet)
{
	...
	if (ERequestCharge_Cash == packet->eChargeType)
		sprintf(szQuery, "update account set cash = cash + %d where id = %d limit 1", packet->dwAmount, packet->dwAID); // @correction028
	else if(ERequestCharge_Mileage == packet->eChargeType)
		sprintf(szQuery, "update account set mileage = mileage + %d where id = %d limit 1", packet->dwAmount, packet->dwAID); // @correction028
	else
	{
...
#ifdef __AUCTION__
void CClientManager::EnrollInAuction (CPeer * peer, uint32_t owner_id, AuctionEnrollProductInfo* data)
{
...
void CClientManager::EnrollInSale (CPeer * peer, uint32_t owner_id, AuctionEnrollSaleInfo* data)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (owner_id);
...
void CClientManager::EnrollInWish (CPeer * peer, uint32_t wisher_id, AuctionEnrollWishInfo* data)
{
...
void CClientManager::AuctionBid (CPeer * peer, uint32_t bidder_id, AuctionBidInfo* data)
{
...
void CClientManager::AuctionImpur (CPeer * peer, uint32_t purchaser_id, AuctionImpurInfo* data)
{
...
void CClientManager::AuctionGetAuctionedItem (CPeer * peer, uint32_t actor_id, uint32_t item_id)
{
...
void CClientManager::AuctionBuySoldItem (CPeer * peer, uint32_t actor_id, uint32_t item_id)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (actor_id);
	AuctionResult result = AUCTION_FAIL;
	if (it == m_map_playerCache.end())
...
void CClientManager::AuctionCancelAuction (CPeer * peer, uint32_t actor_id, uint32_t item_id)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (actor_id);
	AuctionResult result = AUCTION_FAIL;
	if (it == m_map_playerCache.end())
...
void CClientManager::AuctionCancelWish (CPeer * peer, uint32_t actor_id, uint32_t item_num)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (actor_id);
	AuctionResult result = AUCTION_FAIL;
	if (it == m_map_playerCache.end())
...
void CClientManager::AuctionCancelSale (CPeer * peer, uint32_t actor_id, uint32_t item_id)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (actor_id);
	AuctionResult result = AUCTION_FAIL;
	if (it == m_map_playerCache.end())
...
void CClientManager::AuctionDeleteAuctionItem (CPeer * peer, uint32_t actor_id, uint32_t item_id)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (actor_id);
	AuctionResult result = AUCTION_FAIL;
	if (it == m_map_playerCache.end())
...
void CClientManager::AuctionDeleteSaleItem (CPeer * peer, uint32_t actor_id, uint32_t item_id)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (actor_id);
	AuctionResult result = AUCTION_FAIL;
	if (it == m_map_playerCache.end())
...
void CClientManager::AuctionReBid (CPeer * peer, uint32_t bidder_id, AuctionBidInfo* data)
{
	TPlayerTableCacheMap::iterator it = m_map_playerCache.find (bidder_id);
	...
	if (result <= AUCTION_FAIL)
	{
		...
		peer->EncodeHeader(HEADER_DG_AUCTION_RESULT, bidder_id, sizeof(TPacketDGResultAuction) + sizeof(int32_t));
		peer->Encode(&enroll_result, sizeof(TPacketDGResultAuction));
...
void CClientManager::AuctionBidCancel (CPeer * peer, uint32_t bidder_id, uint32_t item_id)
{
