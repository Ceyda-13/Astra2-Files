...
void AuctionManager::LoadAuctionItem()
{
	...
	snprintf(szQuery, sizeof(szQuery),
		"SELECT id,	owner_id, count, vnum, socket0, socket1, socket2, socket3, socket4, socket5, " // @correction144
	...
	std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery)); // @correction023
...
AuctionResult AuctionManager::DeleteAuctionItem (uint32_t actor_id, uint32_t item_id)
{
	...
	if (Auction.DeleteItemInfoCache (item_id) == nullptr)
	{
		sys_err ("how can this accident happen?");
		return AUCTION_FAIL;
	}
...
AuctionResult AuctionManager::DeleteSaleItem (uint32_t actor_id, uint32_t item_id)
{
	...
	if (Sale.DeleteItemInfoCache (item_id) == nullptr)
	{
		sys_err ("how can this accident happen?");
		return AUCTION_FAIL;
	}
