...
struct TItemAttrTable
{
	...
	uint32_t   dwProb;
	int32_t    lValues[ITEM_ATTRIBUTE_MAX_LEVEL]; // @correction105
...
{
	char	login[LOGIN_MAX_LEN + 1];
	uint32_t	ident; // @correction102 IDENT -> uint32_t
} TConnectTable;
...
typedef struct SPacketGDSetup
{
	...
	uint16_t	wP2PPort;
	int32_t	alMaps[MAP_ALLOW_LIMIT]; // @correction100
...
typedef struct SAccumulate
{
	...
	SAccumulate(uint32_t vid, uint32_t count)
	{
		dwVid = vid;
		dwCount = count;
	}
...
{
	TPlayerItemAttribute attributes[ITEM_ATTRIBUTE_NORM_END];
