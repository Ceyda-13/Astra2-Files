...
#if 0
	if (g_nmApp->SelfHooksInstance()->BlockAPI(XOR("ntdll.dll"), XOR("DbgUiIssueRemoteBreakin"), RET_HOOK))
	{
		g_nmApp->OnCloseRequest(EXIT_ERR_HOOK_PATCH_6_FAIL, g_winapiApiTable->GetLastError());
		return false;
	}
	if (g_nmApp->SelfHooksInstance()->BlockAPI(XOR("ntdll.dll"), XOR("DbgUiRemoteBreakin"), RET_HOOK))
	{
		g_nmApp->OnCloseRequest(EXIT_ERR_HOOK_PATCH_7_FAIL, g_winapiApiTable->GetLastError());
		return false;
	}
	if (g_nmApp->SelfHooksInstance()->BlockAPI(XOR("ntdll.dll"), XOR("DbgBreakPoint"), NOP_HOOK))
	{
		g_nmApp->OnCloseRequest(EXIT_ERR_HOOK_PATCH_8_FAIL, g_winapiApiTable->GetLastError());
		return false;
	}
	if (g_nmApp->SelfHooksInstance()->BlockAPI(XOR("ntdll.dll"), XOR("DbgUserBreakPoint"), NOP_HOOK))
	{
...
{
	auto pUserSharedData = (KUSER_SHARED_DATA *)0x7FFE0000;
...
bool CAnticheatManager::DebugObjectHandleCheck()
{
	...
	auto ntStatus = NtQueryInformationProcess(GetCurrentProcess(),  30, &hDebugObject, sizeof(HANDLE), nullptr);
...
bool CAnticheatManager::DebugCheckBugControl()
{
	...
	auto dwBugCheck = 0UL;
	auto ntStatus = NtQueryInformationProcess(GetCurrentProcess(),  30, (PVOID)&dwBugCheck, sizeof(dwBugCheck), &dwBugCheck);
...
bool CAnticheatManager::SimpleDebugCheck()
{
	...
	if (NT_SUCCESS(NtQueryInformationProcess(GetCurrentProcess(),  31, &dwFlags, sizeof(dwFlags), &dwReturnSize)))
	{
