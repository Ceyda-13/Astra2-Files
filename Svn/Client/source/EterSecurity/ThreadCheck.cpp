...
bool CAnalyser::OnThreadCreated(DWORD dwThreadID, LPVOID lpStartAddress, LPVOID lpEIP, LPDWORD pdwErrorType)
{
	...
	if (!_pStartAddress)
	{
		...
		Logf(CUSTOM_LOG_FILENAME, XOR("Illegal thread owner: %s\n"), szTempName.c_str());
	...
	if (dwStartAddress != (DWORD)_pStartAddress)
	{
	...
	if (g_nmApp->DynamicWinapiInstance()->IsLoadedAddress(dwStartAddress))
	{
	...
	auto dwDbgUiRemoteBreakin = (DWORD)g_winapiApiTable->GetProcAddress(g_winapiModuleTable->hNtdll, XOR("DbgUiRemoteBreakin"));
	...
	{
		if (mbi.AllocationBase && mbi.AllocationBase == _pStartAddress) 
		{
