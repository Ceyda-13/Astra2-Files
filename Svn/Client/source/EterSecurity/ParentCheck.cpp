...
void CAnticheatManager::ParentProcessCheck(const std::string& stPatcherName)
{
	...
	else
	{
		...
		if (!strcmp(stParentProcessName.c_str(), szLowerPatcherName.c_str())
			|| !strcmp(stParentProcessName.c_str(), XOR("astra2 patcher yedek.exe"))
			|| !strcmp(stParentProcessName.c_str(), XOR("astra2_patcher_yedek.exe"))
			|| !strcmp(stParentProcessName.c_str(), XOR("astra2_patcher.exe"))
			|| !strcmp(stParentProcessName.c_str(), XOR("yeni astra2 patcher.exe"))
		)
			return;
		/*if (0 == _access(XOR("Astra2.bin"), 0))
			return;*/

		auto stExePath = ExePath();
		std::transform(stExePath.begin(), stExePath.end(), stExePath.begin(), tolower);
		//_TraceError("%s %s", stParentProcessName.c_str(), stExePath.c_str());
