...
bool CPythonSkill::RegisterSkillDesc(const char * c_szFileName)
{
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		if (TokenVector.size() > DESC_TOKEN_TYPE_MOTION_INDEX_GRADE_NUM)
		{
			...
			{
				TraceError("%s[%s] Skill grade motion [%d] Out Of Range.",rSkillData.strName.c_str(), TokenVector[DESC_TOKEN_TYPE_MOTION_INDEX_GRADE_NUM].c_str(), SKILL_EFFECT_COUNT); // @@correction026
				return false;
			}
...
CPythonSkill::CPythonSkill()
{
	...
	m_SkillNeedWeaponIndexMap.insert(std::map<std::string, DWORD>::value_type(std::string("MOUNT_SPEAR"), SKILL_NEED_WEAPON_MOUNT_SPEAR)); // @@correction025
#ifdef ENABLE_WOLFMAN_CHARACTER
	...
	m_SkillWeaponTypeIndexMap.insert(make_pair(std::string("ARROW"), CItemData::WEAPON_ARROW || CItemData::WEAPON_QUIVER));
#else
	m_SkillWeaponTypeIndexMap.insert(make_pair(std::string("ARROW"), CItemData::WEAPON_ARROW));
#endif
	m_SkillWeaponTypeIndexMap.insert(make_pair(std::string("MOUNT_SPEAR"), CItemData::WEAPON_MOUNT_SPEAR)); // @@correction025
#ifdef ENABLE_WOLFMAN_CHARACTER
