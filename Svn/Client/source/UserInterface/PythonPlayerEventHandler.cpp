...
__forceinline void CPythonPlayerEventHandler::OnHit(UINT uSkill, CActorInstance& rkActorVictim, BOOL isSendPacket)
#else
...
//#define ATTACK_TIME_LOG
...
#ifdef ENABLE_ANTICHEAT
		string st(__FUNCTION__);
		DWORD dwCRC = GetCaseCRC32(st.c_str(), st.length());
		//rkStream.SendAttackPacketFromThread(uSkill, dwVIDVictim, THREAD_INFOS);
		TPacketCGAttack kPacketAtk;

		kPacketAtk.header = HEADER_CG_ATTACK;
		kPacketAtk.bType = uSkill;
		...
		kPacketAtk.dwLine = __LINE__;
		kPacketAtk.dwCrc = dwCRC;
		rkStream.Send(sizeof(kPacketAtk), &kPacketAtk);
#else
...
void CPythonPlayerEventHandler::CNormalBowAttack_FlyEventHandler_AutoClear::OnExplodingAtAnotherTarget(DWORD dwSkillIndex, DWORD dwVID)
{
	...
	Tracef("Shoot : Hitting another target : %d, %d\n", dwSkillIndex, dwVID); // @@correction026

	CPythonNetworkStream& rkStream=CPythonNetworkStream::Instance();
#ifdef ENABLE_ANTICHEAT
	//string st(__FUNCTION__);
	//DWORD dwCRC = GetCaseCRC32(st.c_str(), st.length());
	rkStream.SendAttackPacketFromThread(dwSkillIndex, dwVID, 0, 0);

	/*TPacketCGAttack kPacketAtk;

	kPacketAtk.header = HEADER_CG_ATTACK;
	kPacketAtk.bType = dwSkillIndex;
