...
void CPythonExchange::SetElkToTarget(uint64_t elk)
{
	m_victim.elk = elk;
}

void CPythonExchange::SetElkToSelf(uint64_t elk)
{
	m_self.elk = elk;
}

uint64_t CPythonExchange::GetElkFromTarget()
{
	return m_victim.elk;
}

uint64_t CPythonExchange::GetElkFromSelf()
{
	return m_self.elk;
}
...
void CPythonExchange::SetItemChangeLook(int iPos, DWORD dwChangeLook, bool bSelf)
{
	if (iPos >= EXCHANGE_ITEM_MAX_NUM)
		return;

	if (bSelf)
		m_self.changelook[iPos] = dwChangeLook;
	else
		m_victim.changelook[iPos] = dwChangeLook;
}

DWORD CPythonExchange::GetItemChangeLook(int iPos, bool bSelf)
{
	if (iPos >= EXCHANGE_ITEM_MAX_NUM)
		return 0;

	DWORD dwChangeLook = bSelf == true ? m_self.changelook[iPos] : m_victim.changelook[iPos];
	return dwChangeLook;
}
#endif
