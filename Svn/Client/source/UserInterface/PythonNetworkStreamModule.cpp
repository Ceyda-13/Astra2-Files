...
PyObject* netSendItemDestroyPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendItemDestroyPacket(Cell, type);
	return Py_BuildNone();
}
...
{
	uint64_t iElk;
	if (!PyTuple_GetUnsignedLongLong(poArgs, 0, &iElk))
		return Py_BuildException();
...
//	rns.SendRequestRefineInfoPacket(iSlotIndex);
	assert(!"netSendRequestRefineInfoPacket - This function is not used anymore"); // @@correction026
...
PyObject* netSendChestDropInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChestDropInfo(Cell);
...
{
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMiniGameRumiStart();
	return Py_BuildNone();
}
...
PyObject* netSendMiniGameRumiHandCardClick(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMiniGameRumiHandCardClick(bAdd, bIndex);
	return Py_BuildNone();
}
...
PyObject* netSendMiniGameRumiFieldCardClick(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMiniGameRumiFieldCardClick(bIndex);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendMiniGameAttendanceButtonClick(PyObject* poSelf, PyObject* poArgs)
{
	int iNumber;
	if (!PyTuple_GetInteger(poArgs, 0, &iNumber))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendMiniGameAttendanceButtonClick(iNumber);
	return Py_BuildNone();
}

PyObject* netSendMiniGameAttendanceRequestRewardList(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream::Instance().SendMiniGameAttendanceRequestRewardList();
	return Py_BuildNone();
}

PyObject* netSendMiniGameAttendanceRequestData(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bType;
	if (!PyTuple_GetInteger(poArgs, 0, &bType))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendMiniGameAttendanceRequestData(bType);
	return Py_BuildNone();
}
#endif
...
{
	CPythonNetworkStream::Instance().SendRequestFishEventBlock();
	return Py_BuildNone();
}
#endif
...
#endif
#if defined(ENABLE_MONSTER_BACK) || defined(ENABLE_CARNIVAL2016)
		{ "SendMiniGameAttendanceButtonClick",			netSendMiniGameAttendanceButtonClick,		METH_VARARGS },
		{ "SendMiniGameAttendanceRequestRewardList",	netSendMiniGameAttendanceRequestRewardList,	METH_VARARGS },
		{ "SendMiniGameAttendanceRequestData",			netSendMiniGameAttendanceRequestData,		METH_VARARGS },
#endif
#ifdef ENABLE_FISH_EVENT
