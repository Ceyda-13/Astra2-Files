...
class CPythonEventManager : public CSingleton<CPythonEventManager>
{
	public:
		...
		enum
		{
			...
			BOX_VISIBLE_LINE_COUNT = 15, // @@correction045 5 ->15
		};
	...
	public:
		typedef struct SEventSet
		{
			...
			int iTotalLineCount; // @@correction045
			BOOL isConfirmWait;
	...
	public:
		...
		// @@correction045 BEGIN
		void SetFontColor(int iIndex, float fRed, float fGreen, float fBlue, float fAlpha);
		void SetTotalLineCount(TEventSet * pEventSet, std::string strEventString);
	...
	protected:
		...
		bool ProcessEventSet(TEventSet * pEventSet); // @@correction045 void -> bool
