...
void CPythonSystem::SetDefaultConfig()
{
	...
	m_Config.bYangInfo = false;
#endif
#ifdef ENABLE_AUTO_PICKUP
	m_Config.bAutoPick = false;
#endif
}
...
#endif
#ifdef ENABLE_YANG_INFO
		else if (!stricmp(command, "YANG_INFO_ON"))
			m_Config.bYangInfo = atoi(value) == 1 ? true : false;
#endif
#ifdef ENABLE_AUTO_PICKUP
		else if (!stricmp(command, "AUTO_PICK"))
			m_Config.bAutoPick = atoi(value) == 1 ? true : false;
#endif
	}
...
#endif
#ifdef ENABLE_YANG_INFO
	fprintf(fp, "YANG_INFO_ON			%d\n", m_Config.bYangInfo);
#endif
#ifdef ENABLE_AUTO_PICKUP
	fprintf(fp, "AUTO_PICK				%d\n", m_Config.bAutoPick);
#endif
	fprintf(fp, "\n");
...
#ifdef ENABLE_YANG_INFO
void CPythonSystem::SetYangInfo(int iFlag)
{
	m_Config.bYangInfo = iFlag == 1 ? true : false;
}

bool CPythonSystem::GetYangInfo()
{
	return m_Config.bYangInfo;
}
#endif

#ifdef ENABLE_AUTO_PICKUP
void CPythonSystem::SetAutoPick(int iFlag)
{
	m_Config.bAutoPick = iFlag == 1 ? true : false;
}

bool CPythonSystem::GetAutoPick()
{
	return m_Config.bAutoPick;
}
#endif