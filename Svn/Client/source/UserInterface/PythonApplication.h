...
#include "../eterSecurity/CheatQuarentineManager.h"
#include "../eterSecurity/PythonStackCheck.h"
#include "../eterSecurity/HWIDMANAGER.h"
#endif
...
class CPythonApplication : public CMSApplication, public CInputKeyboard, public IAbstractApplication
{
...
#ifdef ENABLE_OFFLINE_PRIVATE_SHOP
		CPythonOffflinePrivateShop 	m_pyOfflinePrivateShop;
#endif
#ifdef ENABLE_GROWTH_PET_SYSTEM
	...
	protected:
		std::map<int, std::string> m_locale_quest;
		std::map<int, std::string> m_locale_string;

	public:
		bool SplitLineByStr(const std::string& str, const char* c_szDelimeter, CTokenVector* pstTokenVector);
		bool LoadLocaleQuest(const char* c_szFileName);
		std::string FindLocaleQuest(DWORD idx);
		std::string Convert_arg(const char* c_szFormat ...);

		bool LoadLocaleString(const char* c_szFileName);
		std::string FindLocaleString(DWORD idx);
#endif
