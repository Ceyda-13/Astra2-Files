...
#else
		int iWidth = pChatSet->m_ix;
		bool isAttached = (pChatLine->InstanceEmpire.GetWidth() > 0);

		if (isAttached)
		{
			pChatLine->InstanceEmpire.SetPosition(iWidth, pChatSet->m_iy + iHeight + 2);
			iWidth += pChatLine->InstanceEmpire.GetWidth() + 2;
		}
...
void CPythonChat::AppendChat(int iType, const char * c_szChat)
{
	...
	{
#ifdef ENABLE_MULTI_LANGUAGE
		std::string LangFlag("/language/");
		int position = strContent.find(LangFlag);

		if (position != std::string::npos)
		{
			strContent = strContent.substr(position + LangFlag.length(), strContent.length());
			position = strContent.find("/");
			std::string c_szLang = strContent.substr(0, position);

			char buf[12 + 1];
			sprintf(buf, "multi/%s.tga", c_szLang.c_str());
...
#endif
		std::string EmpireFlag("/empire/");
		position = strContent.find(EmpireFlag);

		if (position != std::string::npos)
		{
			strContent = strContent.substr(position + EmpireFlag.length(), strContent.length());
			position = strContent.find("/");
			std::string c_szEmpire = strContent.substr(0, position);

			char buf[128];
			sprintf(buf, "d:/ymir work/ui/empire/empire_%s_256.dds", c_szEmpire.c_str());
...
void CPythonChat::__Initialize()
{
	...
	m_akD3DXClrChat[CHAT_TYPE_MONARCH_NOTICE]	= D3DXCOLOR(1.0f, 0.902f, 0.730f, 1.0f); // @@correction025
#ifdef ENABLE_12ZI
