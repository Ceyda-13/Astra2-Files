...
{
	assert(!"CPythonPlayer::SetSkillLevel - Don't use this function"); // @@correction026
	if (dwSlotIndex >= SKILL_MAX_NUM)
		return;
...
void CPythonPlayer::SetItemChangeLook(TItemPos Cell, DWORD dwVnum)
{
	if (!Cell.IsValidCell())
		return;

	(const_cast <TItemData *>(GetItemData(Cell)))->changelook = dwVnum;
	PyCallClassMemberFunc(m_ppyGameWindow, "RefreshInventory", Py_BuildValue("()"));
}

DWORD CPythonPlayer::GetItemChangeLook(TItemPos Cell)
{
	if (Cell.IsValidCell())
	{
		const TItemData * pkItem = GetItemData(Cell);
		if (pkItem)
			return pkItem->changelook;
	}
...
#if defined(ENABLE_MONSTER_BACK) || defined(ENABLE_CARNIVAL2016)
#ifdef ENABLE_ACCUMULATE_DAMAGE_DISPLAY
void CPythonPlayer::SetAccumulateDamage(DWORD dwVid, DWORD dwCount)
{
	bool bContain = false;

	if (!m_Accumulate.empty())
	{
		for (DWORD i = 0; i < m_Accumulate.size(); i++)
		{
			if (m_Accumulate[i].dwVid == dwVid)
			{
				bContain = true;
				m_Accumulate[i].dwCount  = dwCount;
				break;
			}
		}
	}
	...
	if (!bContain)
		m_Accumulate.push_back(TAccumulate(dwVid, dwCount));
}

DWORD CPythonPlayer::GetAccumulateDamageByVID(DWORD dwVid)
{
	DWORD dwCount = 0;
	...
	for (DWORD i = 0; i < m_Accumulate.size(); i++)
	{
		if (m_Accumulate[i].dwVid == dwVid)
		{
			dwCount = m_Accumulate[i].dwCount;
			break;
		}
	}
