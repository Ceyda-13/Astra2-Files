...
void CInstanceBase::ProcessDamage()
{
	...
	*/
	...
	while(damage>0)
	{
		if(index > 7)
		{
			TraceError("ProcessDamage Possibility of endless loop"); // @@correction026
			break;
		}
		num = damage%10;
		damage /= 10;
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	if (GetRace() == 3 && GetInstanceType() == CActorInstance::TYPE_NPC && m_kHorse.GetActorPtr() && m_kHorse.GetActorPtr() != NULL)
	{
		float asd = 0.5f + 0.02f * GetLevel();
		m_kHorse.GetActorPtr()->SetScale(asd, asd, asd);
	}
#endif
	...
	if (IsEnemy() || IsStone())
		s_kLevelColor = D3DXCOLOR(119.0f / 255.0f, 246.0f / 255.0f, 168.0f / 255.0f, 1.0f);
#endif
	char szText[256];
...
void CInstanceBase::__ClearAffects()
{
	...
	if ((IsStone()) && (GetVirtualNumber() != GUILD_DRAGONLAIR_STATUE))
#else
	if (IsStone())
#endif
	{
...
#ifdef ENABLE_GUILD_DRAGONLAIR_SYSTEM
	else if ((IsStone()) && (GetVirtualNumber() != GUILD_DRAGONLAIR_STATUE))
#else
	else if (IsStone())
#endif
	{
