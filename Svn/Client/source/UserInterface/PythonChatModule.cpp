...
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
			len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x", htoi(results[9].c_str()));
#endif
...
void initChat()
{
	...
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_BIG_NOTICE",	CHAT_TYPE_BIG_NOTICE); // @@correction025
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_MONARCH_NOTICE",	CHAT_TYPE_MONARCH_NOTICE); // @@correction025
#ifdef ENABLE_12ZI
