...
bool CPythonApplication::Process()
{
	...
	if (CPythonSystem::Instance().GetPrivateShopLevel())
		m_kChrMgr.SetPrivateShopLevel(CPythonSystem::Instance().GetPrivateShopLevel());
	if (CPythonSystem::Instance().IsPetStatus())
		m_kChrMgr.SetPetStatus(CPythonSystem::Instance().IsPetStatus());
	if (CPythonSystem::Instance().IsNpcNameStatus())
		m_kChrMgr.SetNpcNameStatus(CPythonSystem::Instance().IsNpcNameStatus());
	if (CPythonSystem::Instance().GetEffectLevel())
		m_kChrMgr.SetEffectLevel(CPythonSystem::Instance().GetEffectLevel());
	//m_kChrMgr.SetTextTail(CPythonSystem::instance().GetDropItemLevel());
#endif
...
bool CPythonApplication::CreateDevice(int width, int height, int Windowed, int bit /* = 32*/, int frequency /* = 0*/)
{
	...
	case CGraphicDevice::CREATE_ENUM:
		...
		TraceError("CreateDevice: Enum failed");
		return false;
	case CGraphicDevice::CREATE_DETECT:
		SET_EXCEPTION(CREATE_NO_APPROPRIATE_DEVICE);
		TraceError("CreateDevice: Detect failed");
		return false;
...
{
	auto normalizedKey = ReplaceString(mapKey, "/", "_");

	auto it = gs_MapNames.find(normalizedKey);
	if (it == gs_MapNames.end())
	{
		TraceError("Unknown map key: %s", mapKey.c_str());
		return "dc";
...
void CPythonApplication::InitializeDiscordAPI()
{
    ...
    Discord_Initialize(DISCORD_APP_CLIENT_ID, &handlers, true, nullptr);

	m_nDiscordUpdateTimerID = SetTimer(nullptr, 0, 15000, DiscordUpdateRoutine);
}
void CPythonApplication::UpdateDiscordState(uint8_t stateType, uint8_t channelID, const std::string& mapKey, const std::string& name)
{
	...
	switch (stateType)
	{
		...
		{
			discordPresence.state = "Giri힊 Yap캇yor";
    		discordPresence.startTimestamp = m_nAppStartTimestamp;

			discordPresence.largeImageKey = "dc";
			discordPresence.largeImageText = "ASTRA2.NET";
		} break;
		case DISCORD_STATE_SELECT:
		{
			discordPresence.state = "Karakter Se챌iyor";
    		discordPresence.startTimestamp = m_nAppStartTimestamp;
		...
		{
			discordPresence.state = "Oyun Y체kleniyor";
    		discordPresence.startTimestamp = m_nAppStartTimestamp;
		...
		case DISCORD_STATE_GAME:
		{
			...
			char szState[128];
			// sprintf_s(szState, "At Game: %s", szMapName);
			sprintf_s(szState, "Oyunda");
			...
			discordPresence.largeImageText = szMapName;
  			...
  			discordPresence.smallImageKey = "dc";
			discordPresence.smallImageText = "ASTRA2.NET";
		} break;
		default:
...
bool CPythonApplication::SplitLineByStr(const std::string& str, const char* c_szDelimeter, CTokenVector* pstTokenVector)
{
	...
	do
	{
		...
		int endPos = str.find_first_of(c_szDelimeter, beginPos);
		basePos = endPos;

		pstTokenVector->emplace_back(str.substr(beginPos, endPos - beginPos));
	} while (basePos < str.length());
...
bool CPythonApplication::LoadLocaleQuest(const char* c_szFileName)
{
	...
	for (int i = 0; i < kTextFileLoader.GetLineCount(); ++i)
	{
		...
		m_locale_quest.insert(std::make_pair(idx, kTokenVector[1]));
	}

	return true;
}

std::string CPythonApplication::FindLocaleQuest(DWORD idx)
{
	std::string str = "";
...
std::string CPythonApplication::Convert_arg(const char* c_szFormat ...)
{
	char buf[256];
	va_list args;
...
bool CPythonApplication::LoadLocaleString(const char* c_szFileName)
{
	...
	for (int i = 0; i < kTextFileLoader.GetLineCount(); ++i)
	{
		...
		int idx = atoi(kTokenVector[0].c_str());
		m_locale_string.insert(std::make_pair(idx, kTokenVector[1]));
	}

	return true;
}

std::string CPythonApplication::FindLocaleString(DWORD idx)
{
	if (m_locale_string.count(idx))
		return m_locale_string.at(idx);
