...
class IAbstractApplication : public TAbstractSingleton<IAbstractApplication>
{
	...
	public:
		...
		virtual std::string FindLocaleQuest(DWORD idx) = 0;
		virtual std::string FindLocaleString(DWORD idx) = 0;
		virtual std::string Convert_arg(const char* c_szFormat ...) = 0;
		virtual bool SplitLineByStr(const std::string& str, const char* c_szDelimeter, CTokenVector* pstTokenVector) = 0;
#endif
};