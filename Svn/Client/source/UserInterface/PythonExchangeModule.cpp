...
{
	// TraceError("Yang: %llu", CPythonExchange::Instance().GetElkFromSelf());
	// return PyLong_FromUnsignedLongLong(CPythonExchange::Instance().GetElkFromSelf());
...
PyObject * exchangeGetElkFromTarget(PyObject * poSelf, PyObject * poArgs)
{
	...
	// return PyLong_FromUnsignedLongLong(CPythonExchange::Instance().GetElkFromTarget());
}
...
PyObject * exchangeGetItemChangeLook(PyObject * poTarget, PyObject * poArgs)
{
	int iPos;
	...
	return Py_BuildValue("i", CPythonExchange::Instance().GetItemChangeLook(iPos, bSelf));
}
#endif
...
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
		{ "GetItemChangeLook", 		exchangeGetItemChangeLook, 		METH_VARARGS},
#endif
		{NULL, NULL},
	};
