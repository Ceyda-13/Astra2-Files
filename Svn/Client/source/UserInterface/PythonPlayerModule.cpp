...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		char itemlink[256 + 64 + 12];
#else
		char itemlink[256 + 64];
#endif
...
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
		len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%x", pPlayerItem->changelook);
#endif

		for (int i = 0; i < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++i)
		{
			// if (pPlayerItem->aAttr[i].bType != 0) // @@correction007
			{
...
PyObject * playerIsValuableItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 2:
		if (!PyTuple_GetInteger(poArgs, 0, &SlotIndex.window_type))
			return Py_BuildException();
		if (!PyTuple_GetInteger(poArgs, 1, &SlotIndex.cell)) // @@correction009
			return Py_BuildException();
		break;
	default:
		return Py_BuildException();
...
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
PyObject * playerGetItemChangeLook(PyObject* poSelf, PyObject* poArgs)
{
	...
	switch (PyTuple_Size(poArgs))
	{
		case 1:
		{
			...
			DWORD dwChangeLook = rPlayer.GetItemChangeLook(TItemPos(INVENTORY, iSlotIndex));
			return Py_BuildValue("i", dwChangeLook);
		}
		case 2:
		{
			TItemPos Cell;
			...
			DWORD dwChangeLook = rPlayer.GetItemChangeLook(Cell);
			return Py_BuildValue("i", dwChangeLook);
		}
		default:
			return Py_BuildException();
...
void initPlayer()
{
...
#endif
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
		{ "GetItemChangeLook", 		playerGetItemChangeLook,			METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "SKILL_GRADE_SAGE_MASTER", CPythonPlayer::SKILL_SAGE_MASTER);
#endif
	...
	PyModule_AddIntConstant(poModule, "ITEM_SLOT_ONE_COUNT", c_ItemSlot_One_Count);

	PyModule_AddIntConstant(poModule, "INVENTORY_NORMAL", INVENTORY_NORMAL);
#ifdef ENABLE_EXTEND_INVEN_SYSTEM
	PyModule_AddIntConstant(poModule, "INVENTORY_UPGRADE", INVENTORY_UPGRADE);
	PyModule_AddIntConstant(poModule, "INVENTORY_BOOK", INVENTORY_BOOK);
	PyModule_AddIntConstant(poModule, "INVENTORY_STONE", INVENTORY_STONE);
	PyModule_AddIntConstant(poModule, "INVENTORY_ATTR", INVENTORY_ATTR);
	PyModule_AddIntConstant(poModule, "INVENTORY_GIFTBOX", INVENTORY_GIFTBOX);
	PyModule_AddIntConstant(poModule, "INVENTORY_MAX", INVENTORY_MAX);
#endif
