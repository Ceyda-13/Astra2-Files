...
int CPythonEventManager::RegisterEventSet(const char * c_szFileName)
{
	...
	// @@correction045 BEGIN
	SetTotalLineCount(pEventSet, strEventString);
...
bool CPythonEventManager::ProcessEventSet(TEventSet * pEventSet) // @@correction045 void -> bool
{
	if (pEventSet->isLock)
		return false;
...
void CPythonEventManager::SetFontColor(int iIndex, float fRed, float fGreen, float fBlue, float fAlpha)
{
	if (!CheckEventSetIndex(iIndex))
		return;
	...
	pEventSet->CurrentColor.r = fRed;
	pEventSet->CurrentColor.g = fGreen;
	pEventSet->CurrentColor.a = fBlue;
	pEventSet->CurrentColor.b = fAlpha;
}
...
void CPythonEventManager::SetTotalLineCount(TEventSet * pEventSet, std::string strEventString)
{

	std::string ads = "[ENTER]";
	int a = 0;
	for (size_t i = 0, tfind; (tfind = strEventString.find(ads, i)) != std::string::npos; i = tfind + 1)
		a++;

	pEventSet->iTotalLineCount = a;
}
// @@correction045 END
