    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_DRAGON_SOUL_REFINE:
				...
				break;
			case HEADER_GC_UNK_213: // @@correction006
				ret = RecvUnk213();
				break;
...
bool CPythonNetworkStream::RecvUnk213() // @@correction006
{
...
bool CPythonNetworkStream::RecvChatPacket()
{
	...
	std::string str(buf);
	auto fas = str.find("|<Boss>| ", 0);
	if (fas != std::string::npos)
		str = str.substr(9);

#ifdef ENABLE_MULTI_LANGUAGE
	IAbstractApplication& rkApp = IAbstractApplication::GetSingleton();

	/*if (str.find("|cffffc700|H", 0) != std::string::npos || str.find("|cfff1e6c0|H", 0) != std::string::npos)
	{
	...
	{
		size_t i, tfind;
		for (i = 0; (tfind = str.find("{{{", i)) != std::string::npos; i = tfind + 3)
			counts++;
	}
	...
	{
		size_t i, tfind;
		for (i = 0; (tfind = str.find("{{", i)) != std::string::npos; i = tfind + 2)
			counts++;
	}
	if (counts)
	{
		...
		for (int i = 0; i < counts; ++i)
		{
			...
			{
				std::string ss(rkApp.FindLocaleString(atoi(kBuffTokenVector[0].c_str())));
				...
				for (int i = 1; i < kBuffTokenVector.size(); ++i)
				{
					...
					{
						if (ss.find("%s", c) != std::string::npos)
							find3 = ss.find("%s", c);
					...
					if (ss.find("%d", c) == std::string::npos && ss.find("%s", c) == std::string::npos)
						ddd += ss.substr(c);
				}
				s += ddd;
			}
	...
	if (counts)
	{
		...
		for (int i = 0; i < counts; ++i)
		{
			...
			{
				std::string ss(rkApp.FindLocaleQuest(atoi(kBuffTokenVector[0].c_str())));
				std::string ddd = "";
				size_t c = 0, find3 = 0;
				for (int i = 1; i < kBuffTokenVector.size(); ++i)
				{
...
#ifdef ENABLE_MULTI_LANGUAGE
		if (str.find("[ENTER]", 0) != std::string::npos)
		{
			size_t i, tfind, a;
			for (i = 0, a = 0; (tfind = str.find("[ENTER]", i)) != std::string::npos; i = tfind + 7)
			{
				if (!a)
...
bool CPythonNetworkStream::RecvDeadPacket()
{
	...
	if (pkChrInstSel)
	{
		...
		{
			Tracenf("On MainActor"); // @@correction026
...
bool CPythonNetworkStream::RecvShopPacket()
{
	...
	switch (packet_shop.subheader)
	{
...
#ifdef ENABLE_OFFLINE_PRIVATE_SHOP
				CPythonShop::Instance().SetUnlockSlots(pShopStartPacket->bUnlockSlots);
#endif
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], XOR("StartShop"), Py_BuildValue(XOR("(i)"), dwVID));
			}
			break;
		...
		case SHOP_SUBHEADER_GC_END:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], XOR("EndShop"), Py_BuildValue(XOR("()")));
			__RefreshInventoryWindow();
			break;
		...
		case SHOP_SUBHEADER_GC_UPDATE_PRICE:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetShopSellingPrice", Py_BuildValue("(K)", *(uint64_t *)&vecBuffer[0]));
			break;
...
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
				CPythonExchange::Instance().SetItemChangeLook(iSlotIndex, exchange_packet.dwChangeLook, true);
#endif
			}
			else
			{
...
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
				CPythonExchange::Instance().SetItemChangeLook(iSlotIndex, exchange_packet.dwChangeLook, false);
#endif
			}
...
{
	assert(!"CPythonNetworkStream::RecvSkillLevel - Don't use this function"); // @@correction026
...
#endif
			if (
#ifndef ENABLE_VIEW_TARGET_PLAYER_HP
			pInstTarget->IsPC() || 
#endif
			pInstTarget->IsBuilding())
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "CloseTargetBoardIfDifferent", Py_BuildValue("(i)", TargetPacket.dwVID));
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Recv
uint64_t arr[4] = { 0x12561, 0x1616717, 0x1711614, 0x55645654 };

#ifdef ENABLE_ANTICHEAT
bool CPythonNetworkStream::SendAttackPacketFromThread(UINT uMotAttack, DWORD dwVIDVictim, DWORD dwLine, DWORD dwCrc)
#else
...
#endif
{
	...
	kPacketAtk.bCRCMagicCubeProcPiece = GetProcessCRCMagicCubePiece();
	kPacketAtk.bCRCMagicCubeFilePiece = GetProcessCRCMagicCubePiece();
/*#ifdef ENABLE_ANTICHEAT
	kPacketAtk.dwLine = dwLine;
	kPacketAtk.dwCrc = dwCrc;
#endif*/
	...
	switch(GuildPacket.subheader)
	{
		...
		case GUILD_SUBHEADER_GC_REMOVE:
		{
			...
			if (pMainInstance)
			{
				...
				CPythonApplication::Instance().UpdateDiscordState(
					DISCORD_STATE_GAME,
					m_channelID,
...
bool CPythonNetworkStream::SendGiveItemPacket(DWORD dwTargetVID, TItemPos ItemPos, int iItemCount)
{
	...
	GiveItemPacket.wItemCount = iItemCount;
...
bool CPythonNetworkStream::RecvShopSignPacket()
{
	...
	else
	{
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME],
			...
			Py_BuildValue("(isi)", p.dwVID, p.szSign, p.iTitle)
		);
#else
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME],
			"BINARY_PrivateShop_Appear",
...
bool CPythonNetworkStream::RecvNPCList()
{
	...
	for (int i = 0; i < kNPCPosition.count; ++i)
	{
		...
		if (NPCPosition.bType == CActorInstance::TYPE_STONE || NPCPosition.bType == 99)
			__GlobalPositionToLocalPosition(NPCPosition.x, NPCPosition.y);
#endif
...
{
	TGuildDiscordInfo kDiscordInfo;
...
bool CPythonNetworkStream::RecvOfflinePrivateShop()
{
	...
	switch(packet.bSubHeader)
	{
		...
		{
			auto& vInfo = CPythonOffflinePrivateShop::Instance().GetInfoVectorRef();
			while (packet.wSize)
			{
				TOfflinePrivateShopInfo* Info = new TOfflinePrivateShopInfo;
				memset(Info, 0, sizeof(TOfflinePrivateShopInfo));
				if (!Recv(sizeof(TOfflinePrivateShopInfo), Info))
					return false;

				vInfo.emplace_back(Info);

				packet.wSize -= sizeof(TOfflinePrivateShopInfo);
			}
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflinePrivateShopEditOpen", Py_BuildValue("()"));
		}
		break;
		case OFFLINE_PRIVATE_SHOP_ITEM_UPDATE:
		{
			while (packet.wSize)
			{
				TOfflinePrivateShopItemUpdate* Info = new TOfflinePrivateShopItemUpdate;
				memset(Info, 0, sizeof(TOfflinePrivateShopItemUpdate));
				if (!Recv(sizeof(TOfflinePrivateShopItemUpdate), Info))
					return false;

				auto* pItem = CPythonOffflinePrivateShop::Instance().GetOffflinePrivateShopItemData(Info->dwVid, Info->item.display_pos);
				if (pItem)
				{
					memset(pItem, 0, sizeof(*pItem));
					memcpy(pItem, static_cast<const void*>(&(Info->item)), sizeof(*pItem));
				}

				delete Info;
				Info = nullptr;
				packet.wSize -= sizeof(TOfflinePrivateShopItemUpdate);
			}
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_OfflinePrivateShopEditOpen", Py_BuildValue("()"));
		}
		break;
		...
		{
			TOfflinePrivateShopItemSearch* Info = new TOfflinePrivateShopItemSearch;
		...
		{
			uint32_t dwId = 0;
			uint8_t bTargetPos = 0;
			if (!Recv(sizeof(dwId), &dwId))
				return false;

			if (!Recv(sizeof(bTargetPos), &bTargetPos))
				return false;

			auto& vInfo = CPythonOffflinePrivateShop::Instance().GetSearchInfoVectorRef();
			for (auto& it = vInfo.begin(); it != vInfo.end();)
			{
				if ((*it)->dwVid == dwId && (*it)->item.display_pos == bTargetPos)
				{
					auto* ptr = *it;
					vInfo.erase(it);
					delete ptr;
		...
		{
			uint32_t dwId = 0;
			uint8_t bUnlockSlots = 0;
			if (!Recv(sizeof(dwId), &dwId))
				return false;

			if (!Recv(sizeof(bUnlockSlots), &bUnlockSlots))
				return false;
		...
		break;
		case OFFLINE_PRIVATE_SHOP_LOGS:
		{
			TOfflinePrivateShopLog* Info = new TOfflinePrivateShopLog;
			if (!Recv(sizeof(*Info), Info))
				return false;
			...
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshPShopLogs", Py_BuildValue("()"));
		}
		break;
		case OFFLINE_PRIVATE_SHOP_PRICE_PULL:
		{
			uint32_t dwId = 0;
			uint64_t dwPrices = 0;
			if (!Recv(sizeof(dwId), &dwId))
			...
			if (!Recv(sizeof(dwPrices), &dwPrices))
				return false;
...
bool CPythonNetworkStream::RecvCharacterGold()
{
	TPacketGold pack;
	if (!Recv(sizeof(pack), &pack))
	{
		TraceError("RecvCharacterGold ERROR");
		return false;
	}
	uint64_t amount = pack.gold - CPythonPlayer::Instance().GetGold();
#ifdef ENABLE_YANG_INFO
	if (CPythonSystem::Instance().GetYangInfo())
#endif
	if (amount && !pack.bIsLogin)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnPickMoney", Py_BuildValue("(K)", amount));
	}
	CPythonPlayer::Instance().SetGold(pack.gold);
	//TraceError("Gold %lld",chrInfoPacket.gold);
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshStatus", Py_BuildValue("()"));
	return true;
}
...
bool CPythonNetworkStream::RecvBossTracking()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BOSS_TRACKING", Py_BuildValue("(iiiii)", p.dead_time, p.regen_time, p.channel, p.mob_vnum, p.map_index));
	return true;
}
#endif
