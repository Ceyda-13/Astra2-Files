...
#if defined (ENABLE_GUILD_GENERAL_AND_LEADER) || defined(WJ_SHOW_MOB_INFO)
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pLanguageInstance)
		{
			...
			if (pLevel)
			{
				...
				{
					pTitle->GetTextSize(&iTitleWidth, &iTitleHeight);
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - 8.0f - iTitleWidth - iLevelWidth - pLanguageInstance->GetWidth() - 1.0f, pTextTail->y - 10);
				}
				else
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - 4.0f - iLevelWidth - pLanguageInstance->GetWidth() - 1.0f, pTextTail->y - 10);
			}
		}
#endif
...
DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
...
#ifdef ENABLE_GUILD_GENERAL_AND_LEADER
		if (bNewIsGuildName > 1)
		{
			CPythonNonPlayer& rkNonPlayer = CPythonNonPlayer::Instance();
			std::string str("[");
			if (bNewIsGuildName == 3)
				str += rkNonPlayer.GetMonsterName(2491);
			else if (bNewIsGuildName == 2)
				str += rkNonPlayer.GetMonsterName(2492);

			str += "] - ";
			strGuildName.insert(0, str.c_str());
		}
#endif
...
#ifdef WJ_SHOW_MOB_INFO
	if (pCharacterInstance->GetAIFlag() & CPythonNonPlayer::AIFLAG_AGGRESSIVE)
	{
	...
	if (pCharacterInstance->IsPC())
	{
		...
		char szPath[256];
		sprintf(szPath, "multi/%s.tga", pCharacterInstance->GetLanguage());
		pTextTail->pLanguageInstance->SetImagePointer((CGraphicImage*)CResourceManager::Instance().GetResourcePointer(szPath));
	}
#endif
