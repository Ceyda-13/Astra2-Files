...
bool CPythonPlayer::__UseSkill(DWORD dwSlotIndex)
{
	...
	if (pSkillData->CanChangeDirection())
	{
		...
		{
			Tracenf("CPythonPlayer::__UseSkill(%d) - The screen direction settings should be as standard", dwSlotIndex); // @@correction026
		}
	}
...
bool CPythonPlayer::__HasEnoughArrow()
{
 ...
 || CItemData::WEAPON_QUIVER == pItemData->GetSubType()
#endif
)
	{
		return true;
