...
CInstanceBase* CPythonCharacterManager::GetTabNextTargetPointer(CInstanceBase* pkInstMain)
{
	...
	struct FCharacterManagerInstanceTarget
	{
		...
		{
			CInstanceBase* pkInstTarget = itor.second;
			if (!pkInstTarget || pkInstTarget == pkInstMain || !pkInstTarget->IsEnemy())
				return;

			const float fRadiusDistance = pkInstMain->GetDistance(pkInstTarget);
	...
	const size_t kTargetCount = f.m_vecTargetInstance.size();
	if (kTargetCount == 0)
	{
		ResetTabNextTargetVectorIndex();
		return NULL;
...
float fDistances[] = {100000000.0f, 12000000.0f, 4500000.0f, 1500000.0f, 250000.0f };
void CPythonCharacterManager::SetPrivateShopLevel(unsigned int level)
{
	CInstanceBase* pMainInstance = GetMainInstancePtr();
	for (auto& it : m_kAliveInstMap)
	{
		CInstanceBase* pInstance = it.second;
		if (pInstance && pMainInstance && pMainInstance != pInstance)
		{
			if (pInstance->GetRace() >= 30000 && pInstance->GetRace() <= 30007)
			{
				const TPixelPosition& pos = pInstance->GetGraphicThingInstancePtr()->NEW_GetCurPixelPositionRef();
				float fDist = pMainInstance->CalculateDistanceSq3d(pos);
				if (fDistances[level] < fDist)
				{
					if (pInstance->IsShow())
					{
						pInstance->SetShow(false);
						pInstance->Hide();
					}

					if (pInstance->m_isTextTail)
...
void CPythonCharacterManager::SetEffectLevel(unsigned int level)
{
	CInstanceBase* pMainInstance = GetMainInstancePtr();
	for (auto& it : m_kAliveInstMap)
	{
		...
		if (pInstance && pMainInstance)
		{
			if (level == 0)
			{
				pInstance->GetGraphicThingInstancePtr()->ShowAllAttachingEffect();
				// pInstance->__EffectContainer_Continue();
				pInstance->GetGraphicThingInstancePtr()->SetActiveAllAttachingEffect();
				continue;
			}

			if (level == 1)
			{
				if (pMainInstance != pInstance && !pInstance->IsEnemy())
				{
					pInstance->GetGraphicThingInstancePtr()->HideAllAttachingEffect();
					pInstance->__EffectContainer_Suspend();
					pInstance->GetGraphicThingInstancePtr()->SetDeactiveAllAttachingEffect();
					continue;
				}
				else
				{
...
void CPythonCharacterManager::SetPetStatus(bool bStatus)
{
	CInstanceBase* pMainInstance = GetMainInstancePtr();
	...
	{
		CInstanceBase* pInstance = it.second;
		if (pInstance && pMainInstance && pMainInstance != pInstance && (/*pInstance->IsPet()*/
#ifdef ENABLE_GROWTH_PET_SYSTEM
/*			|| */pInstance->IsNewPet()
#endif
			))
...
void CPythonCharacterManager::SetNpcNameStatus(bool bStatus)
{
	CInstanceBase* pMainInstance = GetMainInstancePtr();
	...
	{
		CInstanceBase* pInstance = it.second;
		if (pInstance && pMainInstance && pMainInstance != pInstance && pInstance->IsNPC())
		{
			if (bStatus)
			{
				if (pInstance->m_isTextTail)
