...
uint64_t arr2[4] = { 0x141, 0x116, 0x16614, 0x5561654 };
...
//		return;
	...
	if (pkInstTarget->GetInstanceType() == CActorInstance::TYPE_NPC && pkInstTarget->GetRace() == 3)
		return;
#endif
...
void CPythonPlayer::__OnPressItem(CInstanceBase& rkInstMain, DWORD dwPickedItemID)
{
	...
	{
		Trace("CPythonPlayer::__OnPressItem - ALREADY PICKED ITEM");
		return;
	}
...
bool CPythonPlayer::__CanAttack()
{
	...
	if (pkInstMain->IsMountingHorse() && (GetSkillGrade(109) < 1 && GetSkillLevel(109) < 11)) // @@correction019
	{
		return false;
	}
