...
{
	assert(!"CPythonNetworkStream::SendSafeBoxMoneyPacket - Don't use this function"); // @@correction026
	return false;
...
bool CPythonNetworkStream::RecvSafeBoxSetPacket()
{
	...
	kItemData.changelook = kItemSet.changelook;
#endif
...
#endif
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
	kItemData.changelook = packet_item_set.changelook;
#endif
	IAbstractPlayer& rkPlayer=IAbstractPlayer::GetSingleton();
	rkPlayer.SetItemData(packet_item_set.Cell, kItemData);
...
#endif
#ifdef ENABLE_CHANGE_LOOK_SYSTEM
	rkPlayer.SetItemChangeLook(packet_item_update.Cell, packet_item_update.changelook);
#endif
	for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
...
bool CPythonNetworkStream::SendItemDestroyPacket(TItemPos pos, BYTE type)
{
	if (!__CanActMainInstance())
		return true;
...
#endif
#ifdef ENABLE_12ZI
		case SE_EFFECT_SKILL:
			effect = CInstanceBase::EFFECT_DEAPO_BOOM;
			break;
#endif
		default:
			TraceError("%d is not a special effect number. TPacketGCSpecialEffect",kSpecialEffect.type); // @@correction026
			break;
	}
