...
#ifdef ENABLE_12ZI
	if (GetRace() >= 2750 && GetRace() <= 2862 && IsUsingSkill() && !bIsUsingShelter && m_fcurRotation == m_rotEnd)
		AttachShelter(dwMotionIndex - CRaceMotionData::NAME_SPECIAL_1);
#endif
...
void CActorInstance::SetLoopMotion(DWORD dwMotion, float fBlendTime, float fSpeedRatio)
{
	if (!m_pkCurRaceData)
	{
		Tracenf("CActorInstance::SetLoopMotion(dwRace=%d, dwMotion=%d, fBlendTime=%f, fSpeedRatio=%f)", GetRace(),
	...
	{
		Tracenf("CActorInstance::SetLoopMotion(dwRace=%d, dwMotion=%d, fBlendTime=%f, fSpeedRatio=%f) - GetMotionKey(m_wcurMotionMode=%d, dwMotion=%d, &MotionKey) ERROR",
			GetRace(), dwMotion, fBlendTime, fSpeedRatio, m_wcurMotionMode, dwMotion);
		return;
	}
...
DWORD CActorInstance::__SetMotion(const SSetMotionData& c_rkSetMotData, DWORD dwRandMotKey)
{
	...
	if (__IsMoveMotion())
	{
		if (uNextMot==CRaceMotionData::NAME_DAMAGE || uNextMot==CRaceMotionData::NAME_DAMAGE_BACK || uNextMot==CRaceMotionData::NAME_DAMAGE_FLYING || uNextMot==CRaceMotionData::NAME_DAMAGE_FLYING_BACK)
		{
			if (!m_isMain)
			{
				Trace("Only MainActor can receive damage when moving"); // @@correction026
				return false;
			}
		}
