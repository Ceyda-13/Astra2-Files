...
bool CActorInstance::IsBowMode()
{
	...
	if (CRaceMotionData::MODE_BOW_SPECIAL == GetMotionMode())
		return true;

	if (CRaceMotionData::MODE_HORSE_BOW_SPECIAL == GetMotionMode())
		return true;
#endif
...
{
	if (TYPE_NPC == m_eActorType && GetRace() == 3)
		return true;
...
{
	if (TYPE_NPC == m_eActorType && (m_eRace >= 34001 && m_eRace <= 34999) || (m_eRace >= 20523 && m_eRace <= 20529))
		return true;
