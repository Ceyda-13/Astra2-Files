...
#ifdef ENABLE_12ZI
std::string GetEffectString(DWORD dwRace)
{
	switch (dwRace)
	{
	case 0:
		return "d:/ymir work/effect/monster2/12_shelter_in_01.mse";
	case 1:
		return "d:/ymir work/effect/monster2/12_shelter_in_02.mse";
	case 2770:
	case 2771:
	case 2772:
		return "d:/ymir work/effect/monster2/12_tiger_s3_drop.mse";
	case 2790:
	case 2791:
	case 2792:
		return "d:/ymir work/effect/monster2/12_dra_s2_drop.mse";
	case 2830:
	case 2831:
	case 2832:
		return "d:/ymir work/effect/monster2/12_mon_s3_drop.mse";
	case 2800:
	case 2801:
	case 2802:
		return "d:/ymir work/effect/monster2/12_sna_s3_drop.mse";
	case 2810:
	case 2811:
	case 2812:
		return "d:/ymir work/effect/monster2/12_hor_s3_drop.mse";
	case 2840:
	case 2841:
	case 2842:
		return "d:/ymir work/effect/monster2/12_chi_s3_drop.mse";
...
float fSpecialSkillPos[4][2] =
{
	{
		700.0 + (float)(rand() % 301),
		500.0 + (float)(rand() % 301),
	},
	{
		-(700.0 + (float)(rand() % 301)),
		500.0 + (float)(rand() % 301),
...
void CActorInstance::AttachShelter(BYTE bType)
{
	if (!m_pkCurRaceMotionData)
		return;
	...
	if (GetRace() >= 2770 && GetRace() <= 2772 && bType == 2)
		count++;
	else if (GetRace() >= 2790 && GetRace() <= 2792 && bType == 1)
		count++;
	else if (GetRace() >= 2830 && GetRace() <= 2832 && bType == 2)
		count++;
	else if (GetRace() >= 2800 && GetRace() <= 2802 && bType == 2)
		count++;
	else if (GetRace() >= 2810 && GetRace() <= 2812 && bType == 2)
		count++;
	else if (GetRace() >= 2840 && GetRace() <= 2842 && bType == 2)
		count++;

	memset(fSpecialPos, 0.0f, sizeof(fSpecialPos));
	bIsHaveSpecial = false;

	for (DWORD a = 0, step = 0; a < m_pkCurRaceMotionData->GetMotionEventDataCount(); ++a)
	{
		const CRaceMotionData::TMotionEventData* c_pData;
		if (!m_pkCurRaceMotionData->GetMotionEventDataPointer(a, &c_pData))
			continue;

		if (c_pData->iType == CRaceMotionData::MOTION_EVENT_TYPE_SPECIAL_ATTACKING)
		{
			const CRaceMotionData::TMotionAttackingEventData* c_pAttackingData = (const CRaceMotionData::TMotionAttackingEventData*)c_pData;
			for (DWORD i = 0; i < c_pAttackingData->CollisionData.SphereDataVector.size(); ++i)
			{
				const TSphereData& c_rSphereData = c_pAttackingData->CollisionData.SphereDataVector[i].GetAttribute();
				float fX = c_rSphereData.v3Position.x;
				float fY = c_rSphereData.v3Position.y;
				if (count == 2)
				{
					fX = fSpecialSkillPos[step][0];
					fY = fSpecialSkillPos[step][1];
					fSpecialPos[step][0] = fX;
					fSpecialPos[step][1] = fY;
					bIsHaveSpecial = true;
				}
				D3DXVECTOR3 v3pos = D3DXVECTOR3(fX, fY, 0.0f);

				CEffectManager& rkEftMgr = CEffectManager::Instance();
				TAttachingEffect ae;
				ae.dwEffectIndex = rkEftMgr.GetEmptyIndex();
				if (v3pos)
					D3DXMatrixTranslation(&ae.matTranslation, v3pos.x, v3pos.y, v3pos.z);
				else
					D3DXMatrixIdentity(&ae.matTranslation);

				rkEftMgr.CreateShelterEffectInstance(ae.dwEffectIndex, GetEffectString(0).c_str(), c_rSphereData.fRadius);
				rkEftMgr.SelectEffectInstance(ae.dwEffectIndex);
				D3DXMATRIX matTransform = ae.matTranslation;
				matTransform *= m_worldMatrix;
				rkEftMgr.SetEffectInstanceGlobalMatrix(matTransform);
				if (count == 2)
					SpecialMatrix[step] = matTransform;
			}
			step++;
		}
	}
	bIsUsingShelter = true;
}

void CActorInstance::AttachSpecial()
{
	for (int i = 0; i < 4; ++i)
	{
		if (fSpecialPos[i][0] == 0.0f && fSpecialPos[i][1] == 0.0f)
			continue;

		CEffectManager& rkEftMgr = CEffectManager::Instance();
		TAttachingEffect ae;
		ae.dwEffectIndex = rkEftMgr.GetEmptyIndex();
		D3DXVECTOR3 v3pos = D3DXVECTOR3(fSpecialPos[i][0], fSpecialPos[i][1], 0.0f);
		if (v3pos)
			D3DXMatrixTranslation(&ae.matTranslation, v3pos.x, v3pos.y, v3pos.z);
		else
			D3DXMatrixIdentity(&ae.matTranslation);

		rkEftMgr.CreateShelterEffectInstance(ae.dwEffectIndex, GetEffectString(GetRace()).c_str(), 0);
		rkEftMgr.SelectEffectInstance(ae.dwEffectIndex);
		rkEftMgr.SetEffectInstanceGlobalMatrix(SpecialMatrix[i]);
	}
	bIsHaveSpecial = false;
	memset(fSpecialPos, 0.0f, sizeof(fSpecialPos));
}
