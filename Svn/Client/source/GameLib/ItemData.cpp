...
void CItemData::__SetIconImage(const char * c_szFileName)
{
	...
	{
		TraceError("%s not found. CItemData::__SetIconImage",c_szFileName); // @@correction026
		m_pIconImage = NULL;
#ifdef ENABLE_LOAD_ALTER_ITEMICON // @@correction024
...
const char* CItemData::GetUseTypeString() const
{
	...
	switch (GetSubType())
	{
		...
		case USE_ADD_COSTUME_ATTRIBUTE:
			return DEF_STR(USE_ADD_COSTUME_ATTRIBUTE);
