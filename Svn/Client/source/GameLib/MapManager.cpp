...
bool CMapManager::UnloadMap(const std::string c_strMapName)
{
	...
	{
		LogBoxf(XOR("%s: Unload Map Failed"), c_strMapName.c_str());
		return false;
	}
...
float CMapManager::GetHeight(float fx, float fy)
{
	if (!m_pkMap)
	{
		TraceError("CMapManager::GetHeight(%f, %f) - Accessing in a not initialized map", fx, fy); // @@correction026
		return 0.0f;
	}
	CMapOutdoor& rkMap=GetMapOutdoorRef();
...
float CMapManager::GetTerrainHeight(float fx, float fy)
{
	if (!m_pkMap)
	{
		TraceError("CMapManager::GetTerrainHeight(%f, %f) - Accessing in a not initialized map", fx, fy); // @@correction026
		return 0.0f;
	}
	CMapOutdoor& rkMap=GetMapOutdoorRef();
...
bool CMapManager::GetWaterHeight(int iX, int iY, long * plWaterHeight)
{
	if (!m_pkMap)
	{
		TraceError("CMapManager::GetTerrainHeight(%f, %f) - Accessing in a not initialized map", iX, iY); // @@correction026
		return false;
	}
...
void CMapManager::SetFogMode(bool flag)
{
	if (mc_pcurEnvironmentData)
	{
