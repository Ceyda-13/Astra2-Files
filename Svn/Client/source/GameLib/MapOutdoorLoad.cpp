...
#ifdef WORLD_EDITOR // @@correction011
...
bool CMapOutdoor::LoadSetting(const char * c_szFileName)
{
 	...
 	if (!m_TextureSet.Load(stTextureSetFileName.c_str(), m_fTerrainTexCoordBase))
	{
		...
		LogBoxf(XOR("TextureSet not successfully applied to [%s].\nTexture not found [%s]."), c_szFileName, stTextureSetFileName.c_str()); // @@correction026
#else
