...
void CGrannyModelInstance::UpdateTransform(D3DXMATRIX * pMatrix, float fSecondsElapsed)
{
	...
	GrannyUpdateModelMatrix(m_pgrnModelInstance, fSecondsElapsed, (const float *) pMatrix, (float *) pMatrix, false); // @@correction020
...
void CGrannyModelInstance::UpdateWorldMatrices(const D3DXMATRIX* c_pWorldMatrix)
{
	...
	for (int i = 0; i < meshCount; ++i)
	{
		...
		// WORK
		const granny_int32x * boneIndices = __GetMeshBoneIndices(i); // @@correction020
		// END_OF_WORK
		...
		{
			const granny_int32x iBone = *boneIndices; // @@correction020
