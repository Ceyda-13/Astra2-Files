...
void CFishEventGridSlotWindow::OnRenderPickingSlot()
{
	if (!UI::CWindowManager::Instance().IsAttaching())
		return;
	...
	if (GetPickedSlotList(byWidth, byHeight, &SlotList))
	{
		...
		int pos = 0;
		if (dwAttachedType == SLOT_TYPE_FISH_EVENT && (dwAttachedIndex == FISH_EVENT_SHAPE_3 || dwAttachedIndex == FISH_EVENT_SHAPE_4 || dwAttachedIndex == FISH_EVENT_SHAPE_6))
			for (auto it = SlotListForCheck.begin(); it != SlotListForCheck.end(); ++it, ++pos)
				if ((dwAttachedIndex == FISH_EVENT_SHAPE_6 && (pos == 1 || pos == 3)) || (dwAttachedIndex == FISH_EVENT_SHAPE_3 && pos == 2) || (dwAttachedIndex == FISH_EVENT_SHAPE_4 && pos == 1))
		...
		if (dwAttachedType == SLOT_TYPE_FISH_EVENT && (dwAttachedIndex == FISH_EVENT_SHAPE_3 || dwAttachedIndex == FISH_EVENT_SHAPE_4 || dwAttachedIndex == FISH_EVENT_SHAPE_6))
		{
			...
			Rect.right = max(Rect.right, m_rect.left + pSlotFish->ixPosition + (dwAttachedIndex == FISH_EVENT_SHAPE_3 ? 32 : 64));
			Rect.bottom = max(Rect.bottom, m_rect.top + pSlotFish->iyPosition + 32);
			
			SecondRect.left = min(SecondRect.left, m_rect.left + pSlotFish->ixPosition + (dwAttachedIndex == FISH_EVENT_SHAPE_3 ? 0 : 32));
			SecondRect.top = min(SecondRect.top, m_rect.top + pSlotFish->iyPosition + 32);
			SecondRect.right = max(SecondRect.right, m_rect.left + pSlotFish->ixPosition + (dwAttachedIndex == FISH_EVENT_SHAPE_6 ? 96 : 64));
...
BOOL CFishEventGridSlotWindow::GetPickedSlotPointer(TSlot ** ppSlot)
{
	if (!UI::CWindowManager::Instance().IsAttaching())
		return CSlotWindow::GetPickedSlotPointer(ppSlot);
...
BOOL CFishEventGridSlotWindow::GetPickedSlotList(int iWidth, int iHeight, std::list<TSlot*> * pSlotPointerList)
{
	long lx, ly;
	GetMouseLocalPosition(lx, ly);
...
DWORD CFishEventGridSlotWindow::Type()
{
	static int s_Type = GetCRC32("CFishEventGridSlotWindow", strlen("CFishEventGridSlotWindow"));
	return s_Type;
}

BOOL CFishEventGridSlotWindow::OnIsType(DWORD dwType)
{
	if (CFishEventGridSlotWindow::Type() == dwType)
		return TRUE;

	return CGridSlotWindow::OnIsType(dwType);
}

CFishEventGridSlotWindow::CFishEventGridSlotWindow(PyObject * ppyObject) : CGridSlotWindow(ppyObject)
{

}

CFishEventGridSlotWindow::~CFishEventGridSlotWindow()
{

}

void CFishEventGridSlotWindow::Destroy()
{
	CGridSlotWindow::Destroy();
	__Initialize();
}

void CFishEventGridSlotWindow::__Initialize()
{
	CGridSlotWindow::__Initialize();
	bIsPickedAreaRender = true;
}
#endif
