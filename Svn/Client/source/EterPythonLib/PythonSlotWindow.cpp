...
void CSlotWindow::AppendSlot(DWORD dwIndex, int ixPosition, int iyPosition, int ixCellSize, int iyCellSize)
{
	...
	Slot.pActiveSlotEffect = nullptr;
	ClearSlot(&Slot);
...
void CSlotWindow::ActivateSlot(DWORD dwIndex, float r, float g, float b, float a)
{
	TSlot * pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
		return;
	...
	if (!pSlot->pActiveSlotEffect)
	{
		...
		pSlot->pActiveSlotEffect = new CAniImageBox(NULL);
		for (int j = 0; j < 13; ++j)
		{
			char cBuf[72];
			sprintf_s(cBuf, "d:/ymir work/ui/public/slotactiveeffect/slot%d/%02d.sub", pSlot->byyPlacedItemSize, j);
			pSlot->pActiveSlotEffect->AppendImage(cBuf, r, g, b, a);
		}
	}

	pSlot->pActiveSlotEffect->SetRenderingMode(CGraphicExpandedImageInstance::RENDERING_MODE_SCREEN);
	pSlot->pActiveSlotEffect->Show();
}
...
void CSlotWindow::OnUpdate()
{
	for (std::deque<DWORD>::iterator itor = m_ReserveDestroyEffectDeque.begin(); itor != m_ReserveDestroyEffectDeque.end(); ++itor)
	{
		...
		{
			delete pSlot->pActiveSlotEffect;
			pSlot->pActiveSlotEffect = nullptr;
		}
	}
...
void CSlotWindow::OnRender()
{
	...
	for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (rSlot.bActive && rSlot.pActiveSlotEffect)
		{
			rSlot.pActiveSlotEffect->SetPosition(m_rect.left + rSlot.ixPosition, m_rect.top + rSlot.iyPosition);
...
void CSlotWindow::GetSlotGlobalPosition(int iIndex, int * pix, int * piy)
{
	TSlot * pSlot;
	if (!GetSlotPointer(iIndex, &pSlot))
		return;

	*pix = pSlot->ixPosition;
	*piy = pSlot->iyPosition;
}
...
void CSlotWindow::__CreateFinishCoolTimeEffect(TSlot * pSlot)
{
	...
	{
		char cBuf[72];
		sprintf_s(cBuf, "d:/ymir work/ui/public/slotfinishcooltimeeffect/%02d.sub", j);
		pFinishCoolTimeEffect->AppendImage(cBuf);
	}
