...
namespace UI
{
	...
	void CWindowManager::Destroy()
	{
		...
		{
			CWindow* pkWnd=*i;
			_TraceError("CWindowManager::Destroy LOST WINDOW %s\n", pkWnd->GetName());
		}
#endif
	...
	CWindow *	CWindowManager::__NewWindow(PyObject * po, DWORD dwWndType)
	{
		switch(dwWndType)
		{
			...
			case WT_SP_GRIDSLOT:
				return new CFishEventGridSlotWindow(po);
				break;
#endif
			default:
...
#ifdef ENABLE_FISH_EVENT
	CWindow * CWindowManager::RegisterFishEventGridSlotWindow(PyObject * po, const char * c_szLayer)
	{
		assert(m_LayerWindowMap.end() != m_LayerWindowMap.find(c_szLayer));

		CWindow * pWin = new CFishEventGridSlotWindow(po);
		m_LayerWindowMap[c_szLayer]->AddChild(pWin);
	...
	void CWindowManager::DestroyWindow(CWindow * pWin)
	{
		...
		pWin->Clear();
		auto it = std::find(m_ReserveDeleteWindowList.begin(), m_ReserveDeleteWindowList.end(), pWin);
		if (it == m_ReserveDeleteWindowList.end())
