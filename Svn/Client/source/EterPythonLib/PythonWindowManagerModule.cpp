...
{
	assert(!"wndMgrSetToggleSlot - Don't use such function"); // @@correction026
	return Py_BuildNone();
}
...
PyObject * wndMgrActivateSlot(PyObject * poSelf, PyObject * poArgs)
{
	...
	UI::CSlotWindow * pSlotWin = (UI::CSlotWindow *)pWin;
	pSlotWin->ActivateSlot(iSlotIndex, r, g, b, a);
	return Py_BuildNone();
}
...
PyObject * wndLeftRightReverse(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWindow;
	if (!PyTuple_GetWindow(poArgs, 0, &pWindow))
		return Py_BuildException();
...
PyObject * wndMgrSetRenderTarget(PyObject * poSelf, PyObject * poArgs)
{
	...
	int iNumber;
	if (!PyTuple_GetInteger(poArgs, 1, &iNumber))
		return Py_BuildException();

	if (pWnd->IsType(UI::CRenderTarget::Type()))
		((UI::CRenderTarget *)pWnd)->SetRenderTarget(iNumber);

	return Py_BuildNone();
}

PyObject * wndMgrSetRenderTargetData(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();

	int iRace, iArmor, iWeapon, iHair, iAcce, iMotionMode;
	if (!PyTuple_GetInteger(poArgs, 1, &iRace))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 2, &iArmor))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 3, &iWeapon))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 4, &iHair))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 5, &iAcce))
		return Py_BuildException();

	if (!PyTuple_GetInteger(poArgs, 6, &iMotionMode))
		return Py_BuildException();

	if (!pWnd->IsType(UI::CRenderTarget::Type()))
		return Py_BuildException();

	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->SetRenderTargetData(iRace, iArmor, iWeapon, iHair, iAcce, iMotionMode);
	return Py_BuildNone();
}

PyObject * wndMgrSetAutoRotation(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->SetAutoRotation(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrShowInstance(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->ShowInstance();
	return Py_BuildNone();
}

PyObject* wndMgrHideInstance(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->HideInstance();
	return Py_BuildNone();
}

PyObject* wndMgrRotateLeft(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->RotateLeft(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrRotateRight(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->RotateRight(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrRotateUp(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->RotateUp(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrRotateDown(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->RotateDown(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrZoomIn(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->ZoomIn(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrZoomOut(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->ZoomOut(fValue);
	return Py_BuildNone();
}

PyObject* wndMgrRestoreStartPosition(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	pRenderWnd->RestoreStartPosition();
	return Py_BuildNone();
}

PyObject* wndMgrGetInstanceVID(PyObject * poSelf, PyObject * poArgs)
{
	UI::CWindow * pWnd;
	if (!PyTuple_GetWindow(poArgs, 0, &pWnd))
		return Py_BuildException();
	...
	UI::CRenderTarget * pRenderWnd = (UI::CRenderTarget *)pWnd;
	return Py_BuildValue("i", pRenderWnd->GetInstanceVID());
}
#endif

#ifdef ENABLE_FISH_EVENT
PyObject * wndMgrRegisterFishEventGridSlotWindow(PyObject * poSelf, PyObject * poArgs)
{
	PyObject * po;
	if (!PyTuple_GetObject(poArgs, 0, &po))
		return Py_BuildException();
	...
	UI::CWindow * pWindow = UI::CWindowManager::Instance().RegisterFishEventGridSlotWindow(po, szLayer);
	return Py_BuildValue("i", pWindow);
}
...
PyObject * wndMgrSetPickedAreaRender(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!pWin->IsType(UI::CFishEventGridSlotWindow::Type()))
	{
		TraceError("wndMgr.SetPickedAreaRender : not a grid window");
		return Py_BuildException();
	}

	UI::CFishEventGridSlotWindow * pFishEventGridSlotWin = (UI::CFishEventGridSlotWindow *)pWin;
	pFishEventGridSlotWin->SetPickedAreaRender(iFlag ? true : false);
...
void initwndMgr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "Over",						wndButtonOver,						METH_VARARGS }, // @@correction046
		...
		{ "SetRenderTargetData",		wndMgrSetRenderTargetData,			METH_VARARGS },
		{ "SetAutoRotation",			wndMgrSetAutoRotation,				METH_VARARGS },
		{ "ShowInstance",				wndMgrShowInstance, 				METH_VARARGS },
		{ "HideInstance",				wndMgrHideInstance,					METH_VARARGS },
		{ "RotateLeft",					wndMgrRotateLeft,					METH_VARARGS },
		{ "RotateRight",				wndMgrRotateRight,					METH_VARARGS },
		{ "RotateUp",					wndMgrRotateUp,						METH_VARARGS },
		{ "RotateDown",					wndMgrRotateDown,					METH_VARARGS },
		{ "ZoomIn", 					wndMgrZoomIn,						METH_VARARGS },
		{ "ZoomOut", 					wndMgrZoomOut,						METH_VARARGS },
		{ "RestoreStartPosition", 		wndMgrRestoreStartPosition, 		METH_VARARGS },
		{ "GetInstanceVID",				wndMgrGetInstanceVID,				METH_VARARGS },
#endif
		...
		{ "ButtonSetScale",				wndButtonSetScale,					METH_VARARGS },
		{ "LeftRightReverse",			wndLeftRightReverse,				METH_VARARGS },
		...
		{ "RegisterFishEventGridSlotWindow",	wndMgrRegisterFishEventGridSlotWindow,			METH_VARARGS },
#endif
		{ NULL,							NULL,								NULL },
	};
