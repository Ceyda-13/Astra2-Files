...
namespace UI
{
	...
	void CNumberLine::SetNumber(const char * c_szNumber)
	{
		...
		for (DWORD i = 0; i < m_strNumber.size(); ++i)
		{
			...
			{
				strImageFileName = m_strPath + "s.sub";
			}
#endif
	...
	void CRenderTarget::SetRenderTarget(DWORD dwNumber)
	{
		...
		success = SUCCEEDED(ms_lpd3dDevice->CreateDepthStencilSurface(GetWidth(), GetHeight(), D3DFMT_D16, D3DMULTISAMPLE_NONE, &m_DepthSurface)); // dx9 0, TRUE, &m_DepthSurface, NULL
		if (!success)
		{
	...
	void CRenderTarget::OnRender()
	{
		...
		//STATEMANAGER.SetFVF(D3DFVF_XYZ | D3DFVF_TEX1 | D3DFVF_DIFFUSE); dx9
		...
		//ms_lpd3dDevice->GetRenderTarget(0, &m_PreviousRenderTarget); dx9
		ms_lpd3dDevice->GetRenderTarget(&m_PreviousRenderTarget);
		/*ms_lpd3dDevice->SetDepthStencilSurface(m_DepthSurface); dx9
		...
		pyGraphic.SetOrtho2D(m_BackGroundImageInstance->GetWidth(), m_BackGroundImageInstance->GetHeight(), 1.0f); // BackGround Kayma sorunu fix
		...
		/*ms_lpd3dDevice->SetDepthStencilSurface(m_PreviousDepthBuffer); dx9
		...
		// m_InstanceModel->GetGraphicThingInstanceRef().SetScale(xScaleFactor, yScaleFactor, zScaleFactor, true);
		...
		ms_matView = CGraphicBase::GetIdentityMatrix();//s_pushedViewMatrix;
	}
#endif
};
