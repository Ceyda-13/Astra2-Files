...
bool NSound::LoadSoundInformationPiece(const char * c_szFileName, NSound::TSoundDataVector & rSoundDataVector, const char * c_szPathHeader)
{
	...
	{
		SetResultString((strResult + " Can not open file for reading").c_str()); // @@correction026
		return false;
	}
	...
	{
		SetResultString((strResult + " File format error, SoundDataCount Unable to find.").c_str()); // @@correction026
		return false;
	}
	...
	for (DWORD i = 0; i < rSoundDataVector.size(); ++i)
	{
		...
		{
			SetResultString((strResult + " File format error: " + szSoundDataHeader + " Unable to find").c_str()); // @@correction026
			return false;
		}

		if (2 != pTokenVector->size())
		{
			SetResultString((strResult + " File format error: The size of the vector is not 2").c_str()); // @@correction026
			return false;
		}
	...
	SetResultString((strResult + " Loaded").c_str()); // @@correction026
	return true;
}
...
bool NSound::SaveSoundInformationPiece(const char * c_szFileName, NSound::TSoundDataVector & rSoundDataVector)
{
	...
	if (!File)
	{
		...
		LogBox(szErrorText, "Error"); // @@correction026
		SetResultString((strResult + " Cannot open file for writing").c_str()); // @@correction026
		return false;
	}
