							 ...
							 float fTerrainTexCoordBase)
{
	...
	{
		LogBox(XOR("You cannot add more than 255 texture.")); // @@correction026
		return false;
	}
	...
	for (unsigned long i = 1; i < GetTextureCount(); ++i)
	{
		...
		{
			LogBox(XOR("Texture of the same name already exists."), "Duplicate"); // @@correction026
			return false;
		}
	}
	...
	if (!pResource->IsType(CGraphicImage::Type()))
	{
		LogBox(XOR("CTerrainImpl::GenerateTexture : It's not an image file. %s"), pResource->GetFileName()); // @@correction026
		return false;
	}
