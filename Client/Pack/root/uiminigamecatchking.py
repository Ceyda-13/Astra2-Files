...
class CatchKingSimpleWaitingPage(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.MiniGameCatchKing = None
		self.__LoadWindow()

	def SetMiniGameCatchKing(self, mini_game_catchking):
		self.MiniGameCatchKing = mini_game_catchking
	...
	def __ClickStartButton(self):
		if None == self.startQuestionDialog:
			...
			self.startQuestionDialog.SetText(localeInfo.MINI_GAME_CATCHKING_START_QUESTION % (30000 * self.betNumber, item.GetItemName(), self.betNumber))
			...
			w,h = self.startQuestionDialog.GetTextSize()
			self.startQuestionDialog.SetWidth(w + 60)
	...
	def __ClickHelpButton(self):
		if self.MiniGameCatchKing:
			if self.MiniGameCatchKing.waiting_page:
				self.MiniGameCatchKing.waiting_page.Show()
				
		self.Close()
		
	def OnUpdate(self):
		(xposEventSet, yposEventSet) = self.descBoard.GetGlobalPosition()
		event.UpdateEventSet(self.descIndex, xposEventSet + 7, -(yposEventSet + 7 + self.desc_y))
	...
	def Show(self):
		...
		event.ClearEventSet(self.descIndex)
		self.descIndex = event.RegisterEventSet(uiScriptLocale.CATCHKING_EVENT_SIMPLE_DESC)
	...
	def __ClickUpArrowButton(self):
		...
		if tempBet > player.GetItemCountByVnum(CARD_SET_ITEM_VNUM):
			return
...
class CatchKingGameWaitingPage(ui.ScriptWindow):
	class DescriptionBox(ui.Window):
		def __init__(self):
			ui.Window.__init__(self)
			self.descIndex = -1
	...
	def __ClickStartButton(self):
		if self.MiniGameCatchKing:
			if self.MiniGameCatchKing.simple_waiting_page:
				self.MiniGameCatchKing.simple_waiting_page.Show()
	...
	def Show(self):
		...
		event.ClearEventSet(self.descIndex)
		self.descIndex = event.RegisterEventSet(uiScriptLocale.CATCHKING_EVENT_DESC)
		event.SetFontColor(self.descIndex, 0.7843, 0.7843, 0.7843, 1.0)
		event.SetVisibleLineCount(self.descIndex, SHOW_LINE_COUNT_MAX)
		...
		if SHOW_LINE_COUNT_MAX >= total_line:
...
class CatchKingGamePage(ui.ScriptWindow):
	...
	def ShowPopupDialog(self, handCard, fieldCard, isFiveNear):
		...
		if handCard < fieldCard:
			self.popupResult.SetText(localeInfo.MINI_GAME_CATCHKING_HELP_MSG1)
		elif handCard == fieldCard:
			self.popupResult.SetText(localeInfo.MINI_GAME_CATCHKING_HELP_MSG2)
		elif handCard > fieldCard:
			self.popupResult.SetText(localeInfo.MINI_GAME_CATCHKING_HELP_MSG3)
	...
	def OverInToolTip(self, eventType, arg):
		...
		self.infoTooltip.AppendDescription(localeInfo.GetVariableValue("MINI_GAME_CATCHKING_TOOLTIP_CARD%d" % arg), 52)
		#aici
	...
	def __ClickRetryButton(self):
		if self.MiniGameCatchKing:
			self.MiniGameCatchKing.StartAgain()
...
class MiniGameCatchKing(ui.Window):
	...
	def Destroy(self):
		...
		if self.simple_waiting_page:
			self.simple_waiting_page.Destroy()
			self.simple_waiting_page = None
	...
	def __LoadWindow(self):
		...
		try:
			self.simple_waiting_page = CatchKingSimpleWaitingPage()
			self.simple_waiting_page.SetMiniGameCatchKing(self)

			self.waiting_page = CatchKingGameWaitingPage()
			self.waiting_page.SetMiniGameCatchKing(self)

			self.game_page = CatchKingGamePage()
			self.game_page.SetMiniGameCatchKing(self)
		except:
			import exception
	...
	def Close(self):
		if self.waiting_page:
			if self.waiting_page.IsShow():
				self.waiting_page.Close()
				
		if self.simple_waiting_page:
			if self.simple_waiting_page.IsShow():
				self.simple_waiting_page.Close()
