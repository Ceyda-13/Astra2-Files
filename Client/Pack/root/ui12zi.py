	...
	def OnUpdate(self):
		if True == self.BeadIcon.IsIn():
			...
			if leftTime == 0:
				...
				self.tooltipInfo.SetText(localeInfo.BEAD_TIME_REMAINING_TO_GET % (leftTime/60))
			# chat.AppendChat(0, "%d, %d, %d" % (curTime, leftTime, self.nextBeadUpdateTime))
			self.tooltipInfo.Show()
		else:
			self.tooltipInfo.Hide()

		self.beadInfo.SetPosition(18, 20)

	def NextBeadUpdateTime(self, value):
		curTime = app.GetGlobalTime() / 1000
		self.nextBeadUpdateTime = curTime + int(value)
		# chat.AppendChat(0, "Time: %d %d" % (curTime, int(value)))
		
	def SetBeadCount(self, value):
		self.beadInfo.SetNumber("%d" % int(value))
		# chat.AppendChat(0, "Bead :%d" % (int(value)))
...
class FloorLimitTimeWindow(ui.ScriptWindow):
	...
	def PressJumpButton(self):
		...
		self.btnJump.Hide()
		event.QuestButtonClick(constInfo.SHOW_BUTTON)
...
class Reward12ziWindow(ui.ScriptWindow):
			...
			for i in xrange(self.COLUMN_SIZE):
				for j in xrange(self.ROW_SIZE):
					if i%2 == j%2:
						if j%2 == 0:
							if localeInfo.IsARABIC():
								CheckButton = ui.MakeButton(self.CheckButtonWindow, 311-i*28, j*28, "", "d:/ymir work/ui/game/12zi/reward/", "check_yellow.sub", "clear_yellow.sub", "clear_yellow.sub")
							else:
							...
							if localeInfo.IsARABIC():
								CheckButton = ui.MakeButton(self.CheckButtonWindow, 311-i*28, j*28, "", "d:/ymir work/ui/game/12zi/reward/", "check_green.sub", "clear_green.sub", "clear_green.sub")
							else:
	...
	def SendClear(self, value):
		...
		# chat.AppendChat(0, "SendClear:%d %d" % (type, value))
