...
class CharacterDetailsUI(ui.ScriptWindow):
	...
	def __Initialize(self):
		...
		if ENABLE_PENDANT:
			...
			if ENABLE_WOLFMAN_CHARACTER:
				self.InfoList.append( [ localeInfo.DETAILS_RESIST_MOUNT_FALL,	localeInfo.DETAILS_TOOLTIP_RESIST_MOUNT_FALL	, item.GetApplyPoint( item.APPLY_RESIST_MOUNT_FALL ) ] )
		self.InfoList.append( [ "", "", self.CATEGORY_ENDLINE ] )
		...
		if ENABLE_NEW_BONUSES:
			self.InfoList.append( [ localeInfo.DETAILS_78, localeInfo.DETAILS_TOOLTIP_78, item.GetApplyPoint( item.APPLY_ATTBONUS_STONE ) ] )
			self.InfoList.append( [ localeInfo.DETAILS_79, localeInfo.DETAILS_TOOLTIP_79, item.GetApplyPoint( item.APPLY_ATTBONUS_BOSS ) ] )
		...
		for i in xrange(self.UI_MAX_COUNT) :
			...
			if idx < len(self.InfoList) :
				...
				if type == self.CATEGORY_STARTLINE:
					...
					if ENABLE_ELEMENT_ADD and type >= player.POINT_ENCHANT_ELECT and type <= player.POINT_ENCHANT_DARK:
						value += player.GetStatus(item.GetApplyPoint(item.APPLY_ENCHANT_AVATAR))
