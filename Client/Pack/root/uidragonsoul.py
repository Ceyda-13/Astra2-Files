...
class DragonSoulWindow(ui.ScriptWindow):
	...
	def SetInventoryPage(self, page):
		...
		for i in xrange(1, 6 if ENABLE_DS_GRADE_MYTH else 5):
			self.inventoryTab[(page + i) % 6 if ENABLE_DS_GRADE_MYTH else 5].SetUp()
...
# @@@correction051 END
	...
	def __InventoryLocalSlotPosToGlobalSlotPos(self, window_type, local_slot_pos):
		...
		return (self.DSKindIndex * (6 if ENABLE_DS_GRADE_MYTH else 5) * player.DRAGON_SOUL_PAGE_SIZE) + self.inventoryPageIndex * player.DRAGON_SOUL_PAGE_SIZE + local_slot_pos
	...
	def ShowToolTip(self, window_type, slotIndex):
		if None != selfs.wndToolTipItem:
			if player.INVENTORY == window_type:
				...
				selfs.wndToolTipItem.SetInventoryItem(slotIndex, player.DRAGON_SOUL_INVENTORY)
	...
	def SelectItemSlot(self, itemSlotIndex):
		...
		if mouseModule.mouseController.isAttached():
			...
			# @@@correction010 BEGIN (changed behavior)
		...
		try:
			...
			if selfs.wndDragonSoulRefine.IsShow():
				if uiPrivateShopBuilder.IsBuildingPrivateShop():
					...
					return
				selfs.wndDragonSoulRefine.AutoSetItem((player.DRAGON_SOUL_INVENTORY, slotIndex), 1)
				return
		except:
			pass
	...
	def SelectEquipItemSlot(self, itemSlotIndex):
		...
		if mouseModule.mouseController.isAttached():
			...
			if player.SLOT_TYPE_INVENTORY == attachedSlotType and itemSlotIndex == attachedSlotPos:
				mouseModule.mouseController.DeattachObject() # @@@correction010
				return

			# @@@correction010 BEGIN (enable only the extract items so far)
			attachedItemVID = mouseModule.mouseController.GetAttachedItemIndex()
			attachedInvenType = player.SlotTypeToInvenType(attachedSlotType)
			if player.SLOT_TYPE_INVENTORY == attachedSlotType:
				ITEM_USE = 3
	...
	def ActivateDragonSoulByExtern(self, deck):
		...
		self.RefreshEquipSlotWindow()
		self.ActivateEquipSlotWindow(deck) # @@@correction051

	def DeactivateDragonSoul(self):
		self.isActivated = False
		self.activateButton.SetUp()
		self.DeactivateEquipSlotWindow() # @@@correction051

	if ENABLE_AUTO_DS_REFINE:
		def OpenRefineWindow(self):
			if selfs.wndGame:
				selfs.wndGame.BINARY_DragonSoulRefineWindow_Open()

		def OpenShopWindow(self):
			net.SendChatPacket("/npc_open " + str(51)) 
...
class DragonSoulRefineWindow(ui.ScriptWindow):
	...
	def __GetRefineStrengthInfo (self, itemVnum):
		try:
			...
			if not (item.ITEM_TYPE_MATERIAL == item.GetItemType() \
					and (item.MATERIAL_DS_REFINE_NORMAL <= item.GetItemSubType() and item.GetItemSubType() <= item.MATERIAL_DS_REFINE_MASTER if ENABLE_DS_REFINE_MASTER else item.MATERIAL_DS_REFINE_HOLLY)):
				return None
	...
	def __OverInRefineItem(self, slotIndex):
		if self.refineItemInfo.has_key(slotIndex):
			...
			selfs.wndToolTipItem.SetInventoryItem(inven_pos, inven_type)
	...
	if ENABLE_AUTO_DS_REFINE:
		...
		def OnUpdate(self):
			...
			if self.autoRefineTime + 0.0 > float(app.GetTime()):
				return
