...
class MyCharacters :
	class MyUnit :
		if ENABLE_ACCE_SYSTEM:
			...
			def __init__(self, const_id, name, level, race, playtime, guildname, form, hair, stat_str, stat_dex, stat_hth, stat_int, change_name): #hair, acce,
				self.UnitDataDic = {
					"ID" 	: 	const_id,
					"NAME"	:	name,
					"LEVEL"	:	level,
	...
	def LoadCharacterData(self) :
		...
		for i in xrange(5) :
			...
			last_playtime	= net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_PLAYTIME)
			...
			if ENABLE_ACCE_SYSTEM:
				...
				self.myUnitDic[i] = self.MyUnit(i, name, level, race, playtime, guildname, form, hair, stat_str, stat_dex, stat_hth, stat_int, change_name) #acce, stat_str,
		...
		for i in xrange(len(self.PriorityData)) :
			...
			if ENABLE_ACCE_SYSTEM:
				...
				self.MakeCharacter(i, DestDataDic["NAME"], DestDataDic["RACE"], DestDataDic["FORM"], DestDataDic["HAIR"]) #DestDataDic["HAIR"], DestDataDic["ACCE"]
	...
	def MakeCharacter(self, slot, name, race, form, hair, acce = 0):
		...
		if ENABLE_WOLFMAN_CHARACTER and chr.RaceToJob(race) == JOB_WOLFMAN:
...
class SelectCharacterWindow(ui.Window) :	
	...
	if ENABLE_WOLFMAN_CHARACTER:
		RACE_FACE_PATH[playerSettingModule.RACE_WOLFMAN_M] = "D:/ymir work/ui/intro/public_intro/face/face_wolfman_m_0"
		DISC_FACE_PATH[playerSettingModule.RACE_WOLFMAN_M] = "icon/face/wolfman_m.tga"
	...
	'''if ENABLE_DELETE_FAILURE_TYPE:
		if app.LOCALE_SERVICE_EUROPE:
