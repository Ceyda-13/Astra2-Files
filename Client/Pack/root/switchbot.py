...
class ItemTabBar(ui.Window):
	...
	class ItemTab(ui.Bar):
		...
		def __init__(self,parent,tabnum,index = 0,vnum = 0): ## Init ItemTab
			...
			self.sunRise = sunRise(self)
			self.sunRise.SetParentProxy(self.parentWindow.parentWindow)
			self.sunRise.SetPosition(10,35+36*3+4*2)
			self.sunRise.Hide()
			self.vnum = 0
			self.SetIndex(index)
		...
		def Select(self):
			for a in self.parentWindow.tabList:
				...
				self.parentWindow.tabList[a].sunRise.Hide()
			self.sunRise.Show()
			self.SetSize(self.width,self.height_selected)
			self.Update()
			# self.parentWindow.parentWindow.but_deactivate_all.SetParent(self.sunRise)
			...
			self.parentWindow.parentWindow.enable_rebuy.SetParent(self.sunRise)
			self.parentWindow.parentWindow.enable_rebuy.SetPosition(10,200+10)
			
			self.parentWindow.parentWindow.help_show_rare.SetParent(self.sunRise)
			self.parentWindow.parentWindow.help_rebuy.SetParent(self.sunRise)
			# self.parentWindow.parentWindow.help_stop_all.SetParent(self.sunRise)
			self.parentWindow.parentWindow.help_duration.SetParent(self.sunRise)
		...
		def checkSwitch(self):
			...
			if ok == 1 or self.prob >= 90:
				if self.prob >= 90:
					chat.AppendChat(1,"Slot %d: Baþarýlý! sunRise Efsun Botu" % (self.index+1))
					self.sunRise.Finish()
					return
		...
		def SetIndex(self,index):
			self.index = index
			self.sunRise.index = index
			self.sunRise.Starter.SetText("Efsun ("+str(index+1)+") Baþlat")
			self.Update()
			self.UpdateItem()
		...
		def GetProb_rare(self):
			...
			for i in range(0,2):
				try:
					...
					self.sunRise.ibi[i+6].SetText(affectString)
				except:
					pass
			prob = 0
			max  = 0
			...
			for x in yp:
				if yp[x] in val2 and val2[yp[x]] >= int(self.sunRise.boni[x+5][3].GetText()):
					prob = prob+1
				max = max+1
			if max > 0:
		...
		def GetProb(self):
			...
			for i in range(0,5):
				try:
					affectString = AFFECT_DICT[values[i][0]](values[i][1])
					val2[values[i][0]] = values[i][1]
					self.sunRise.ibi[i].SetText(affectString)
				except:
					pass
			prob = 0
			max  = 0
			yp = self.GenList()
			for x in yp:
				if yp[x] in val2 and val2[yp[x]] >= int(self.sunRise.boni[x][3].GetText()):
					prob = prob+1
				max = max+1
			if max > 0:
				prozent = 100/max*prob
		...
		def GenList(self):
			ret = {}
			for i in range(0,5):
				if self.sunRise.boni[5-i][1].selected.value != 0:
					ret[5-i] = self.sunRise.boni[5-i][1].selected.value
			return ret
		def GenList_rare(self):
			ret = {}
			for i in range(0,2):
				if self.sunRise.boni[6+i][1].selected.value != 0:
					ret[i+1] = self.sunRise.boni[6+i][1].selected.value
			return ret
	...
	def __init__(self,parent):
		...
		self.AddText.SetText("Üzerine Ýtem Sürükle")
...
class Bot(ui.BoardWithTitleBar):
		...
		def parse_dir(self,dir):
			...
			for listWin in self.txtlist:
				...
				if dir >= 3:
					...
					w+=92
					if itm.sunRise.EnableRareBoni.checked == 1:
						w+=15
	...
	def __init__(self):
		...
		self.help_add_item = HelpBar(0.8,'Üzerine Item Sürükle')
	...
	def _Hide(self):
		...
		self.sunRise_bot_shown = 0
		
		
	def deactivate_all(self):
		for a in self.itemTabBar.tabList:
			self.itemTabBar.tabList[a].sunRise.Starter.SetUp()
			self.itemTabBar.tabList[a].sunRise.Starter.OnToggleUp()
			self.itemTabBar.tabList[a].sunRise.Starter_rare_boni.SetUp()
			self.itemTabBar.tabList[a].sunRise.Starter_rare_boni.OnToggleUp()
		pass
	...
	def OnUpdate(self):
		if self.AboutWindow:
			if self.AboutWindow.x_counter > 1:
				...
				self.AboutWindow.text6.SetText("sunRise: %0.1f" % (self.AboutWindow.x_counter/45.0))
	...
	def about(self):
		...
		self.AboutWindow.text1.SetText("sunRise")
		...
		self.AboutWindow.text6.SetText("sunRise: %d" % self.AboutWindow.x_counter)
