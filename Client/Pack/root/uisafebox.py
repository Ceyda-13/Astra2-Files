...
class SafeboxWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		if ENABLE_SORT_ITEMS:
			self.GetChild2("YenileButton").SetEvent(ui.__mem_func__(self.ClickYenileButton))
	...
	def RefreshSafebox(self):
		...
		for i in xrange(safebox.SAFEBOX_PAGE_SIZE):
			...
			if ENABLE_CHANGE_LOOK_SYSTEM: # @@@correction025
				itemTransmutedVnum = safebox.GetItemChangeLook(slotIndex)
	...
	def __ShowToolTip(self, slotIndex):
		if selfs.wndToolTipItem:
			selfs.wndToolTipItem.SetSafeBoxItem(slotIndex)
	...
	if ENABLE_SORT_ITEMS:
		def ClickYenileButton(self):
			net.SendChatPacket('/click_sort_safebox_items')
...
class MallWindow(ui.ScriptWindow):
	...
	def RefreshMall(self):
		...
		for i in xrange(safebox.GetMallSize()):
			...
			if ENABLE_CHANGE_LOOK_SYSTEM: # @@@correction025
				itemTransmutedVnum = safebox.GetMallItemChangeLook(i)
				if itemTransmutedVnum:
					self.wndItem.DisableCoverButton(i)
				else:
					self.wndItem.EnableCoverButton(i)
	...
	def __ShowToolTip(self, slotIndex):
		if selfs.wndToolTipItem:
			selfs.wndToolTipItem.SetMallItem(slotIndex)
