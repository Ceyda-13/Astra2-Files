...
class RumiWaitingPage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.scrollbar.SetUpScrollButtonEvent(ui.__mem_func__(self.__OnScroll))
			self.scrollbar.SetDownScrollButtonEvent(ui.__mem_func__(self.__OnScroll))
...
class RumiGamePage(ui.ScriptWindow):
	...
	def OnUpdate(self):
		...
		if len(self.card_move_queue) > 0:
			if False == self.move_img.GetMove():
				...
				(dslx, dstly) = self.card_slot[dst_pos].GetSlotLocalPosition(dst_index)
				self.move_img.SetMovePosition(dslx, dstly)
	...
	def CardMoveStartEvent(self):
		if len(self.card_move_queue) > 0:
			...
			self.move_img.LoadImage(CARD_IMG_DICT[dst_color * 10 + dst_number])
			...
			self.move_img.SetPosition(local_x, local_y)
	...
	def CardMoveEndEvnet(self):
		if len(self.card_move_queue) > 0:
			...
			self.card_slot[dst_pos].SetSlot(dst_index, dst_index, 1, 1, self.card_img_dict.get(dst_color * 10 + dst_number))
			self.move_img.Hide()

			if len(self.card_move_queue) == 0 and False == self.score_text_effect.IsShow() and False == self.score_effect1.IsShow() and False == self.score_effect2.IsShow() and False == self.score_effect3.IsShow():
				self.lock = False
	...
	def IncreaseScore(self, score, total_score):
		self.lock = True
	...
	def MoveCard(self, srcCard, dstCard):
		(src_pos, src_index, src_color, src_number) = srcCard
		(dst_pos, dst_index, dst_color, dst_number) = dstCard

		# chat.AppendChat(0, "Veri: Slot:%d Slot index:%d Renk:%d Numara:%d : Slot:%d Slot index:%d Renk:%d Numara:%d" % (src_pos, src_index, src_color, src_number, dst_pos, dst_index, dst_color, dst_number))
	...
	if ENABLE_MINI_GAME_OKEY_NORMAL:
		def SetOkeyNormalBG(self):
			if player.GetMiniGameOkeyNormal():
				if self.back_ground:
...
class MiniGameRumi(ui.Window):
	...
	def MoveCard(self, srcCard, dstCard):
		self.game_page.MoveCard(srcCard, dstCard)

	def SetDeckCount(self, deck_card_count):
		self.game_page.SetDeckCount(deck_card_count)

	def IncreaseScore(self, score, total_score):
		self.game_page.IncreaseScore(score, total_score)
