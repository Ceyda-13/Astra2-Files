...
if ENABLE_GUILD_DRAGONLAIR_SYSTEM: # @@@correction040
	import background
...
class OptionDialog(ui.ScriptWindow):
	...
	def __Initialize(self):
		...
		if ENABLE_NEW_MINIMAP: # @@@correction059
			self.minimapChangeButtonDict = {}
		if WJ_SHOW_MOB_INFO:
			self.showMobInfoButtonList = []
		if ENABLE_DOG_MODE:
			self.dogModeButtonList = []
		if ENABLE_YANG_INFO:
			self.yangInfoButtonList = []
		if ENABLE_AUTO_PICKUP:
			self.autoPickButtonList = []
		if ENABLE_BLOCK_EXP:
			self.blockExpButtonList = []
	...
	def __Load_BindObject(self):
		try:
			...
			if ENABLE_BLOCK_EQUIPMENT: # @@@correction042
			...
			if ENABLE_NEW_MINIMAP: # @@@correction059
				self.minimapChangeButtonDict[0] = GetObject("minimap_new")
			if WJ_SHOW_MOB_INFO:
			...
			if ENABLE_YANG_INFO:
				self.yangInfoButtonList.append(GetObject("yang_info_open"))
				self.yangInfoButtonList.append(GetObject("yang_info_close"))
			if ENABLE_AUTO_PICKUP:
				self.autoPickButtonList.append(GetObject("auto_pick_open"))
				self.autoPickButtonList.append(GetObject("auto_pick_close"))
			if ENABLE_BLOCK_EXP:
				self.blockExpButtonList.append(GetObject("exp_block_open"))
				self.blockExpButtonList.append(GetObject("exp_block_close"))
	...
	def __Load(self):
		...
		if ENABLE_BLOCK_EQUIPMENT: # @@@correction042
			self.blockButtonList[6].SetToggleDownEvent(self.__OnClickBlockViewEquipmentButton) # @@@correction042
		...
		if ENABLE_NEW_MINIMAP: # @@@correction059
			for i in xrange(len(self.minimapChangeButtonDict)):
				self.minimapChangeButtonDict[i].SetToggleUpEvent(self.__OnClickMiniMap, 0)
				self.minimapChangeButtonDict[i].SetToggleDownEvent(self.__OnClickMiniMap, 1)
		...
		if ENABLE_YANG_INFO:
			self.yangInfoButtonList[0].SAFE_SetEvent(self.__OnClickYangInfo)
			self.yangInfoButtonList[1].SAFE_SetEvent(self.__OffClickYangInfo)

		if ENABLE_AUTO_PICKUP:
			self.autoPickButtonList[0].SAFE_SetEvent(self.__OnClickAutoPick)
			self.autoPickButtonList[1].SAFE_SetEvent(self.__OffClickAutoPick)

		if ENABLE_BLOCK_EXP:
			self.blockExpButtonList[0].SAFE_SetEvent(self.__OnClickBlockExp)
			self.blockExpButtonList[1].SAFE_SetEvent(self.__OffClickBlockExp)
			self.blockExpButtonList[1].Down()
	...
	if ENABLE_NEW_MINIMAP: # @@@correction059
		def __OnClickMiniMap(self, i):
			selfs.MINIMAP = i
			selfs.wndMiniMap.ChangeMiniMap(i)
	...
	if ENABLE_YANG_INFO:
		def __OnClickYangInfo(self):
			systemSetting.SetYangInfo(True)
			self.RefreshYangInfo()

		def __OffClickYangInfo(self):
			systemSetting.SetYangInfo(False)
			self.RefreshYangInfo()

		def RefreshYangInfo(self):
			if systemSetting.GetYangInfo():
				self.yangInfoButtonList[0].Down()
				self.yangInfoButtonList[1].SetUp()
			else:
				self.yangInfoButtonList[0].SetUp()
				self.yangInfoButtonList[1].Down()

	if ENABLE_AUTO_PICKUP:
		def __OnClickAutoPick(self):
			systemSetting.SetAutoPick(True)
			self.RefreshAutoPick()

		def __OffClickAutoPick(self):
			systemSetting.SetAutoPick(False)
			self.RefreshAutoPick()

		def RefreshAutoPick(self):
			if systemSetting.GetAutoPick():
				net.SendChatPacket("/fasstt_pick 1")
				self.autoPickButtonList[0].Down()
				self.autoPickButtonList[1].SetUp()
			else:
				net.SendChatPacket("/fasstt_pick 0")
				self.autoPickButtonList[0].SetUp()
				self.autoPickButtonList[1].Down()

	if ENABLE_BLOCK_EXP:
		def __OnClickBlockExp(self):
			self.RefreshBlockExp(1)

		def __OffClickBlockExp(self):
			self.RefreshBlockExp(0)

		def RefreshBlockExp(self, i):
			if i:
				net.SendChatPacket("/blockk_exp 1")
				self.blockExpButtonList[0].Down()
				self.blockExpButtonList[1].SetUp()
			else:
				net.SendChatPacket("/blockk_exp 0")
				self.blockExpButtonList[0].SetUp()
				self.blockExpButtonList[1].Down()
