	...
	def execfile(self, filename, dic):
		...
		for allowed_module_name in self.allowed_modules:
			try:
				...
				sys.modules['dbg'].TraceError("UISCRIPT_LOAD_ERROR: Could not import %s [filename %s]" % (allowed_module_name, filename)) # @@@correction005 (sys.stderr.write to dbg.TraceError)
				pass
		...
		try:
			...
			old_modules['dbg'].TraceError("UISCRIPT_LOAD_ERROR: %s [filename %s]" % (str(e), filename)) # @@@correction005 (sys.stderr.write to dbg.TraceError)
		finally:
