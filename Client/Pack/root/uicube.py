...
class CubeResultWindow(ui.ScriptWindow):
	...
	def __OnOverInItem(self, slotIndex):
		if selfs.wndToolTipItem:
			if 0 != self.itemVnum:
				selfs.wndToolTipItem.SetItemToolTip(self.itemVnum)
...
class CubeWindow(ui.ScriptWindow):
	...
	def __OverOutMaterialSlot(self):
		selfs.wndToolTipItem.SetCannotUseItemForceSetDisableColor(True)
		selfs.wndToolTipItem.HideToolTip()
	...
	def __OverInCubeResultSlot(self, trash, resultIndex):
		...
		selfs.wndToolTipItem.AddItemData(itemVnum, metinSlot, attrSlot)
	...
	def __OverInMaterialSlot(self, trash, resultIndex, col):
		selfs.wndToolTipItem.ClearToolTip()
		selfs.wndToolTipItem.SetCannotUseItemForceSetDisableColor(False)
		...
		for itemVnum, count in materialInfo[col]:
			...
			if player.GetItemCountByVnum(itemVnum) >= count:
				selfs.wndToolTipItem.AppendTextLine("%s" % (item.GetItemName()), grp.GenerateColor(0.5411, 0.7254, 0.5568, 1.0)).SetFeather()
			else:
				selfs.wndToolTipItem.AppendTextLine("%s" % (item.GetItemName()), grp.GenerateColor(0.9, 0.4745, 0.4627, 1.0)).SetFeather()

			if i < materialCount - 1:
				selfs.wndToolTipItem.AppendTextLine(uiScriptLocale.CUBE_REQUIRE_MATERIAL_OR)

			i = i + 1

		selfs.wndToolTipItem.Show()
	...
	def __OnOverInItem(self, slotIndex):
		if selfs.wndToolTipItem:
			if self.cubeItemInfo.has_key(slotIndex):
				selfs.wndToolTipItem.SetInventoryItem(self.cubeItemInfo[slotIndex])
