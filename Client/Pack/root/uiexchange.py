...
class ExchangeDialog(ui.ScriptWindow):
	...
	def OpenDialog(self):
		if ENABLE_LEVEL_IN_TRADE: # @@@correction018
	...
	def SelectOwnerEmptySlot(self, SlotIndex):
		...
		if mouseModule.mouseController.IsAttachedMoney():
			...
			if (player.SLOT_TYPE_INVENTORY == attachedSlotType
				...
				if WJ_ENABLE_TRADABLE_ICON:
					self.SetCantMouseEventSlot(attachedInvenType, SrcSlotNumber, DstSlotNumber)
	...
	def RefreshOwnerSlot(self):
		for i in xrange(exchange.EXCHANGE_ITEM_MAX_NUM):
			...
			if ENABLE_CHANGE_LOOK_SYSTEM: # @@@correction025
				itemTransmutedVnum = exchange.GetItemChangeLook(i, True)
	...
	def RefreshTargetSlot(self):
		for i in xrange(exchange.EXCHANGE_ITEM_MAX_NUM):
			...
			if ENABLE_CHANGE_LOOK_SYSTEM: # @@@correction025
				itemTransmutedVnum = exchange.GetItemChangeLook(i, False)
				if itemTransmutedVnum:
	...
	def OverInOwnerItem(self, slotIndex):
		...
		if 0 != selfs.wndToolTipItem:
			selfs.wndToolTipItem.SetExchangeOwnerItem(slotIndex)

	def OverInTargetItem(self, slotIndex):

		if 0 != selfs.wndToolTipItem:
			selfs.wndToolTipItem.SetExchangeTargetItem(slotIndex)
	...
	def OnUpdate(self):
		if WJ_ENABLE_TRADABLE_ICON:
			for [inven_type, idx, exidx] in self.AddList:
				self.SetCantMouseEventSlot(inven_type, idx, exidx)
	...
	if WJ_ENABLE_TRADABLE_ICON:
		def SetCantMouseEventSlot(self, inven_type, idx, exidx):
			if not [inven_type, idx, exidx] in self.AddList:
				self.AddList.append([inven_type, idx, exidx])

			if inven_type == player.INVENTORY:
				selfs.wndInventory.SetCantMouseEventSlot(idx)

		def SetCanMouseEventSlot(self, inven_type, idx, exidx):
			if [inven_type, idx, exidx] in self.AddList:
				self.AddList.remove([inven_type, idx, exidx])
