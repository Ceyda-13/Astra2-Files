...
class MyShopDecoration(ui.ScriptWindow):
	...
	def Open(self):	
		...
		self.RightBoard = self.GetChild("RightBoard")

		self.RenderTarget = self.GetChild("RenderTarget")
		self.RenderTarget.SetAutoRotation(1.0)
		self.RenderTarget.SetRenderTargetData(30000)
		self.RenderTarget.ZoomOut(-1.99)
	...
	def UseCompleteButton(self) :	
		...
		# player.SendMyShopDecoSet(self.TitleType, self.PolyVnum)
		selfs.wndPrivateShopBuilder.Open("", self.TitleType, self.PolyVnum)
		self.Close()
		
	def SelectShopModel(self, idx) :
		PolyVnum = self.DecoObjList[idx].GetInfo()
		self.PolyVnum = PolyVnum
		self.RenderTarget.SetRenderTargetData(PolyVnum)
