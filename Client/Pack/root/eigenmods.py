...
class EigenartigSwitchBotDialog(ui.ScriptWindow):
	...
	def AddGui(self):
		Gui = [
		[[ui.BoardWithTitleBar, ""], [508, 404], [0,0], [["SetCenterPosition", [""]], ["SetCloseEvent", [self.__del__]], ["SetTitleName", ["Efsun Botu "]]], ["float","movable"]],			
		...
		chat.AppendChat(1, "Ýtemi silmek için üzerine çift týklayýn...")
		...
		if not type:	
			if not self.Cfg[Tab][0]:
				if self.Config[Tab][0] == -1:
					chat.AppendChat(chat.CHAT_TYPE_INFO, "Hiçbir item seçilmemiþ!")
					return			
			
				for Slot in xrange(player.INVENTORY_PAGE_SIZE*4):
					itemVNum = player.GetItemIndex(Slot)
					if itemVNum == 71084:
						self.Cfg[Tab][1] = [Slot, player.GetItemCount(Slot)]
						break
			
				self.DefineBoni()
				chat.AppendChat(1, "Efsunbot baþladý.")
				self.Cfg[Tab][0] = 1			
				self.Gui[self.lenGui+Tab].SetbtnText(self.Cfg[Tab][0])
			else:
				self.Cfg[Tab][0] = 0
			...
			if not self.Cfg[Tab][7]:
				...
				for Slot in xrange(player.INVENTORY_PAGE_SIZE*4):
					itemVNum = player.GetItemIndex(Slot)
					if itemVNum == 71052:
						self.Cfg[Tab][8] = [Slot, player.GetItemCount(Slot)]
						break
			
				self.DefineBoni67()
				chat.AppendChat(1, "6/7 Efsunbot baþladý.")
				self.Cfg[Tab][7] = 1			
				self.Gui[self.lenGui+Tab].SetbtnText67(self.Cfg[Tab][7])
			else:
				self.Cfg[Tab][7] = 0
				chat.AppendChat(1, "6/7 Efsunbot durdu.")
				self.Gui[self.lenGui+Tab].SetbtnText67(self.Cfg[Tab][7])		
	...
	def ControllBoni(self, Boni, Values, tabNumber):
		try:
			...
			chat.AppendChat(1, "Efsun Sayýsý: " + str(self.Cfg[tabNumber][5]))
			chat.AppendChat(1, "Efsunlar: " + str(Boni))
			chat.AppendChat(1, "Deðerler: " + str(Values))
		except:
			if player.GetItemCountByVnum(71084) <= 0:
				if shop.IsOpen():
					...
					chat.AppendChat(1, "Efsun nesnesi kalmadý")
					self.Cfg[tabNumber][0] = 0
					self.Gui[self.lenGui+tabNumber].SetbtnText(self.Cfg[tabNumber][0])
					return
			...
			for Slot in xrange(player.INVENTORY_PAGE_SIZE*4):
				...
				if ItemValue == 71084:
					if self.Cfg[tabNumber][0] == 0:
						return
					self.Cfg[tabNumber][1] = [Slot, player.GetItemCount(Slot)]
					net.SendItemUseToItemPacket(Slot, self.Config[tabNumber][0])
					self.Gui[self.lenGui+Tab].RefreshToolTip()
					break
	...
	def ControllBoni67(self, Boni, Values, tabNumber):
		try:
			...
			self.Gui[self.lenGui+Tab].RefreshToolTip()
			chat.AppendChat(1, "Efsun Sayýsý: " + str(self.Cfg[tabNumber][12]))
			chat.AppendChat(1, "Efsunlar: " + str(Boni))
			chat.AppendChat(1, "Deðerler: " + str(Values))
		except:
			if player.GetItemCountByVnum(71052) <= 0:
				if shop.IsOpen():
					...
					chat.AppendChat(1, "2.Efsun nesnesi kalmadý")
					self.Cfg[tabNumber][7] = 0
					self.Gui[self.lenGui+tabNumber].SetbtnText67(self.Cfg[tabNumber][7])
					return
			...
			for Slot in xrange(player.INVENTORY_PAGE_SIZE*4):
				...
				if ItemValue == 71052:
					if self.Cfg[tabNumber][7] == 0:
						return
					self.Cfg[tabNumber][8] = [Slot, player.GetItemCount(Slot)]
					net.SendItemUseToItemPacket(Slot, self.Config[tabNumber][0])
					self.Gui[self.lenGui+Tab].RefreshToolTip()
					break
	...
	def UpdateBoni(self, Index):
		...
		if (player.GetItemCount(self.Cfg[Index][1][0]) <= 0 and (self.Cfg[Index][3] != Boni or self.Cfg[Index][4] != Values)) or (self.Cfg[Index][3] != Boni or self.Cfg[Index][4] != Values) or app.GetTime() >= self.Cfg[Index][2] + 0.8:
...
##		
	...
	def __init__(self):
		...
		self.BonusIDListe = [["Seçilmedi", 0], ["Max HP.", 1], ["Max SP.", 2], ["Canlýlýk", 3], ["Zeka", 4], ["Güç", 5], ["Çeviklik", 6], ["Saldýrý Hýzý", 7], ["Hareket Hýzý", 8], ["Büyü Hýzý", 9], ["Hp Üretimi", 10], ["SP Üretimi", 11], ["Zehirleme Deðiþimi", 12], ["Kanama Saldýrýsý", 93], ["Sersemletme Þansý", 13], ["Yavaþlatma Þansý", 14], ["Kritik Vuruþ", 15], ["Delici Vuruþ", 16], ["Yarý Ýnsanlara Karþý Güçlü", 17], ["Hayvanlara Karþý Güçlü", 18], ["Orklara Karþý Güçlü", 19], ["Mistiklere Karþý Güçlü", 20], ["Ölümsüzlere Karþý Güçlü", 21], ["Þeytanlara Karþý Güçlü", 22], ["HP Absorbe", 23], ["SP Absorbe", 24], ["Beden K. Ataklarýn Blok.", 27], ["Oklardan Korunma Þansý", 28], ["Kýlýç Savunmasý", 29], ["Çift El Savunmasý", 30], ["Býçak Savunmasý", 31], ["Pençe Savunmasý", 97],["Ok K. Dayanýklýlýk", 34], ["Ateþe Karþý Dayanýklýlýk", 35], ["Þimþeðe Karþý Dayanýklýlýk", 36], ["Büyüye K. Dayanýklýlýk", 37], ["Rüzgara K. Dayanýklýlýk", 38], ["Vücut D. Yansýtmasý", 39], ["Zehre Karþý Koyma", 41], ["Kanama Saldýrýlarýnda Direnç", 94], ["Exp-Bonus", 43], ["Ýki Kat Yang", 44], ["Ýki Kat Ýtem", 45], ["Sersemlik K. Baðýþýklýlýk", 48], ["Yavaþlama K. Baðýþýklýlýk", 49], ["Saldýrý Deðeri", 53], ["Beceri Hasarý", 71], ["Ortalama Hasar", 72]]
		self.BonusIDListe2 = [["Seçilmedi", 0], ["Max HP.", 1], ["Max SP.", 2], ["Canlýlýk", 3], ["Zeka", 4], ["Güç", 5], ["Çeviklik", 6], ["Kritik Vuruþ", 15], ["Delici Vuruþ", 16], ["Saldýrý Deðeri", 53], ["Savaþçýya K. Güçlü", 59], ["Ninjaya K. Güçlü", 60], ["Suraya K. Güçlü", 61], ["Þamana K. Güçlü", 62], ["Lycana K. Güçlü", 95], ["Beceri Hasarý", 71], ["Ortalama Hasar", 72]]
		...
		self.gridSlowWind.SetUseSlotEvent(ui.__mem_func__(self.UseItemSlot))
...
class ToolTipWithoutMetins(uiToolTip.ItemToolTip):
	...
	def SetInventoryItem(self, slotIndex):
		...
		self.ResizeToolTip()
		self.AddItemData(itemVnum, [0,0,0], attrSlot)	
