...
def LoadLocaleFile(srcFileName, localeDict):
	...
	funcDict = {"SA":SA, "SNA":SNA, "SAA":SAA, "SAN":SAN, "SAAAA":SAAAA} # @@@correction013
...
if ENABLE_OFFLINE_PRIVATE_SHOP:
	MINIMAP_ZONE_NAME_DICT_BY_IDX = {
		...
		15 : MAP_WL_01,
		16 : MAP_NUSLUCK01,
		17 : MAP_SPIDERDUNGEON,
		18 : MAP_SPIDERDUNGEON_02,
		19 : MAP_SPIDERDUNGEON_03,
		20 : MAP_SKIPIA_DUNGEON_01,
		21 : MAP_SKIPIA_DUNGEON_02,
		22 : MAP_SKIPIA_DUNGEON_BOSS,
		23 : MAP_WEDDING_01,
		24 : MAP_GUILD_02,
		25 : MAP_T1,
		25 : MAP_T2,
		...
		27 : MAP_T4,
		28 : MAP_OXEVENT,
		32 : MAP_DEVILCATACOMB,
		33 : MAP_CAPEDRAGONHEAD,
		34 : MAP_DAWNMISTWOOD,
		35 : MAP_BAYBLACKSAND,
		36 : MAP_MT_THUNDER,
		37 : MAP_N_FLAME_DUNGEON_01,
		38 : MAP_N_SNOW_DUNGEON_01,
		39 : MAP_DAWNMIST_DUNGEON_01,
		40 : MAP_MT_TH_DUNGEON_01,
		44 : MAP_DUEL,
	}
	if ENABLE_GUILD_DRAGONLAIR_SYSTEM:
		MINIMAP_ZONE_NAME_DICT_BY_IDX[41] = MAP_N_FLAME_DRAGON
	...
	# import shop
	def NumberToPriceString(n, priceType):
		PriceTypeDict = {
			0 : CHEQUE_SYSTEM_UNIT_YANG,
		}
		if priceType in PriceTypeDict:
			if n <= 0 :
				return "0 %s" % (PriceTypeDict[priceType])
				
			return "%s %s" % ('.'.join([ i-3<0 and str(n)[:i] or str(n)[i-3:i] for i in range(len(str(n))%3, len(str(n))+1, 3) if i ]), PriceTypeDict[priceType])
		else:
			return "%s Unknown Type" % (str(n))
...
def NumberToMoneyStringNew(number):
	if number <= 0:
		return "0 Yang"
	...
	result = CutMoneyString(number, 12, 15, "|cFFc7b81fKT|r", result)
	result = CutMoneyString(number, 15, 18, "|cFFc7b81fKKT|r", result)
	result = CutMoneyString(number, 18, 20, "|cFFc7b81fO|r", result)
