...
class RefineDialog(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		if ENABLE_REFINE_PCT: # @@@correction020
...
class RefineDialogNew(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		if ENABLE_REFINE_RENEWAL:
			...
			self.checkBox.SetPosition(0, 82)
	...
	def Open(self, targetItemPos, nextGradeItemVnum, cost, prob, type):
		...
		if ENABLE_REFINE_RENEWAL:
			if player.GetMoney() < int(self.cost):
				self.costText.SetText("|Eemoji/refinefail|e " + localeInfo.REFINE_COSTNEW % (localeInfo.NumberToMoneyString(int(self.cost))))
			else:
				self.costText.SetText("|Eemoji/refineokey|e " + localeInfo.REFINE_COSTNEW % (localeInfo.NumberToMoneyString(int(self.cost))))
		else:
	...
	def Close(self):
		...
		if WJ_ENABLE_TRADABLE_ICON:
			self.SetCanMouseEventSlot(player.INVENTORY, self.targetItemPos)
	...
	def UpdateDialog(self):
		...
		newHeight = self.dialogHeight + 75 # @@@correction020
		if ENABLE_REFINE_RENEWAL:
			newHeight += 15
		newHeight -= 8
	...
	if WJ_ENABLE_TRADABLE_ICON:
		def OnUpdate(self):
			self.SetCantMouseEventSlot(player.INVENTORY, self.targetItemPos)
