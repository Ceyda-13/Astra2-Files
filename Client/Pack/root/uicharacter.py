...
class CharacterWindow(ui.ScriptWindow):
	...
	def __init__(self):
		...
		if ENABLE_FAST_STAT: # @@@correction028
	...
	def OverInItem(self, slotNumber):
		...
		if skill.SKILL_TYPE_ACTIVE == skillType:
			...
			if overInSkillGrade == skill.SKILL_GRADE_COUNT-1 and skillGrade == skill.SKILL_GRADE_COUNT:
				selfs.wndToolTipSkill.SetSkillNew(srcSlotIndex, skillIndex, skillGrade, skillLevel)
			elif overInSkillGrade == skillGrade:
				if ENABLE_SKILLS_LEVEL_OVER_P and skillGrade == 2:
					selfs.wndToolTipSkill.SetSkillOnlyName(srcSlotIndex, skillIndex, overInSkillGrade + 1)
				else:
					selfs.wndToolTipSkill.SetSkillNew(srcSlotIndex, skillIndex, overInSkillGrade, skillLevel)
			elif ENABLE_SKILLS_LEVEL_OVER_P and overInSkillGrade == skill.SKILL_GRADE_COUNT - 2 and skillGrade >= skill.SKILL_GRADE_COUNT - 1:
				selfs.wndToolTipSkill.SetSkillNew(srcSlotIndex, skillIndex, skillGrade, skillLevel)
			else:
				if ENABLE_SKILLS_LEVEL_OVER_P and overInSkillGrade == 1 and skillGrade == 2:
					selfs.wndToolTipSkill.SetSkillNew(srcSlotIndex, skillIndex, overInSkillGrade + 1, skillLevel)
				elif ENABLE_SKILLS_LEVEL_OVER_P and overInSkillGrade == 2 and skillGrade < 3:
					selfs.wndToolTipSkill.SetSkillOnlyName(srcSlotIndex, skillIndex, overInSkillGrade + 1)
				else:
					selfs.wndToolTipSkill.SetSkillOnlyName(srcSlotIndex, skillIndex, overInSkillGrade)
	...
	def OverOutItem(self):
		if 0 != selfs.wndToolTipSkill:
			selfs.wndToolTipSkill.HideToolTip()
	...
	def __RefreshSkillPage(self, name, slotCount):
		...
		for i in xrange(slotCount+1):
			...
			if not ENABLE_SKILLS_LEVEL_OVER_P:
	...
	def __SetSkillGroupName(self, race, group):
		...
		if 0 == group:
			...
			if ENABLE_WOLFMAN_CHARACTER and playerSettingModule.RACE_WOLFMAN_M == race:
	...
	if ENABLE_FAST_STAT: # @@@correction028
		def ChooseCountPlusStat(self, statusKey):
			...
			inputDialog.SetTitle(uiScriptLocale.GAME_STAT_UP)
		...
		def ChooseCountPlusStatConfirm(self, statusKey):
			...
			try:
				...
				if count <= 0:
					chat.AppendChat(chat.CHAT_TYPE_INFO, "L?fen Ge?rli Puan Girin.")
					return
				net.SendChatPacket(statusPlusCommand % count)
			except ValueError:
				chat.AppendChat(chat.CHAT_TYPE_INFO, "Bir Hata Olu?tu.")
