...
class FishEventGamePage(ui.ScriptWindow):
	def __init__(self):
		...
		self.fishPieceDict = {
			FISH_EVENT_SHAPE_1	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_1.sub"),
			FISH_EVENT_SHAPE_2	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_2.sub"),
			FISH_EVENT_SHAPE_3	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_3.sub"),
			FISH_EVENT_SHAPE_4	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_4.sub"),
			FISH_EVENT_SHAPE_5	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_5.sub"),
			FISH_EVENT_SHAPE_6	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_6.sub"),
			FISH_EVENT_SHAPE_7	: grpImage.Generate("D:/Ymir Work/UI/minigame/fish_event/fish_7.sub"),
		}

		self.fishPieceAdjustPosDict = {
			FISH_EVENT_SHAPE_1	: (0, 0),
			FISH_EVENT_SHAPE_2	: (0, 0),
			FISH_EVENT_SHAPE_3	: (-16, -16),
			FISH_EVENT_SHAPE_4	: (-16, -16),
			FISH_EVENT_SHAPE_5	: (-16, -16),
			FISH_EVENT_SHAPE_6	: (-8, -16),
			FISH_EVENT_SHAPE_7	: (-8, -16),
		}

		self.fishPieceSizeDict = {
			FISH_EVENT_SHAPE_1	: (1, 3),
			FISH_EVENT_SHAPE_2	: (1, 1),
			FISH_EVENT_SHAPE_3	: (2, 2),
			FISH_EVENT_SHAPE_4	: (2, 2),
			FISH_EVENT_SHAPE_5	: (2, 2),
			FISH_EVENT_SHAPE_6	: (3, 2),
			FISH_EVENT_SHAPE_7	: (3, 2),
		}
		self.fish = {
			FISH_EVENT_SHAPE_1	: ["D:/Ymir Work/UI/minigame/fish_event/fish_1.sub", (0, 6, 12)],
			FISH_EVENT_SHAPE_2	: ["D:/Ymir Work/UI/minigame/fish_event/fish_2.sub", (0)],
			FISH_EVENT_SHAPE_3	: ["D:/Ymir Work/UI/minigame/fish_event/fish_3.sub", (0, 6, 7)],
			FISH_EVENT_SHAPE_4	: ["D:/Ymir Work/UI/minigame/fish_event/fish_4.sub", (0, 1, 7)],
			FISH_EVENT_SHAPE_5	: ["D:/Ymir Work/UI/minigame/fish_event/fish_5.sub", (0, 1, 6, 7)],
			FISH_EVENT_SHAPE_6	: ["D:/Ymir Work/UI/minigame/fish_event/fish_6.sub", (0, 1, 7, 8)],
			FISH_EVENT_SHAPE_7	: ["D:/Ymir Work/UI/minigame/fish_event/fish_7.sub", (0, 1, 2, 6, 7, 8)],
		}
	...
	# def OnTop(self):
		...
		# selfs.wndInterFace.SetOnTopWindow(player.ON_TOP_WND_FISH_EVENT)
		# selfs.wndInterFace.RefreshMarkInventoryBag()
	...
	def Show(self):
		...
		if 0 == self.use_count:
			net.SendRequestFishEventBlock()

		mouseModule.mouseController.DeattachObjectPostProcess()
		special_item_pos = player.GetItemPosByVnum(player.INVENTORY, ITEM_FISH_EVENT_BOX_SPECIAL, 0)
		self.__AddFishEventSlot(SPECIAL_TYPE, special_item_pos)
		normal_item_pos = player.GetItemPosByVnum(player.INVENTORY, ITEM_FISH_EVENT_BOX, 0)
	...
	def OverInItem(self, slotIndex, type):
		if selfs.wndToolTipItem and self.fish_box_slot_pos[type] != -1:
			selfs.wndToolTipItem.SetInventoryItem(self.fish_box_slot_pos[type])
	...
	def __AddFishEventSlot(self, type, pos):
		...
		item_vnum  = player.GetItemIndex(player.INVENTORY, pos)
		item_count = player.GetItemCount(player.INVENTORY, pos)
	...
	def __SelectEmptySlot(self, slotIndex, type):
		...
		item_count = player.GetItemCount(player.INVENTORY, attachedSlotPos)
	...
	def __AddAccept(self, slotIndex):
		...
		net.SendAddFishBox(slotIndex, self.shape)
		self.shape = 0
		self.DeattachObject()

	def MiniGameFishAdd(self, pos, shape):
		if FISH_EVENT_SHAPE_3 == shape or FISH_EVENT_SHAPE_4 == shape or FISH_EVENT_SHAPE_6 == shape:
			shapeImage = self.fish[shape][0]
			if shape in self.fish:
				positionInfo = self.fish[shape][1]
				self.grid_slot.SetCoverButton(pos, shapeImage, shapeImage, shapeImage, shapeImage, False, False)
				self.grid_slot.SetAlwaysRenderCoverButton(pos, True)
				for i in xrange(len(positionInfo)):
					print pos, shape, i, positionInfo[i]
					self.grid_slot.SetSlot(pos + positionInfo[i], shape, 1, 1, 0)
	...
	def OnUpdate(self):
		...
		for type in range(1, FISH_EVENT_TYPE_MAX):
			...
			invenPage = selfs.wndInventory.GetInventoryPageIndex() ## 0 or 1
			...
			if min_range <= inven_slot_pos < max_range:
				...
				if selfs.wndInventory.wndItem:
					selfs.wndInventory.wndItem.SetCantMouseEventSlot(inven_slot_pos)
			...
			for index in range(1, FISH_EVENT_TYPE_MAX):
				...
				if inven_slot_pos >= player.INVENTORY_PAGE_SIZE:
					...
					if app.ENABLE_EXTEND_INVEN_SYSTEM:
						inven_page = selfs.wndInventory.GetInventoryPageIndex()
				...
				if selfs.wndInventory.wndItem:
					selfs.wndInventory.wndItem.SetCanMouseEventSlot(inven_slot_pos)
		else:
	...
	def __UseAccept(self, type):
		self.__QuestionCancel()
		if self.fish_box_slot_pos[type] != -1:
			net.SendUseFishBox(player.SLOT_TYPE_INVENTORY, self.fish_box_slot_pos[type])
	...
	def MiniGameFishUse(self, window, shape):
		...
		for index in range(1, FISH_EVENT_TYPE_MAX):
			item_count = player.GetItemCount(window, self.fish_box_slot_pos[index])
			item_vnum  = player.GetItemIndex(window, self.fish_box_slot_pos[index])
	...
	def DeattachObject(self):
		...
		for type in range(1, FISH_EVENT_TYPE_MAX):
			item_count = player.GetItemCount(player.INVENTORY, self.fish_box_slot_pos[type])
			if item_count:
				self.fish_box_slot[type].ActivateSlot(0)
			else:
				self.fish_box_slot[type].DeactivateSlot(0)
...
class MiniGameFish(ui.Window):
	...
	def MiniGameFishUse(self, window, shape):
		...
		self.cur_page.MiniGameFishUse(window, shape)
