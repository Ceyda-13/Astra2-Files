...
class WhisperDialog(ui.ScriptWindow):
	...
	def LoadDialog(self):
		...
		self.empireFlag.SetPosition(143, 12)
		if ENABLE_MULTI_LANGUAGE:
			self.languageFlag.SetPosition(163, 12)
	...
	def Minimize(self):
		...
		if None != self.eventMinimize:
			if ENABLE_MULTI_LANGUAGE:
				self.eventMinimize(self.targetName, self.empireID, self.languageID)
			else:
				self.eventMinimize(self.targetName, self.empireID)	
	...
	def OnMouseLeftButtonDown(self):
		...
		if hyperlink:
			if app.IsPressed(app.DIK_LALT):
				...
				selfs.wndGame.MakeHyperlinkTooltip(hyperlink)

	if app.ENABLE_MULTI_LANGUAGE:
		def SetFlag(self, empireID, languageID):
			if languageID == "":
				self.languageFlag.Hide()
			if empireID == "":
				self.empireFlag.Hide()

			self.languageFlag.LoadImage("multi/%s.tga" % (languageID))
			self.empireFlag.LoadImage("d:/ymir work/ui/empire/empire_%s_256.dds" % (empireID))
