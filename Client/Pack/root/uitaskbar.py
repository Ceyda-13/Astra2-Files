	...
	class ExpandedMoneyTaskBar(ui.ScriptWindow):
		...
		def LoadWindow(self):
			...
			try:
				...
				self.wndRedGemSlotIcon = self.GetChild("RedGem_Icon")
			except:
				import exception
...
class TaskBar(ui.ScriptWindow):
	...
	class SkillButton(ui.SlotWindow):
		...
		def OnOverInItem(self, dummy):
			selfs.wndToolTipSkill.SetSkill(self.skillIndex)
	...
	def LoadWindow(self):
		...
		if ENABLE_FAST_EQUIP: # @@@correction029
			toggleButtonDict[7]=self.GetChild("EkipmanButton")
	...
	def OverInItem(self, slotNumber):
		...
		if player.SLOT_TYPE_INVENTORY == Type:
			selfs.wndToolTipItem.SetInventoryItem(Position)
			selfs.wndToolTipSkill.HideToolTip()
			...
			selfs.wndToolTipSkill.SetSkillNew(Position, skillIndex, skillGrade, skillLevel)
