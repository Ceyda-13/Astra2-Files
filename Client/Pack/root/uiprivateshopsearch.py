...
player.GOLD_MAX = 0xffffffffffffffff
...
class OfflinePrivateShopSearch(ui.ScriptWindow):
	MAX_LINE_COUNT = 10
	MAX_CHAR_LEVEL = 105
	CLICK_LIMIT_TIME = 3
	...
	ITEM_MASK_TYPE_DIC = { 	
		...
		item.ITEM_TYPE_SKILLBOOK	: localeInfo.PRIVATESHOPSEARCH_SKILLFORGET,
	...
	if ENABLE_QUIVER_SYSTEM:
		WEAPON_MASK_SUBTYPE_DIC[1][item.WEAPON_QUIVER] = localeInfo.CATEGORY_WEAPON_WEAPON_QUIVER

	if ENABLE_WOLFMAN_CHARACTER:
		WEAPON_MASK_SUBTYPE_DIC
		WEAPON_MASK_SUBTYPE_DIC[4][item.WEAPON_CLAW] = localeInfo.CATEGORY_WEAPON_WEAPON_CLAW
	...
	subItemsUse = {
		...
		20							: localeInfo.PRIVATESHOPSEARCH_DIAMOND,
	}
	...
	subItemsFish = {
		0 : localeInfo.PRIVATESHOPSEARCH_LIVE,
		1 : localeInfo.PRIVATESHOPSEARCH_DEAD,
	}
	...
	subItemsResource = {
		0 : localeInfo.PRIVATESHOPSEARCH_MATERIAL,
		3 : localeInfo.PRIVATESHOPSEARCH_BLOOD_PEARL,
		4 : localeInfo.PRIVATESHOPSEARCH_BLUE_PEARL,
		5 : localeInfo.PRIVATESHOPSEARCH_WHITE_PEARL,
		7 : localeInfo.PRIVATESHOPSEARCH_CRYSTAL,
		8 : localeInfo.PRIVATESHOPSEARCH_GEM,
		9 : localeInfo.PRIVATESHOPSEARCH_STONE,
		1 : localeInfo.PRIVATESHOPSEARCH_METIN,
		1 : localeInfo.PRIVATESHOPSEARCH_ORE,
	 }
	...
	if ENABLE_ACCE_SYSTEM:
		subItemsCostume[item.COSTUME_TYPE_ACCE] = localeInfo.PRIVATESHOPSEARCH_COSTUME_ACCE

	if ENABLE_WEAPON_COSTUME_SYSTEM:
		subItemsCostume[item.COSTUME_TYPE_WEAPON] = localeInfo.CATEGORY_COSTUMES_COSTUME_WEAPON

	subItemsDS = {
		0 : localeInfo.PRIVATESHOPSEARCH_DS_WHITE,
		1 : localeInfo.PRIVATESHOPSEARCH_DS_RED,
		2 : localeInfo.PRIVATESHOPSEARCH_DS_GREEN,
		3 : localeInfo.PRIVATESHOPSEARCH_DS_BLUE,
		4 : localeInfo.PRIVATESHOPSEARCH_DS_YELLOW,
		5 : localeInfo.PRIVATESHOPSEARCH_DS_BLACK,
	}
	...
	ITEM_MASK_MASK_SUBTYPE_DICS = {
		...
		item.ITEM_TYPE_USE			: subItemsUse,
		item.ITEM_TYPE_MATERIAL		: subItemsMaterial,
		item.ITEM_TYPE_METIN		: subItemsMetin,
		item.ITEM_TYPE_FISH			: subItemsFish,
		item.ITEM_TYPE_BELT			: subItemsBelt,
		item.ITEM_TYPE_RESOURCE		: subItemsResource,
		item.ITEM_TYPE_ROD			: subItemsRod,
		item.ITEM_TYPE_UNIQUE		: subItemsUnique,
		item.ITEM_TYPE_SKILLBOOK	: subItemsSkillbook,
		item.ITEM_TYPE_QUEST		: subItemsQuest,
		item.ITEM_TYPE_TREASURE_BOX	: subItemsTreasureBox,
		item.ITEM_TYPE_SKILLBOOK	: subItemsSkillForget,
		item.ITEM_TYPE_GIFTBOX		: subItemsGiftbox,
		item.ITEM_TYPE_PICK			: subItemsPick,
		item.ITEM_TYPE_BLEND		: subItemsBlend,
		item.ITEM_TYPE_COSTUME		: subItemsCostume,
		item.ITEM_TYPE_RING			: subItemsRing,
		item.ITEM_TYPE_DS			: subItemsDS,
		item.ITEM_TYPE_SPECIAL		: subItemsSpecial,
	}
	...
	def __LoadWindow(self):
		try:
			...
			pyScrLoader.LoadScriptFile(self, "UIScript/PrivateShopSearch.py")
		except:
			import exception
			exception.Abort("OfflinePrivateShopSearch.__LoadWindow.UIScript/PrivateShopSearch.py")
	...
	def __MakeResultSlot(self):
		...
		for i in range(0,self.MAX_LINE_COUNT):
			...
			# itemSlotButtonImage.SetScale(CalcScale(533, 100, False), 1.0)
	...
	def RefreshList(self):
		# count = ops.GetPrivateShopSearchResultCount()
		maxcount = ops.GetPrivateShopSearchResultMaxCount()
		page = self.nowpagenumber#shop.GetPrivateShopSearchResultPage()

		for line, nameSlotList in self.SerchItemSlotList.items():
			(itemname, sellername, count, price) = ops.GetPrivateShopSearchResult(line + ((page - 1) * self.MAX_LINE_COUNT))
			if "" == itemname:
	...
	def lastnextbutton(self):
		maxsize = ops.GetPrivateShopSearchResultMaxCount()
	...
	def nextbutton(self):
		maxitemcount = ops.GetPrivateShopSearchResultMaxCount()
	...
	def Search(self):
		...
		text1 = self.mingoldvalue.GetText()
		text2 = self.maxgoldvalue.GetText()
		if ENABLE_MONEY_SHORTING:
			text1 = text1.replace("k", "000")
			text2 = text2.replace("k", "000")

		maxlevel = int(self.maxlevelvalue.GetText())
		mingold = int(text1)
		maxgold = int(text2)
		itemname = self.ItemNameValue.GetText().replace(" ", "_")
		
		if mingold > player.GOLD_MAX - 1:
			mingold = player.GOLD_MAX - 1
			self.mingoldvalue.SetText(str(player.GOLD_MAX - 1))
			
		if maxgold > player.GOLD_MAX - 1:
			maxgold = player.GOLD_MAX - 1
			self.maxgoldvalue.SetText(str(player.GOLD_MAX - 1))

		if itemname == "":
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Lütfen Ýsim Giriniz !")
			return

		if len(itemname) <= 2:
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Lütfen 2 den uzun isim giriniz !")
			return

		net.SendChatPacket("/click_offline_private_shop 4 %d %d %d %d %d %d %d %d %d %s" % (job, Masktype, Masksub, minrefine, maxrefine, minlevel, maxlevel, mingold, maxgold, itemname))
	...
	def OnBuyAcceptEvent(self):
		...
		(vid, slot) = ops.GetPrivateShopSearchInfo((self.selectitemRealindex) + ((self.nowpagenumber - 1) * self.MAX_LINE_COUNT))
		net.SendChatPacket("/click_offline_private_shop 5 %d %d" % (vid, slot))
		self.clearEffectEtc()
		# self.Search()
		self.popup.Close()
		self.popup = None
	...
	def __ShowToolTip(self, slotIndex):
		if selfs.wndToolTipItem:
			selfs.wndToolTipItem.SetPrivateSearchItem(slotIndex + ((self.nowpagenumber - 1) * self.MAX_LINE_COUNT))
