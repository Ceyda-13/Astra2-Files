	...
	def AttachObject(self, Owner, Type, SlotNumber, ItemIndex, count = 0):
		...
		try:
			...
			if Type == player.SLOT_TYPE_INVENTORY or\
				...
				or (ENABLE_SWITCHBOT and Type == player.SLOT_TYPE_SWITCHBOT):
	...
	def DeattachObject(self):
		...
		if self.AttachedIconHandle != 0:
			...
			if self.AttachedType == player.SLOT_TYPE_INVENTORY or\
				...
				or (ENABLE_SWITCHBOT and self.AttachedType == player.SLOT_TYPE_SWITCHBOT):
			...
			if ENABLE_FISH_EVENT and self.AttachedType == player.SLOT_TYPE_FISH_EVENT:
	...
	def Update(self, x, y):
		...
		if True == self.isAttached():
			if 0 != self.AttachedIconHandle:
				if ENABLE_FISH_EVENT and player.SLOT_TYPE_FISH_EVENT == self.GetAttachedType():
